...
- 文档与黄金澄清：`docs/USAGE_*`、`docs/OPERATIONS_*` 已补充 plan-b 缓存窗口语义（默认 300s，命中过期视为 stale 并清空），stderr 标签覆盖 `plan-b-*` 以及 `plan-b-hit/stale/empty`，批量黄金与自检黄金均已启用 plan-b 断言；backoff 章节保持既有 `[DNS-BACKOFF]` 契约。
