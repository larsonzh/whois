...
- WP-13A 代理访问契约/依赖 spike：新增专用代理 RFC、本地确定性 HTTP CONNECT/SOCKS5 fake proxy 探针与 OpenSSL 静态链接依赖探针；首轮协议合同 9/9 PASS（`out/artifacts/proxy_protocol_spike/20260829-030918`）。本切片不增加产品 CLI、不改变网络路径；HTTPS proxy 仍须通过九架构静态依赖门禁。
...
- WP-13A proxy-access contract/dependency spike: adds a dedicated proxy RFC, a deterministic local HTTP CONNECT/SOCKS5 fake-proxy probe, and an OpenSSL static-link dependency probe. The initial protocol contract passes 9/9 cases (`out/artifacts/proxy_protocol_spike/20260829-030918`). This slice adds no product CLI and changes no network path; HTTPS proxy remains gated on the nine-architecture static-dependency matrix.
...
- WP-10 响应读取上限语义审计（2026-08-25）：确认 `--buffer-size` 默认 512 KiB，并作为主查询路径传给 `wc_recv_until_idle(..., max_bytes)`，同时控制工作缓冲容量与实际网络接收上限；协议安全层 10 MiB 最大响应约束未接入主接收路径，因此不存在独立于现有选项的控制对象，决定不新增 `--max-bytes`。native Linux GCC socketpair 审计直接调用真实 `wc_recv_until_idle`：低于/恰好/超过上限 3 案例全部 PASS，且超限时函数返回成功并只交付上限内字节，确认静默截断；分类审计覆盖 IPv4 权威尾部、IPv6 referral、CIDR ERX/IANA 与批量拒绝访问 4 案例，完整与截断响应均走真实分类 helper，确认截断可导致这四类判定漂移（`out/artifacts/response_limit_audit/20260825-183151`）。静默截断已登记为独立内部 fail-close bugfix 候选；本版网络读取行为与公开 CLI 不变。

English summary
- Release-candidate verification (2026-08-28): the strict `lto-auto` nine-architecture build has no compiler/LTO warnings, all 9/9 recomputed SHA-256 hashes match the remote report, smoke logs contain no warning/error, and Golden plus the three referral origins pass (`out/artifacts/20260828-081629`, 212s). In addition, the dry-run build+sync (including the Step47 preclass preflight) exits 0 (`20260828-061931`), both D6 double-round consistency rounds report `RoundPass=True` (`d6_consistency_double_round/20260828-065758`), the CIDR bundle passes body `4/4` plus matrix `9/9` (`cidr_bundle/...075315`), the Redirect Matrix reports `authMismatchFiles=0 errorFiles=0` (`redirect_matrix_10x6/20260828-075458`), the Step47 prerelease passes 7/7 (`step47_prerelease/20260828-080030`), and the write-core guard reports `pass=True`. This round's binaries carry an intermediate version string (`v3.3.1-9-g23d061d3-dirty`); the final v3.3.2 artifacts are rebuilt by the One-Click Release with a forced version after the changes are committed.
...
- WP-10 response read-limit semantics audit (2026-08-25): confirms `--buffer-size` defaults to 512 KiB and is passed as `max_bytes` to `wc_recv_until_idle` on the main query path, governing both work-buffer capacity and actual network receive bytes; the protocol-safety 10 MiB maximum is not wired into the main receive path, so no control object exists independent of the existing option and a new `--max-bytes` is not added. A native Linux GCC socketpair harness exercises the real `wc_recv_until_idle` and passes all three below/at/over-cap cases, while the over-cap case returns success with only the in-cap bytes delivered, confirming silent truncation; a classification harness covers IPv4 authority tail, IPv6 referral, CIDR ERX/IANA, and batch denied markers and confirms truncation can drift all four judgments (`out/artifacts/response_limit_audit/20260825-183151`). Silent truncation is filed as a separate internal fail-close bugfix candidate; network receive behavior and the public CLI are unchanged in this release.

## 3.3.1

中文摘要 / Chinese summary- 发布候选验证（2026-08-28）：Strict `lto-auto` 九架构构建无编译/LTO 告警，本地实算 9/9 SHA-256 与远程报告一致，冒烟日志无 warning/error，Golden 与三起点 referral PASS（`out/artifacts/20260828-081629`，212s）；同时 DryRun build+sync（含 Step47 preclass preflight）`exit_code=0`（`20260828-061931`）、D6 双轮一致性两轮 `RoundPass=True`（`d6_consistency_double_round/20260828-065758`）、CIDR body `4/4`+matrix `9/9`（`cidr_bundle/...075315`）、Redirect Matrix `authMismatchFiles=0 errorFiles=0`（`redirect_matrix_10x6/20260828-075458`）、Step47 预发布 7/7 pass（`step47_prerelease/20260828-080030`）与写盘核心守卫 `pass=True` 全部通过。该轮产物版本串为中间态（`v3.3.1-9-g23d061d3-dirty`），正式 v3.3.2 产物以 One-Click Release 提交后强制版本重建为准。- 发布与真实演练（2026-08-25）：版本号提升至 3.3.1，同步 RELEASE_NOTES 3.3.1 段、README 标题/发布列表/速览与 `docs/release_bodies/v3.3.1.md`；WP-01 真实 one-click 发布演练 PASS（build+verify → statics 同步/提交 → annotated tag v3.3.1 指向最终静态产物提交 `4357c6a0` → GitHub/Gitee Release 更新，Gitee body 更新 exit=0），全程无故障；WP-01 关闭（done）。
- WP-07 fold token 容量预留优化（2026-08-24）：WP-02 的高密度 fold 基准定位到 `append_token_with_format` 对几乎每个输出字节调用一次 `wc_workbuf_reserve`。现改为每个 token 按格式化输出严格上界一次性预留，扫描、空白折叠、大小写、CR/LF 截断、分隔符、去重与 stdout 均不变。linux x86_64、aarch64 QEMU、win64 各 46/46 冻结 SHA 场景及 ASan/UBSan 压力复核 PASS；三架构 reserve 均从 `1,417,000/1,420,000` 降至 `26,000/29,000`。x86_64 两压力场景 median 分别改善 55.1%/51.3%，aarch64 QEMU 改善 41.0%/53.4%，win64 稳定复跑改善 3.9%/12.6%。最新 Strict `lto-auto` 重建无编译/LTO 告警，九架构 build/hash、Golden、三起点 referral 和双目录 release sync 全 PASS（`out/artifacts/20260824-205103`，311s）；Linux/QEMU、win32、win64 smoke=`18/3/3`，首尾一一对应且零告警，IANA/ARIN/AFRINIC 三起点均收敛至 AFRINIC，artifact 与两个 release 目录均 `9/9` SHA 一致。
- WP-06 `--stats`（2026-08-24）：新增仅批量可用的 stdout 尾部固定 18 字段汇总，覆盖 total/success/error、lookup/rejected/internal 错误分类、固定 RIR 桶及精确 nearest-rank p50/p95。聚合器直接消费结构化查询结果，不解析 stdout，也不复用 `[WORKBUF-STATS]`；每批上限 1,000,000 项，超限/分配失败/SIGINT 不输出部分汇总。与 no-body/fold/filter/pick/chain/meta 合法，与单条或 plain 查询前 fail-fast。真实联网复核修复了 pipeline 渲染清空 `res.body` 后 stats 把成功查询误计为 lookup error 的问题，成功状态现于渲染前冻结；meta/fold 双查询组合均回归为 `success=2 error=0`、ARIN/APNIC 各 1。修复后最终 Strict 九架构 `lto-auto` build/hash、Golden、三起点 referral 与双目录 release sync 全 PASS且无编译/LTO 告警（`out/artifacts/20260824-185823`，316s），artifact、仓库 release 与外部 lzispro release SHA-256 均 `9/9` 一致；可运行 Linux/QEMU、win32、win64 smoke=`18/3/3` 且零告警。最终同步 win64 专项合同 `12/12` PASS（`out/artifacts/stats_contract/20260824-190108`），standalone 新增三项 selftest 均唯一 PASS。
- WP-05B `--pick`（2026-08-24）：新增精确、不区分大小写的 WHOIS 标题字段抽取，首版白名单为 `netname,country,inetnum,inet6num,origin,route,descr`；保持请求顺序并去重，支持 `--pick-mode first|join`、续行 `; `、多值 `|`、缺失空值和逐字段 64 KiB `...` 截断。处理顺序冻结为 title → grep → pick → fold/body，多观测按 pick → chain → meta 排列；与 no-body/fold/filter/batch 合法，与 plain 查询前 fail-fast。专项合同 `12/12` PASS（`out/artifacts/pick_contract/20260824-164050`），最终 standalone 四项 pick selftest 全 PASS。WP-05 最终重建复核 PASS（`out/artifacts/20260824-170256`，351s）：Strict 九架构无编译/LTO 告警，artifact 与两个发布目录 SHA-256 均 `9/9` 一致，Linux/QEMU/native 与 Windows Wine smoke 零告警，Golden 和三起点 referral PASS。
- WP-05A `--print-chain`（2026-08-24）：新增每查询一行的有序逻辑 WHOIS hop 观测 `chain=server1>server2>...`；链路独立存于结果元数据，不从会增删/别名折叠的 `visited[]` 推导，同 hop 的 DNS 候选与重试不重复。未触网路径输出 `chain=unknown`，最多保留 16 hop并以 `>truncated` 标记溢出；与 `--no-body`、`--fold`、`--print-meta`、过滤和批量合法，与 `--plain` 查询前 fail-fast。专项合同 `12/12` PASS（`out/artifacts/print_chain_contract/20260824-155911`）；x86_64/win32/win64 `lto-auto` build/hash/smoke 与三起点 referral PASS（`out/artifacts/20260824-155726`，294s），standalone parser/冲突自测 PASS；最终完整门禁已随 WP-05B 收口。
- WP-03 `--no-body`（2026-08-24）：新增仅抑制最终 WHOIS 正文的可选输出模式，保留查询首行、适用的 `Address Status:` 与权威尾行；网络读取、重试、重定向、权威判定及 title/grep 过滤语义不变。该模式支持单条与批量查询，并对 `--plain`、`--fold` 及 fold 修饰项执行查询前 fail-fast。独立合同 smoke `13/13` PASS；Batch Golden 四策略与 Selftest Golden core+四策略全 PASS；CIDR bundle `4/4 + 9/9`、12x6 authority/error 零失败、Step47 preflight `5/5` PASS。最终同步制品 Strict `lto-auto` 九架构 build/hash、Golden、三起点 referral 全 PASS且无编译/LTO 告警（`out/artifacts/20260824-115609`，248s）；Linux/QEMU、win32、win64 smoke=`18/3/3` 且首尾一一对应、零告警，两个 lzispro 同步目录均与 artifact `9/9` SHA 一致；最终 win64 制品合同 smoke 再次 `13/13` PASS（`out/artifacts/no_body_contract/20260824-120031`）。同时修正 raw selftest golden 在未提供 batch 输入时错误要求私网正文的测试编排。
- WP-04 `--print-meta`（2026-08-24）：新增观测选项，在每条查询记录末尾追加一行 TAB 分隔的 `k=v` 元信息（`query`、`rir`、`status=success|error`、`duration_ms`、`attempts`、`redirects`），首行/正文/尾行与退出码不变；与 `--no-body`、`--fold`、`-g`/`--grep*`、批量组合合法，与 `--plain` 查询前 fail-fast；字段名与语义冻结，新增字段仅允许追加。实现与独立复核修复共享 workbuf 覆盖、普通/折叠失败及 lookup 前短路漏行、前导空白未去除和 attempts 上下文错配；归一化改为无堆分配流式输出，并保持未启用元信息时 fold 失败 stdout 为空的既有行为。最终扩展合同 smoke `18/18` PASS（`out/artifacts/print_meta_contract/20260824-151824`），最终同步 win64 standalone selftest 及 parser/冲突断言 PASS；Strict `lto-auto` 九架构 build/hash、Golden、三起点 referral 全 PASS（`out/artifacts/20260824-151759`，350s），两个同步目录与 artifact 的 SHA 均为 `9/9` 一致。
- WP-02 离线条件输出性能基线与最终制品复核（2026-08-24）：新增 9 份固定离线响应、46 场景 C harness 及 PowerShell/Python 驱动，win64、linux x86_64、linux aarch64 QEMU 均完成 46/46 场景、230 行原始测量和冻结 stdout SHA 一致性校验；压力样本以 ASan 定位并修复 `fold-unique` 在 workbuf 扩容后读取旧 token 指针的 heap-use-after-free。最终 Strict `lto-auto` 默认轮无编译/LTO 告警，九架构 hash、Golden、三起点 referral 与两个同步目录均 `9/9` PASS（`out/artifacts/20260824-094617`，299s）；Linux/QEMU、win32、win64 smoke 为 `18/3/3`，标题/权威尾行一致且零告警。本轮无需追加代码修复。
...
- post-3.3.0 WP-01 发布工具首批收口（2026-08-24）：一键发布现按 build/verify、同步并提交静态产物、创建 tag、发布 release 的顺序执行；目标 tag 不存在时通过进程环境强制远程构建版本为 `v<Version>`，现有 tag 若不指向最终静态产物提交则 fail-close。GitHub/Gitee token 不再拼入或回显于 `bash -lc` 命令文本。dry-run smoke 新增发布顺序、强制版本、token 内联风险以及九架构加 checksum 集合断言，本地无构建路径 PASS（`out/artifacts/oneclick_dryrun_guard/20260824-071904`）。真实 one-click build+sync 与最终 tag 指向演练已于 2026-08-25 完成（v3.3.1：tag 指向最终静态产物提交 `4357c6a0`，产物 `out/artifacts/20260825-101612`，Gitee Release body 更新 exit=0），WP-01 关闭（done）。

English summary
- Release and live rehearsal (2026-08-25): version bumped to 3.3.1, with the RELEASE_NOTES 3.3.1 section, README title/release list/what's-new, and `docs/release_bodies/v3.3.1.md` synchronized; the WP-01 real one-click release rehearsal PASSED (build+verify → statics sync/commit → annotated tag v3.3.1 pointing at the final statics commit `4357c6a0` → GitHub/Gitee Release update, Gitee body update exit 0) with no incidents; WP-01 is closed (done).
- WP-07 fold token capacity reservation optimization (2026-08-24): the WP-02 dense-fold benchmark traced the hotspot to `append_token_with_format` calling `wc_workbuf_reserve` for almost every output byte. It now reserves once per token using a strict upper bound; scanning, whitespace folding, case conversion, CR/LF truncation, separators, uniqueness, and stdout remain unchanged. All 46 frozen-SHA cases pass on linux x86_64, aarch64 under QEMU, and win64, as do the ASan/UBSan stress checks. Reserves fall from `1,417,000/1,420,000` to `26,000/29,000` on all three architectures; median improvements for the two stress cases are 55.1%/51.3% on x86_64, 41.0%/53.4% on aarch64 QEMU, and 3.9%/12.6% on the stable win64 rerun. The latest strict `lto-auto` rebuild has no compile/LTO warnings; all nine builds/hashes, Golden, three referral starts, and both release-directory syncs pass (`out/artifacts/20260824-205103`, 311s). Linux/QEMU, win32, and win64 smoke counts are `18/3/3`, with matching headers/tails and zero alerts; IANA, ARIN, and AFRINIC starts all converge to AFRINIC, and the artifact plus both release directories match `9/9` SHA-256 hashes.
- WP-06 `--stats` (2026-08-24): adds a batch-only fixed 18-field stdout summary after all query records, covering total/success/error, lookup/rejected/internal error classes, fixed RIR buckets, and exact nearest-rank p50/p95 latency. The aggregator consumes structured query results without parsing stdout or reusing `[WORKBUF-STATS]`; each batch is capped at 1,000,000 effective inputs, and overflow, allocation failure, or SIGINT emits no partial summary. It combines with no-body/fold/filter/pick/chain/meta and fails before lookup with a positional single query or plain. Live-network review fixed successful queries being counted as lookup errors after the pipeline cleared `res.body`; lookup success is now frozen before rendering. The meta and fold two-query cases both regress to `success=2 error=0`, with one ARIN and one APNIC result. After the fix, final strict nine-architecture `lto-auto` builds/hashes, Golden, three referral starts, and both release-directory syncs pass with no compile/LTO warnings (`out/artifacts/20260824-185823`, 316s). The artifact, repository release directory, and external lzispro release directory each match all `9/9` SHA-256 hashes; runnable Linux/QEMU, win32, and win64 smoke counts are `18/3/3` with zero alerts. The final synchronized win64 artifact passes the dedicated contract smoke `12/12` (`out/artifacts/stats_contract/20260824-190108`), and each of the three new standalone selftest labels passes exactly once.
- WP-05B `--pick` (2026-08-24): adds exact, case-insensitive WHOIS-header extraction with the initial `netname,country,inetnum,inet6num,origin,route,descr` allowlist. It preserves request order while deduplicating keys and supports `--pick-mode first|join`, `; ` continuations, `|` repeated values, explicit empty values, and per-field 64 KiB truncation ending in `...`. Processing is frozen as title → grep → pick → fold/body, with observations ordered pick → chain → meta. It combines with no-body/fold/filter/batch and fails before lookup with plain. The dedicated contract smoke passes `12/12` (`out/artifacts/pick_contract/20260824-164050`), and all four final standalone pick selftests pass. The final WP-05 rebuild passes (`out/artifacts/20260824-170256`, 351s): strict nine-architecture builds have no compile/LTO warnings, artifact and both release directories match `9/9` SHA-256 hashes, Linux/QEMU/native and Windows Wine smoke have zero alerts, and Golden plus all three referral starts pass.
- WP-05A `--print-chain` (2026-08-24): adds one ordered logical WHOIS-hop observation per query as `chain=server1>server2>...`. The result owns a separate chronological chain instead of deriving it from mutable/alias-folded `visited[]`, and DNS candidates or retries within one hop do not duplicate entries. No-network paths emit `chain=unknown`; up to 16 hops are retained and overflow appends `>truncated`. It combines with `--no-body`, `--fold`, `--print-meta`, filters, and batch mode, while `--plain` fails before lookup. The dedicated contract smoke passes `12/12` (`out/artifacts/print_chain_contract/20260824-155911`); x86_64/win32/win64 `lto-auto` builds, hashes, smoke tests, and three referral starts pass (`out/artifacts/20260824-155726`, 294s), as do standalone parser/conflict selftests; the final full gate closed with WP-05B.
- WP-03 `--no-body` (2026-08-24): adds an opt-in mode that suppresses only the final WHOIS body while retaining the query header, an applicable `Address Status:` line, and the authoritative tail. Network reads, retries, referrals, authority resolution, and title/grep filtering remain unchanged. Single and batch queries are supported; combinations with `--plain`, `--fold`, or fold modifiers fail before lookup. The dedicated contract smoke passes `13/13`; all four Batch Goldens and the core plus four strategy Selftest Goldens pass; the CIDR bundle passes `4/4 + 9/9`, the 12x6 matrix has zero authority mismatches/errors, and Step47 preflight passes `5/5`. The final synchronized-artifact Strict `lto-auto` run passes all nine builds/hashes, Golden, and three referral starts with no compile/LTO warnings (`out/artifacts/20260824-115609`, 248s). Linux/QEMU, win32, and win64 smoke counts are `18/3/3`, with matching query headers and authoritative tails and zero alerts; both lzispro sync directories match the artifact hashes `9/9`. The final win64 artifact passes the dedicated contract smoke again at `13/13` (`out/artifacts/no_body_contract/20260824-120031`). The raw selftest golden orchestration was also corrected so it no longer requires private-query body output when no batch input was supplied.
- WP-04 `--print-meta` (2026-08-24): adds an observation option that appends one TAB-separated `k=v` metadata line per query record (`query`, `rir`, `status=success|error`, `duration_ms`, `attempts`, `redirects`) while headers, body, tail, and exit codes stay unchanged. It combines with `--no-body`, `--fold`, `-g`/`--grep*`, and batch mode, and fails before lookup with `--plain`. Field names and semantics are frozen; new fields may only be appended. Implementation and independent review fixed shared-workbuf corruption, missing metadata on normal/folded failures and pre-lookup exits, leading-whitespace normalization, and attempts-context mismatch; normalization now streams without heap allocation while preserving empty stdout for folded failures when metadata is not enabled. The final expanded contract smoke passes `18/18` (`out/artifacts/print_meta_contract/20260824-151824`), and the final synchronized win64 artifact passes the standalone parser/conflict selftests. The Strict `lto-auto` run passes all nine builds/hashes, Golden, and three referral starts (`out/artifacts/20260824-151759`, 350s), with both sync directories matching the artifact hashes `9/9`.
- WP-02 offline conditional-output performance baseline and final artifact review (2026-08-24): adds nine fixed offline responses, a 46-case C harness, and PowerShell/Python drivers. win64, linux x86_64, and linux aarch64 under QEMU each pass 46/46 cases, produce 230 raw measurements, and agree on all frozen stdout hashes. The stress fixture and ASan exposed and verified a fix for a `fold-unique` heap-use-after-free caused by reading stale token pointers after workbuf growth. The final default Strict `lto-auto` round has no compile/LTO warnings; all nine artifact hashes, Golden, three referral starts, and both sync directories pass `9/9` (`out/artifacts/20260824-094617`, 299s). Linux/QEMU, win32, and win64 smoke counts are `18/3/3`, with matching query headers and authoritative tails and zero alerts. No additional code fix is required.
