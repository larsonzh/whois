...
- 文档与黄金澄清：`docs/USAGE_*`、`docs/OPERATIONS_*` 现标注 `--batch-strategy plan-b` 为占位（当前仅打印 `action=unknown-strategy name=plan-b fallback=health-first`），对应的 batch 黄金预设会自动标记 SKIPPED，等待后端落地后再恢复断言；backoff 章节未改动，仍按既有 `[DNS-BACKOFF]` 标签对齐。
- 远程冒烟（2025-12-11，默认参数）完成一轮：路径 `out/artifacts/20251211-220644/...`，多架构构建 + SHA256 校验 + referral gate 均 PASS，黄金未触发 plan-b 校验（占位状态）。
...
- plan-b 状态告知：当前二进制尚未实现 plan-b；远程批量剧本显式启用 plan-b 时会在日志中输出 `action=unknown-strategy`，脚本自动将该轮黄金标记为 SKIPPED，待策略落地并产出 `[DNS-BATCH] plan-b-*` 标签后再恢复严格校验。
...
English summary
- Docs & golden clarification: `docs/USAGE_*` and `docs/OPERATIONS_*` now call out `--batch-strategy plan-b` as a placeholder (current builds log `[DNS-BATCH] action=unknown-strategy name=plan-b fallback=health-first` once and then behave like health-first). The batch golden preset marks the plan-b leg SKIPPED until the actual scheduler ships; backoff docs remain unchanged and still reference existing `[DNS-BACKOFF]` coverage.
- Remote smoke (2025-12-11, default args) finished cleanly: `out/artifacts/20251211-220644/...` contains multi-arch builds, SHA256 verification, and referral gate outputs; plan-b golden checks were skipped per the placeholder status.
...
- Batch plan-b assertions (prep): `tools/test/golden_check_batch_presets.sh` 与 `tools/test/remote_batch_strategy_suite.ps1` 现预置 plan-b 批量黄金参数（`plan-b-force-start,plan-b-fallback,debug-penalize,start-skip,force-last,force-override` 等），便于后续实现落地后直接接入黄金断言；默认不启用 plan-b 轮次，需显式触发。
