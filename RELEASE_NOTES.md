...
- IPv4/IPv6 启动探测 + 默认偏好调整（2026-01-xx）：进程启动即探测本机 IPv4/IPv6 是否可用，双栈且未显式指定 prefer/only 时默认改为 `--prefer-ipv4-ipv6` + `--dns-family-mode seq-v4-then-v6`；仅单栈时自动强制对应族并忽略冲突偏好（stderr 提示），两族都不可用直接 fatal 退出。`--debug` 下追加 `[NET-PROBE] ipv4=... ipv6=...`。
- fold unique body aliasing 修复（2025-12-31）：fold unique 去重改用 workbuf scratch 视图，消除 body aliasing 崩溃与长行/高续行/CRLF 截断风险；远程冒烟默认 `out/artifacts/20251231-203337` 与 debug+metrics+interleave `out/artifacts/20251231-203656` `[golden] PASS`，批量与自检全矩阵 `batch_{raw,health,plan,planb}/20251231-20{3950,4330,4730,5124}/...`、`batch_{raw,health,plan,planb}/20251231-20{5426,5630,5841,6058}/...` 均 PASS。
- Windows 产物与拨号稳定性（2025-12-29）：默认构建 win32/win64（`tools/remote/remote_build_and_test.sh` 现无需 `-w 1`），wine 冒烟命令改为 `env WINEDEBUG=-all wine64 ./whois-win64.exe ...`；Winsock 路径统一用 `wc_safe_close`/`closesocket`，`wc_dial_43` 映射 WSAEWOULDBLOCK→EINPROGRESS 并在 select 超时填充 WSAETIMEDOUT，新增 `[NET-DEBUG]` 记录 getaddrinfo/重试详情。四轮黄金（含 Windows 冒烟）全 PASS：远程冒烟+黄金 `out/artifacts/20251229-161038`、`-161508`，批量策略与自检矩阵 `batch_{raw,health,plan,planb}/20251229-16*` 全部 PASS。
- CR/CRLF 归一化 + 指标复验（2025-12-27）：单条/批量 stdin 在过滤前统一 CR-only/CRLF→LF，自测覆盖标题/正文；默认黄金 **[golden] PASS**（`out/artifacts/20251227-115517`），debug+metrics+stats 单查询（`--title --debug --retry-metrics --dns-cache-stats`）**PASS**（`out/artifacts/20251227-120336`，`[RETRY-METRICS] attempts=3 successes=2 failures=1`），同参数批量 stdin（`-F testdata/queries.txt`）**PASS**（`out/artifacts/20251227-120810`，`[RETRY-METRICS] attempts=9 successes=8 failures=1`，`[DNS-CACHE-SUM] hits=6 neg_hits=6 misses=42`）。
- 自测标记修复与降噪（2025-12-25）：`[SELFTEST]` 注入标记（force-suspicious/force-private/injection-view-fallback）统一带 `action=` 前缀，进程内仅输出一次，即便未显式执行 `--selftest` 也会落盘；DNS ipv6-only/fallback 自测从 FAIL 降级为 WARN。VS Code 自测任务默认移除 `--selftest`，actions/expectations 默认值包含 injection-view-fallback。四轮自检黄金（raw/health-first/plan-a/plan-b，`--selftest-force-suspicious 8.8.8.8`）均 **[golden-selftest] PASS**（日志 `out/artifacts/batch_{raw,health,plan,planb}/20251225-054{112,233,355,516}/...`）。
- 前端适配器封装（2025-12-23）：入口新增 `wc_client_frontend_run` 包装 bootstrap+pipeline，whois_client 主函数改用该适配器，行为不变；远程冒烟（默认、`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）与批量 raw/health-first/plan-a/plan-b、自检全矩阵均 **无告警 + [golden|golden-selftest] PASS**（日志 `out/artifacts/20251223-035715`、`035954`、`batch_* /20251223-04{0140,0404,0626,0843}/...`、`batch_* /20251223-04{1113,1229,1344,1459}/...`）。
- 入口用法错误收口（2025-12-23 04:36 批次）：`main` 改为直接调用 `wc_client_exit_usage_error`，保持 parse → frontend 单链，去除 runner/flow 头依赖。远程冒烟（默认、`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）与批量 raw/health-first/plan-a/plan-b、自检全矩阵再次 **无告警 + [golden|golden-selftest] PASS**（日志 `out/artifacts/20251223-043625`、`043833`、`batch_* /20251223-04{4838,5112,5328,5548}/...`、`batch_* /20251223-04{5745,5903,0020,0135}/...`）。
- runtime 视图内收 + 批量调试修正（2025-12-23）：runtime cfg/dns 视图改为 runtime.c 私有快照（不再对外导出 getter），batch health-first/plan-a 调试日志补齐 ctx 传递，编译警告清零；默认/调试双轮冒烟及批量/raw+health-first+plan-a+plan-b、自检全矩阵均 **无告警 + [golden|golden-selftest] PASS**（见 `out/artifacts/20251223-014414`、`014708`、`batch_* /20251223-01{4935,5155,5414,5631}/...`、`batch_* /20251223-01{5831,5945,6100,6223}/...`）。
- 追加黄金复跑（2025-12-23 03:04–03:23）：入口头文件瘦身后，默认/调试冒烟与批量 raw/health-first/plan-a/plan-b、自检四策略再次 **无告警 + [golden|golden-selftest] PASS**（日志 `out/artifacts/20251223-030442`、`030722`、`batch_* /20251223-03{0940,1211,1434,1659}/...`、`batch_* /20251223-03{1914,2029,2150,2303}/...`）。
- DNS family-mode 黄金覆盖加强：2025-12-21 新增两轮冒烟（默认 `out/artifacts/20251221-130758/...`，`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first` `out/artifacts/20251221-131058/...`）均无告警 `[golden] PASS`；批量 raw/health-first/plan-a/plan-b 与自检套件全矩阵 `[golden|golden-selftest] PASS`，批量已在 raw 轮断言 `mode=interleave-v4-first start=ipv4`、health-first 轮断言 `mode=seq-v4-then-v6 start=ipv4`（日志 `batch_raw/20251221-125729`、`batch_health/20251221-125954`、`batch_plan/20251221-130223`、`batch_planb/20251221-130452`，自检 `batch_* /20251221-1313**/` 与 `1316**/1317**/`）。
- referral 守卫容错：`tools/test/referral_143128_check.sh` 允许 AfriNIC 路径由 ARIN 直接权威返回时接受 `whois.arin.net` 或 `whois.afrinic.net` 尾行，避免 Plan-A 批次误报（头/Additional 仍按原链路检查）。
- 信号清理优化 + 四轮黄金（2025-12-21 00:12/00:14/01:50+）：默认/`--debug --retry-metrics --dns-cache-stats` 冒烟均无告警 `[golden] PASS`（`out/artifacts/20251221-001203/...`、`-001409/...`、`-015000/...`、`-015221/...`）；批量 raw/health-first/plan-a/plan-b 及 `--selftest-force-suspicious 8.8.8.8` 全矩阵 `[golden|golden-selftest] PASS`（`batch_{raw,health,plan,planb}/20251221-0154**/` 与 `20251221-0204**/`）。
- 运行期配置快照 + 缓存计数采样开关（2025-12-18 13:22+）：`wc_runtime` 维护轻量配置快照驱动 net/context 初始化（push/pop 同步刷新，快照现为 runtime.c 私有，未对外暴露 getter），新增 `wc_runtime_set_cache_counter_sampling()`（默认 0）+ housekeeping 钩子，按需输出与 debug 同款 cache counters 而不影响 stdout。四轮远程冒烟全部无告警 + `[golden|golden-selftest] PASS`：默认 `out/artifacts/20251218-132235/...`，`--debug --retry-metrics --dns-cache-stats` `out/artifacts/20251218-132516/...`，批量 raw/health-first/plan-a/plan-b `out/artifacts/batch_{raw,health,plan,planb}/20251218-13{2717,2939,3209,3429}/...`，自检（`--selftest-force-suspicious 8.8.8.8`）`out/artifacts/batch_{raw,health,plan,planb}/20251218-13{3714,3839,4002,4115}/...`。
- 缓存计数封装 + 四轮黄金（2025-12-18 10:29+）：`wc_cache` 的 DNS/负缓存计数改为模块内 `static` 并在 init/cleanup 归零，行为与标签不变。默认/`--debug --retry-metrics --dns-cache-stats` 冒烟与批量 raw/health-first/plan-a/plan-b、自检（`--selftest-force-suspicious 8.8.8.8`）全矩阵 **无告警 + [golden|golden-selftest] PASS**，日志 `out/artifacts/20251218-102901/...`、`-103101/...`、批量 `batch_{raw,health,plan,planb}/20251218-103{257,519,739,956}/...` 与自检 `batch_{raw,health,plan,planb}/20251218-104{148,302,414,527}/...`。
- 运行期配置显式注入 + 四轮黄金（2025-12-18）：`signal.c` / `client_flow.c` 配置仅由调用方注入，不再回退 `wc_runtime_config()`；lookup 自测改用公开的 `wc_selftest_config_snapshot()`，消除隐式声明与 runtime 读。默认与 `--debug --retry-metrics --dns-cache-stats` 冒烟各一轮，以及批量 raw/health-first/plan-a/plan-b、自检（`--selftest-force-suspicious 8.8.8.8`）全矩阵均无告警 + `[golden|golden-selftest] PASS`，日志位于 `out/artifacts/20251218-094015/...`、`-094311/...`、`batch_{raw,health,plan,planb}/20251218-09{4505,4735,4953,5216}/...` 与自检集 `batch_{raw,health,plan,planb}/20251218-09{5455,5611,5721,5842}/...`。
...
- IPv4/IPv6 startup probe + default flip (2026-01-xx): on startup we probe local IPv4/IPv6 availability once; if both work and no prefer/only flags were set, the default becomes `--prefer-ipv4-ipv6` + `--dns-family-mode seq-v4-then-v6`. Single-stack hosts auto-force the matching family and ignore conflicting preferences with a notice; zero families is fatal. `[NET-PROBE] ipv4=... ipv6=...` appears under `--debug`.
- Fold-unique body aliasing fix (2025-12-31): fold unique now uses a workbuf scratch view to avoid body aliasing crashes and long-line/CRLF/continuation truncation; remote smokes `out/artifacts/20251231-203337` (default) and `out/artifacts/20251231-203656` (`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`) are `[golden] PASS`, and batch/selftest matrices `batch_{raw,health,plan,planb}/20251231-20{3950,4330,4730,5124}/...` and `batch_{raw,health,plan,planb}/20251231-20{5426,5630,5841,6058}/...` also PASS.
- Windows artifacts & Winsock stability (2025-12-29): win32/win64 now build by default (no `-w 1` needed in `tools/remote/remote_build_and_test.sh`); wine smoke uses `env WINEDEBUG=-all wine64 ./whois-win64.exe ...`. Winsock paths now use `wc_safe_close`/`closesocket`, `wc_dial_43` maps WSAEWOULDBLOCK→EINPROGRESS and stamps WSAETIMEDOUT on select timeouts, and `[NET-DEBUG]` logs getaddrinfo and per-attempt details. Four fresh golden runs (incl. Windows smokes) all PASS: remote + golden at `out/artifacts/20251229-161038`, `-161508`; batch strategies and selftests under `batch_{raw,health,plan,planb}/20251229-16*` all PASS.
- CR/CRLF normalization + metrics reruns (2025-12-27): single/batch stdin normalize CR-only/CRLF to LF before filters; selftests cover header/body. Baseline golden **[golden] PASS** (`out/artifacts/20251227-115517`); debug+metrics+stats single run `--title --debug --retry-metrics --dns-cache-stats` **PASS** (`out/artifacts/20251227-120336`, `[RETRY-METRICS] attempts=3 successes=2 failures=1`); same flags with batch stdin `-F testdata/queries.txt` **PASS** (`out/artifacts/20251227-120810`, `[RETRY-METRICS] attempts=9 successes=8 failures=1`, `[DNS-CACHE-SUM] hits=6 neg_hits=6 misses=42`).
- Selftest marker fix & noise reduction (2025-12-25): `[SELFTEST]` hooks (force-suspicious/force-private/injection-view-fallback) now carry an `action=` prefix and emit once per process even when the suite is not invoked explicitly; DNS ipv6-only/fallback selftests are downgraded from FAIL to WARN. VS Code selftest task defaults drop the explicit `--selftest` flag while keeping actions/expectations (now including injection-view-fallback). Four selftest golden runs (raw/health-first/plan-a/plan-b, `--selftest-force-suspicious 8.8.8.8`) all **[golden-selftest] PASS** with logs `out/artifacts/batch_{raw,health,plan,planb}/20251225-054{112,233,355,516}/...`.
- Frontend adapter wrap (2025-12-23): added `wc_client_frontend_run` (bootstrap + pipeline facade) and switched `whois_client` main to call it; behavior unchanged. Default and debug smokes plus batch raw/health-first/plan-a/plan-b and selftest matrices all **clean + [golden|golden-selftest] PASS** (logs `out/artifacts/20251223-035715`, `035954`, `batch_* /20251223-04{0140,0404,0626,0843}/...`, `batch_* /20251223-04{1113,1229,1344,1459}/...`).
- Usage-error exit tidy (2025-12-23 04:36 batch): `main` now calls `wc_client_exit_usage_error` directly and drops runner/flow includes; keeps parse → frontend single chain. Default and debug smokes plus batch raw/health-first/plan-a/plan-b and selftest matrices **clean + [golden|golden-selftest] PASS** (logs `out/artifacts/20251223-043625`, `043833`, `batch_* /20251223-04{4838,5112,5328,5548}/...`, `batch_* /20251223-04{5745,5903,0020,0135}/...`).
- Runtime views privatized + batch debug fix (2025-12-23): runtime cfg/dns snapshots are now private to runtime.c (no exported getters); batch health-first/plan-a debug logging passes ctx explicitly, clearing the build warnings. Default and debug smokes plus batch raw/health-first/plan-a/plan-b and selftest matrices all **clean + [golden|golden-selftest] PASS** (`out/artifacts/20251223-014414`, `014708`, `batch_* /20251223-01{4935,5155,5414,5631}/...`, `batch_* /20251223-01{5831,5945,6100,6223}/...`).
- DNS family mode option: added `--dns-family-mode interleave-v4-first|interleave-v6-first|seq-v4-then-v6|seq-v6-then-v4` (lower priority than `--ipv4-only/--ipv6-only`, hop-aware `--prefer-ipv4-ipv6/--prefer-ipv6-ipv4`, and `--prefer-ipv4/--prefer-ipv6`); `[DNS-CAND] mode=<...> start=<ipv4|ipv6>` exposes the effective start family under debug. Full remote matrix on 2025-12-21 passed clean: baseline `out/artifacts/20251221-100318/...`; `--debug --retry-metrics --dns-cache-stats` `out/artifacts/20251221-100617/...`; batch raw/health-first/plan-a/plan-b `out/artifacts/batch_{raw,health,plan,planb}/20251221-100814/` `101032/` `101248/` `101508/`; selftest matrix (`--selftest-force-suspicious 8.8.8.8`) `out/artifacts/batch_{raw,health,plan,planb}/20251221-101848/` `102015/` `102132/` `102253/`.
- DNS family-mode coverage tightened: two fresh smokes on 2025-12-21 (baseline `out/artifacts/20251221-130758/...`; `--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first` `out/artifacts/20251221-131058/...`) both **clean + [golden] PASS**. Batch raw/health-first/plan-a/plan-b and the selftest suite all **[golden|golden-selftest] PASS** with family-mode assertions baked in: raw enforces `mode=interleave-v4-first start=ipv4`, health-first enforces `mode=seq-v4-then-v6 start=ipv4` (logs `batch_raw/20251221-125729`, `batch_health/20251221-125954`, `batch_plan/20251221-130223`, `batch_planb/20251221-130452`; selftests `batch_* /20251221-1313**/` and `1316**/1317**/`).
- Referral guard tolerance: `tools/test/referral_143128_check.sh` now accepts `whois.arin.net` or `whois.afrinic.net` as the authoritative tail for the 143.128.0.0 chain (header/Additional still validated), preventing false negatives when ARIN responds authoritatively in Plan-A runs.
- Signal cleanup + full golden matrix (2025-12-21 00:12/00:14/01:50+): baseline and `--debug --retry-metrics --dns-cache-stats` smokes all **clean + [golden] PASS** (`out/artifacts/20251221-001203/...`, `-001409/...`, `-015000/...`, `-015221/...`); batch raw/health-first/plan-a/plan-b and selftests (`--selftest-force-suspicious 8.8.8.8`) all **[golden|golden-selftest] PASS** under `out/artifacts/batch_{raw,health,plan,planb}/20251221-0154**/` and `20251221-0204**/`.
- Runtime config snapshot + cache counter sampling toggle (2025-12-18 13:22+): `wc_runtime` keeps lightweight snapshots to drive net/context init (push/pop refresh the snapshot; snapshots are now private to runtime.c with no exported getter). Added `wc_runtime_set_cache_counter_sampling()` (default off) plus a housekeeping hook so cache counters can be emitted on demand without `--debug`; stdout contract unchanged. Four-way remote runs all **clean + [golden|golden-selftest] PASS**: baseline `out/artifacts/20251218-132235/...`; `--debug --retry-metrics --dns-cache-stats` `out/artifacts/20251218-132516/...`; batch raw/health-first/plan-a/plan-b `out/artifacts/batch_{raw,health,plan,planb}/20251218-13{2717,2939,3209,3429}/...`; selftests (`--selftest-force-suspicious 8.8.8.8`) `out/artifacts/batch_{raw,health,plan,planb}/20251218-13{3714,3839,4002,4115}/...`.
- Cache counter encapsulation + full golden (2025-12-18 10:29+): `wc_cache` DNS/negative counters are now module-static and reset in init/cleanup; behavior and telemetry unchanged. Baseline and `--debug --retry-metrics --dns-cache-stats` smokes plus batch raw/health-first/plan-a/plan-b and the selftest matrix (`--selftest-force-suspicious 8.8.8.8`) all finished **clean + [golden|golden-selftest] PASS**. Logs: `out/artifacts/20251218-102901/...`, `-103101/...`, batch `batch_{raw,health,plan,planb}/20251218-103{257,519,739,956}/...`, selftests `batch_{raw,health,plan,planb}/20251218-104{148,302,414,527}/...`.
- Explicit config injection + full golden (2025-12-18): `signal.c` and `client_flow.c` now rely solely on caller-injected `Config` without `wc_runtime_config()` fallback; lookup selftests use the exported `wc_selftest_config_snapshot()` instead of implicit declarations/runtime reads. Both baseline and `--debug --retry-metrics --dns-cache-stats` smokes, plus the batch raw/health-first/plan-a/plan-b and the selftest matrix (`--selftest-force-suspicious 8.8.8.8`), finished clean with `[golden|golden-selftest] PASS`. Logs: `out/artifacts/20251218-094015/...`, `-094311/...`, `batch_{raw,health,plan,planb}/20251218-09{4505,4735,4953,5216}/...`, and selftests `batch_{raw,health,plan,planb}/20251218-09{5455,5611,5721,5842}/...`.
