    ...
    while ($true) {
        try {
            ...
            if (($sessionStatus -in @('FAIL', 'BLOCKED')) -and -not $running) {
                ...
                $eventTicketPolicySuffix = ' If eligible, run business_resume immediately. Execute every ticket step without omission, then execute atomic_closeout_command exactly once and claim closure only from its successful machine facts. The 3-minute closeout window is a target, not an agent-enforced transaction timeout. When recovery_transaction_command is running, wait for its natural exit and never kill, Stop-Process, terminate its terminal, or cancel it merely because it exceeds 3 minutes or 240 seconds; 240 seconds is only the script internal stage-start verification budget. Then wait silently for the next ticket delivered by guard/trigger/dispatch. Never run scheduled, persistent, or long-running cross-round monitoring commands.'
