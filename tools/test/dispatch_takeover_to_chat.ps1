[System.Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingInvokeExpression', '', Justification = 'route_guard_command is generated by the internal takeover brief workflow and must preserve full command-line quoting when probed live.')]
param(
...
'@
$runningStatusShortMessageZh = '[SHORT-CARD] 票据 {0}（event={1}），先读 {2}。1）用一段话先报根因+修复路径；若本票是 healthy 的 running-status-report，根因应写“无活动故障/常规定时状态票”，修复路径应写“continue_watch only”。2）执行已给出的 business_command 作为健康检查，然后执行 continue_watch_command；除非出现独立事故票据，否则不得从该状态票发起 A/B 阶段重启。3）处置后回到票据驱动只读监控（沿用定时状态票节奏），直到“stop monitoring”，不要再询问是否继续轮询/监控。4）强制回执：本票当轮动作完成后必须回传 handled_at（YYYY-MM-DD HH:mm:ss）；running-status-report 在 business/continue_watch 后必须立即回传 handled_at；session_closed_at 仅在 stop monitoring 或 A/B 终态时回传，且最终收尾消息必须显式写出会话结束日期时间；若返回 ticket_closure_check_command / event_dedup_health_check_command / final_status_closeout_command（含 final_status_closeout_apply_ack_command）则按 next_command_order 继续执行。不得仅凭旧 exit 证据推断需要重启 B；不得手工创建 chat_heartbeat/handled 临时回执文件；不得在未获同意时创建非 tmp 新脚本。回传 chat_heartbeat：SESSION/A/B、run_dir、main_round、supervisor/companion/guard 最新心跳、B exit digest。状态：{3}。'
