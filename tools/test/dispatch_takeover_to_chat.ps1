...
$codeFixRecoveryMessageEn = 'Please take over ticket {0} (event={1}) in CODE-FIX dedicated flow: read {3} first, run route_guard_command, and execute only route.allowed_actions. Focus on source/task-definition mismatch or compile/verify failures; when the fix belongs to self-heal-generated output, modify the matching task-definition round under testdata (for example, B D4) instead of directly editing business source code. If the failure happened in V1-V4 rounds, prefer appending the incremental patch after the existing D4 definition instead of rewriting already-validated D1-D4 rounds. Do not mix with script-only remediation. Complete bounded fix -> verify -> resume chain, then return handled_at with exact fix scope and validation evidence.'
