...
修复 code-step 故障后，先运行 SyntaxOnly；可定位时检查当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`，再对当前故障轮运行不带 -OperationIndex 的递进严格检查。checker 与 code-step 都只在当前 op 通过后推进内存副本，首错即停，不检查后续 op 或后续轮。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。单实例冲突、正则或 worker 超时均硬阻断重启。只有当前故障轮通过才允许重启或 resume；重启时只能按本票目标阶段启动正确的主进程（A 问题重启 A，B 问题重启 B，禁止猜阶段、串阶段），然后继续监控，最后回传 chat_heartbeat 与修复结论。'
...
修复 code-step 故障后，先运行 SyntaxOnly；可定位时用 -OperationIndex 快检当前 op，再对当前故障轮运行不带 -OperationIndex 的递进严格检查。checker 与 code-step 首错即停，不检查后续 op 或轮；单实例冲突、正则或 worker 超时均硬阻断。只有当前故障轮通过才允许重启或 resume。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
