...
$taskDefinitionPromotionGate = 'before recovery/restart/resume require same-ticket manifest.state=promoted; validated_candidate_sha256=promoted_sha256=official SHA-256; successful promotion-receipt ticket/hash match; and current failing round strict check PASS; candidate edited or Inspect/focused-check PASS is not completion; quarantined/validation_failed/promotion_failed/abandoned/missing receipt/hash mismatch must fail-close'
if ($briefSettings.Contains('task_definition_promotion_gate')) {
    $briefTaskDefinitionPromotionGate = Convert-ToSingleLineText -Text ([string]$briefSettings.task_definition_promotion_gate)
    if (-not [string]::IsNullOrWhiteSpace($briefTaskDefinitionPromotionGate)) {
        $taskDefinitionPromotionGate = $briefTaskDefinitionPromotionGate
    }
}
$briefMainRound = ''
...
$briefTaskDefinitionSchemaVersion = if ($briefSettings.Contains('task_definition_schema_version')) { Convert-ToSingleLineText -Text ([string]$briefSettings.task_definition_schema_version) } else { '1' }
$briefTargetSetSha256 = if ($briefSettings.Contains('target_set_sha256')) { Convert-ToSingleLineText -Text ([string]$briefSettings.target_set_sha256) } else { '' }
$briefFailureOperation = if ($briefSettings.Contains('task_definition_failure_operation')) { Convert-ToSingleLineText -Text ([string]$briefSettings.task_definition_failure_operation) } else { '' }
$briefFailureTargetId = if ($briefSettings.Contains('failure_target_id')) { Convert-ToSingleLineText -Text ([string]$briefSettings.failure_target_id) } else { '' }
$briefFailureTargetPath = if ($briefSettings.Contains('failure_target_path')) { Convert-ToSingleLineText -Text ([string]$briefSettings.failure_target_path) } else { '' }
$briefFailureTargetKind = if ($briefSettings.Contains('failure_target_kind')) { Convert-ToSingleLineText -Text ([string]$briefSettings.failure_target_kind) } else { '' }
$briefFailureTargetLifecycle = if ($briefSettings.Contains('failure_target_lifecycle')) { Convert-ToSingleLineText -Text ([string]$briefSettings.failure_target_lifecycle) } else { '' }
$modeRestorePolicy = ''
...
Fix placement rules:
    [D1-D4 task-static phase failed: task-definition-mismatch]
        Only forward D rounds are eligible for their own repairs: D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4. Never edit an earlier D round. A defect belonging to one round must be fixed only in that same round; never edit another round to repair or bypass it. The source has NOT yet been changed by the failing round. In that round, ops before the failing op are read-only; modify, delete, insert, or append only from the failing op onward.
...
$codeFixRecoveryMessageEn = 'Please take over ticket {0} (event={1}) in CODE-FIX dedicated flow: read {3} first, run route_guard_command, and execute only route.allowed_actions. Focus on source/task-definition mismatch or compile/verify failures; structured validation failures from preflight/check/golden/selftest/matrix/verify/smoke/preclass flows inherit the child result and stay in code-fix when no environment marker is present, even if a PowerShell wrapper stack frame is present. Diagnose from the first structured child failure and first compiler/test error, not the outer wrapper summary. When the fix belongs to self-heal-generated output, keep the matching official task-definition file under testdata read-only: run task_definition_repair_transaction.ps1 -Mode Prepare, read operation-preview.* and apply-patch-context.txt, modify only candidate.json with VS Code `apply_patch`, preferably run read-only -Mode Inspect after editing to refresh the hash-bound preview, then run Validate and Promote in order; do not directly edit business source or the official task definition. Resolve zero/multiple matches, a still-matchable post-replacement pattern, and double-escape warnings before validation. Inspect changes no candidate, official file, or source and does not replace checker gates; Validate reports preview_stale=true|false as diagnostic evidence. Never use inline Python/PowerShell, redirection, generic string replacement, or a formatter. Validate in order: SyntaxOnly load check, focused failing-op check when available, then progressive strict check for the current failing D round. After those checks pass, do not stop at local validation: run pre_restart_launch_ready_command when present, then execute recovery_transaction_command exactly once when present; otherwise execute the single allowed atomic closeout path. If a true script fault is discovered while handling this code-fix ticket, stop the code-fix flow and reclassify it through the script policy: enter SCRIPT-FIX only when LOCAL_GUARD_SCRIPT_SELF_HEAL_ENABLED is explicitly true; otherwise enter SCRIPT-DIAGNOSE-ONLY and do not edit files or control processes.

Fix placement rules:
    [D1-D4 task-static phase failed: task-definition-mismatch]
        Only forward D rounds are eligible for their own repairs: D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4. Never edit an earlier D round. A defect belonging to one round must be fixed only in that same round; never edit another round to repair or bypass it. Source NOT yet changed by the failing round. In that round, ops before the failing op are read-only; modify/delete/insert/append only from the failing op onward.
  [D1-D4 code-step phase PASSED, but compile/verify phase failed]
...
修复位置规则：
    [D1-D4 task-static 阶段失败：task-definition-mismatch]
        前向范围内的 D 轮只能分别修复各轮自身问题：D1 -> D1-D4，D2 -> D2-D4，D3 -> D3-D4，D4 -> D4；禁止修改前置 D 轮。某轮问题只能在该轮内修复，禁止修改其他轮来修复或绕过。源码尚未被当前故障轮修改；该轮故障 op 之前的 op 为只读，仅可从故障 op 起修改、删除、插入或追加。
...
本流程只授权处理 task-static 故障，以及经分类确认为代码故障的编译/验证故障；任何 code-step 故障均属于 noncode，绝不允许修改源码或任务定义。修复 task-static 故障后，先运行 SyntaxOnly；可定位时检查当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`，再对当前故障轮运行不带 -OperationIndex 的递进严格检查。独立 task-static checker 默认只检查当前轮；使用 -ChainRounds 时在同一内存副本上继续检查到 D4。两种模式都只在当前 op 通过后推进，遇到首个失败 op 或轮立即停止；operations、replay 与 postApplyAssertions 全部通过后生成哈希绑定的有效源码产物。code-step 只读取、验证、原子写入并写后验证该产物。可在同一修复工单内按诊断反复调用 checker，这些本地检查不消耗相同指纹主进程重启预算。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。单实例冲突、正则或 worker 超时均硬阻断重启。只有 brief 选定范围全部通过才允许重启或 resume；重启时只能按本票目标阶段启动正确的主进程（A 问题重启 A，B 问题重启 B，禁止猜阶段、串阶段），然后继续监控，最后回传 chat_heartbeat 与修复结论。'
...
$codeFixRecoveryMessageZh = '请接管票据 {0}（event={1}），进入“代码修复专用流程”：先阅读 {3}，执行 route_guard_command，并严格按 route.allowed_actions 执行。仅处理源码/任务定义不匹配、编译或校验失败，不与脚本修复流程混用。诊断必须下钻到第一个结构化子失败和编译器/测试首错，不得用外层 wrapper 摘要覆盖子流程证据。如果这是自愈生成物修复，保持 testdata 下对应阶段正式任务定义只读：先执行 task_definition_repair_transaction.ps1 -Mode Prepare，读取 operation-preview.* 与 apply-patch-context.txt，只用 VS Code `apply_patch` 修改 candidate.json；修改后推荐执行只读 -Mode Inspect 刷新哈希绑定预览，先排除零/多匹配、替换后仍匹配与双重转义风险，再按顺序执行 Validate 与 Promote。Inspect 不修改候选、正式文件或源码，也不替代 checker 门禁；Validate 输出 preview_stale=true|false 作为诊断证据。不得直接修改正式任务定义或业务源码。禁止终端内联 Python/PowerShell、重定向、通用字符串替换或格式化器代改。验证顺序固定为 SyntaxOnly 装载检查、故障目标 op 快检（可定位时）、当前故障 D 轮递进严格检查。上述本地验证通过后不得停下：brief 提供 pre_restart_launch_ready_command 时先执行它；brief 提供 recovery_transaction_command 时必须只执行一次；否则只执行唯一允许的 atomic closeout 路径。若处理本代码修复票时发现脚本故障，必须停止代码修复流程并按脚本策略重新分类：仅当 LOCAL_GUARD_SCRIPT_SELF_HEAL_ENABLED 显式为 true 时进入“脚本自愈专用流程”，否则进入“脚本故障排查专用流程”，不得修改文件或控制进程。
...
本流程只授权处理 task-static 故障，以及经分类确认为代码故障的编译/验证故障；任何 code-step 故障均属于 noncode，绝不允许修改源码或任务定义。修复 task-static 故障后，先运行 SyntaxOnly；可定位时用 -OperationIndex 快检当前 op，再对当前故障轮运行不带 -OperationIndex 的递进严格检查。独立 task-static checker 负责 operations、replay 与 postApplyAssertions，首错即停；跨轮模式由 -ChainRounds 在同一内存源码上推进到 D4。完整通过后生成哈希绑定产物。code-step 只读取、验证、原子写入并写后验证该产物。可在同一修复工单内按诊断反复调用 checker，这些本地检查不消耗相同指纹主进程重启预算。单实例冲突、正则或 worker 超时均硬阻断。只有 brief 选定范围全部通过才允许重启或 resume。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
...
$crossRoundRepairStatusEn = if ($taskStaticCrossRoundRepairEnabled) {
    '[Cross-round repair enabled] Keep one candidate. Every checker run starts at the ticket failing round with -ChainRounds, replays converged rounds on one in-memory source, and stops at the first unresolved round. Fix only that round; never skip it or edit another round as a workaround. Do not Promote between rounds. Run Prepare and Validate with -ValidateThroughRound D4 -ChainRounds, then Promote exactly once after the complete sequence passes.'
}
else {
    '[Cross-round repair disabled] Use the failing-round source baseline and check, repair, Validate, and Promote only the failing D round; later rounds remain runtime-gated. A round defect may only change operations in that same round.'
}
$crossRoundRepairStatusZh = if ($taskStaticCrossRoundRepairEnabled) {
    '[跨轮次修复已开启] 保持一个 candidate；每次 checker 都从工单故障轮带 -ChainRounds 启动，在同一内存源码上重放已收敛轮并停在首个未收敛轮。只修该轮，禁止跳过或修改别轮绕过。轮次之间禁止 Promote。Prepare 与 Validate 均使用 -ValidateThroughRound D4 -ChainRounds，全部通过后只执行一次 Promote。'
}
else {
    '[跨轮次修复已关闭] 使用故障轮源码基线，只检查、修复、Validate 和 Promote 当前故障 D 轮，不得预演后续轮；后续轮仍由运行时门禁处理。某轮问题只能修改该轮 operations。'
}
$crossRoundTransactionSuffixEn = if ($taskStaticCrossRoundRepairEnabled) { "`n`n[Cross-round transaction execution] Use the complete task_definition_*_command values from the brief; do not reconstruct parameters. Prepare one candidate with -ValidateThroughRound D4 -ChainRounds. After each edit, run task_definition_checker_command from the original failing round; stop at its first unresolved round and edit only that round. Never run a later round against the original failing-round source alone and never Promote between rounds. Run the full Validate command only after the chain reaches D4, require exact failing-round-through-D4 validated_rounds coverage, then run Promote exactly once. Promote itself rejects incomplete coverage." } else { '' }
$crossRoundTransactionSuffixZh = if ($taskStaticCrossRoundRepairEnabled) { "`n`n[跨轮事务执行] 直接使用 brief 中完整的 task_definition_*_command，不得自行重拼参数。使用 -ValidateThroughRound D4 -ChainRounds Prepare 一个 candidate；每次编辑后从原故障轮执行 task_definition_checker_command，停在首个未收敛轮且只修改该轮。禁止以后续轮单独使用原故障轮源码基线，轮次之间禁止 Promote。链到 D4 全部收敛后才执行完整 Validate 命令，validated_rounds 必须精确覆盖故障轮到 D4，再且仅再执行一次 Promote；Promote 自身会拒绝覆盖不完整。" } else { '' }
$taskDefinitionSafetySuffixEn = "`n`n" + '[Task-definition safety] Change task-definition JSON semantics only with the VS Code `apply_patch` editing tool. Diagnose pattern/replacement across three distinct layers: JSON source, the PowerShell string decoded by standard ConvertFrom-Json, and the pattern consumed by .NET Regex. For example, valid JSON "\\)" decodes to regex \), which matches a literal ). pattern_unmatched=0 means JSON loading and regex compilation already succeeded but the pattern matched the current sequential in-memory text zero times; it is not a JSON decoder failure. Use operation-preview.txt decoded views, source matches, and the first checker error; do not modify checker JSON handling for a zero match. After editing a task-static fault, run SyntaxOnly, optionally a focused -OperationIndex check, then follow task_static_cross_round_repair_enabled from the brief: false checks only the failing D round; true uses -ChainRounds to check the failing round and each later D round through D4 in one sequential in-memory pass. The independent task-static checker defaults to one round; ChainRounds advances the same in-memory text across rounds. Both modes advance only after each op passes, stop at the first failure, and validate operations, replay, and postApplyAssertions. Code-step only reads, validates, atomically writes, and validates the current round artifact; every code-step failure is noncode. Rerun checker as needed within one repair ticket; local checker calls do not consume the identical-fingerprint main-process relaunch budget. Keep qualityPolicy.operationSafetyPolicy=enforce. Use minimal type=noop only for a design-time empty round; reject pattern-equals-replacement self-replacement and keep runtime absorbed rounds as regex-patch with absorbed-by-prior-round/idempotent-replay evidence. Every op owns a replacement-produced marker and must converge. Update same-round postApplyAssertions only when operation results change. Preserve helper definitions, prototypes before first callers, and real call sites. Single-instance conflict and regex/worker timeout are hard failures.'
$taskDefinitionSafetySuffixZh = "`n`n" + '[任务定义安全] 任务定义 JSON 的语义修改只允许使用 VS Code `apply_patch` 编辑工具。诊断 pattern/replacement 时必须区分三层：JSON 源码、标准 ConvertFrom-Json 解码后的 PowerShell 字符串、.NET Regex 接收的 pattern。例如合法 JSON "\\)" 解码为正则 \)，用于匹配字面量 )。pattern_unmatched=0 表示 JSON 已加载且正则已编译，但对当前顺序内存文本零命中；这不是 JSON 解码失败。应读取 operation-preview.txt 解码视图、源码匹配和 checker 首错，不得因零命中修改 checker 的 JSON 处理。修复 task-static 故障后先运行 SyntaxOnly，可定位时运行 -OperationIndex 快检，再遵循 brief 中的 task_static_cross_round_repair_enabled：false 时只检查故障 D 轮，true 时使用 -ChainRounds 在一次顺序内存迭代中检查故障轮到 D4。独立 task-static checker 默认每次只验证一轮；ChainRounds 在轮次间继续推进同一内存文本。两种模式都只在当前 op 通过后推进，首错即停，并验证 operations、replay 与 postApplyAssertions。code-step 只读取、验证、原子写入并写后验证当前轮绑定产物；任何 code-step 故障均属于 noncode。同一修复工单内可按需反复调用 checker，本地检查不消耗相同指纹主进程重启预算。保持 qualityPolicy.operationSafetyPolicy=enforce。最小 type=noop 仅用于设计时空轮，禁止 pattern 与 replacement 相同的自替换；运行时吸收必须保持 regex-patch，并以 absorbed-by-prior-round/idempotent-replay 证明。每个 op 使用 replacement 自产 marker 并保证收敛；仅当 operation 结果变化时同步更新同轮 postApplyAssertions。保留 helper definition、首次 caller 前 prototype 与真实 call site。单实例冲突、正则或 worker 超时均为硬失败。'
$taskDefinitionTransactionSuffixEn = "`n`n" + ('[Task-definition repair transaction] Keep the official task-definition file unchanged while editing. Run tools/test/task_definition_repair_transaction.ps1 -Mode Prepare with the ticket id, official task path, stage, failing D round, and operation index; read operation-preview.json, operation-preview.txt, and apply-patch-context.txt, then edit only the emitted candidate.json with VS Code apply_patch. After editing, preferably run read-only -Mode Inspect to refresh the SHA-256-bound preview and resolve zero/multiple matches, a still-matchable post-replacement pattern, or double-escape warnings. Inspect changes no candidate, official task definition, or business source and does not replace validation gates. Run -Mode Validate, which reports preview_stale=true|false as diagnostic evidence, binds the unchanged official baseline hash, and performs SyntaxOnly, the focused op check when configured, and the full failing-round check. Run -Mode Promote only after validation succeeds; promotion rechecks official and candidate hashes, atomically replaces the official file, performs a post-write SyntaxOnly check, writes a promotion receipt, and then removes candidate/baseline working copies. Candidate edited, preview refreshed, Inspect PASS, or a focused checker PASS is not repair completion. Before recovery_transaction_command, restart, or resume, require the same-ticket manifest state promoted, validated/promoted/official SHA-256 equality, a successful promotion receipt with matching ticket/hash, and a fresh strict check of the official file for the current failing round. prepared, validation_failed, promotion_failed, quarantined, abandoned, missing receipt, or hash mismatch must fail-close: continue a permitted candidate repair or report the blocker, but do not recover or claim successful handled closeout. Brief task_definition_promotion_gate: {0}. Failed, stale, abandoned, or quarantined candidates remain under out/artifacts for diagnosis and retention pruning. If tool output contains unexpected tokens, altered paths, malformed commands, or corrupted patch context, treat it as tool-call-parameter corruption: stop complex retries, reread authoritative files, run -Mode Quarantine, and never promote or restart from ambiguous evidence. Diagnose from the first structured child failure, checker effective source/manifest, and first compiler/test error. Do not infer that remote builds cannot see uncommitted work; remote_build_and_test.sh uploads the current local worktree, so verify transport, upload, VERSION, hashes, and logs.' -f $taskDefinitionPromotionGate)
$taskDefinitionTransactionSuffixZh = "`n`n" + ('[任务定义修复事务] 编辑期间保持正式任务定义文件不变。先用票据 ID、正式任务路径、stage、故障 D 轮和 operation index 执行 tools/test/task_definition_repair_transaction.ps1 -Mode Prepare；读取 operation-preview.json、operation-preview.txt 与 apply-patch-context.txt，只允许用 VS Code apply_patch 修改其生成的 candidate.json。修改后推荐执行只读 -Mode Inspect 刷新 SHA-256 绑定预览，先排除零/多匹配、替换后 pattern 仍匹配与双重转义风险；Inspect 不修改 candidate、正式任务定义或业务源码，也不替代验证门禁。随后执行 -Mode Validate；该步骤输出 preview_stale=true|false 作为诊断证据，绑定未变化的正式基线哈希，并依次执行 SyntaxOnly、已配置时的目标 op 快检和故障轮完整检查。仅在验证成功后执行 -Mode Promote；提升会重新校验正式文件与候选哈希，原子替换正式文件，执行写后 SyntaxOnly，持久化 promotion receipt，然后删除 candidate/baseline 工作副本。candidate 已编辑、preview 已刷新、Inspect PASS 或局部 checker PASS 都不表示修复完成。执行 recovery_transaction_command、重启或 resume 前，必须确认同票据 manifest.state=promoted，validated/promoted/正式 SHA-256 一致，promotion receipt 成功且 ticket/hash 匹配，并对正式文件再次运行当前故障轮不带 -OperationIndex 的严格检查。prepared、validation_failed、promotion_failed、quarantined、abandoned、receipt 缺失或哈希不一致必须 fail-close：只继续允许范围内的 candidate 修复或报告阻塞，禁止恢复或成功 handled 收尾。brief 的 task_definition_promotion_gate：{0}。失败、过期、放弃或隔离的候选保留在 out/artifacts，供诊断和统一保留策略清理。若工具回显出现意外词元、路径改写、命令畸变或 patch 上下文污染，按工具调用参数污染处理：停止复杂重试，重读权威文件，执行 -Mode Quarantine，禁止从不明确证据提升或重启。诊断必须下钻到第一个结构化子失败、checker 有效源码/manifest 和编译器或测试首错。禁止仅凭未提交/未推送推断远端看不到修改；remote_build_and_test.sh 上传当前本地工作树，必须核对 transport、upload、VERSION、哈希和日志。' -f $taskDefinitionPromotionGate)
...
$vxCodeFixSuffixEn = ''
$vxCodeFixSuffixZh = ''
if ($briefTaskDefinitionSchemaVersion -eq 'vx-draft') {
    $vxCodeFixSuffixEn = "`n`n[Vx multi-target contract] The brief binds target_set_sha256=$briefTargetSetSha256 and global operation_index=$briefFailureOperation to target id=$briefFailureTargetId path=$briefFailureTargetPath kind=$briefFailureTargetKind lifecycle=$briefFailureTargetLifecycle. Operation indexes are global across files within the round; Inspect must simulate every preceding operation in order, including operations on other targets. Keep the official schemaVersion, targetFiles, defaultTarget, and frozen target registry read-only; edit only the permitted operation/assertion range in candidate.json, and reference only registered targets. A create-file fault never authorizes directly creating or overwriting a business file: repair only the candidate operation, and already-exists still must pass content binding, markers, and target assertions. Split declaration, definition, and caller changes into separate single-target operations with per-target assertions. Validate, Promote, and recovery manifests/receipts must bind the same target_set_sha256. Missing or inconsistent schema/target facts, registry drift, or receipt mismatch must fail-close without guessing from logs or paths. Code-step create/commit/journal/rollback faults remain noncode. rollback_status=incomplete is a hard block on edit/promote/restart/resume."
    $vxCodeFixSuffixZh = "`n`n[Vx 多目标契约] brief 将 target_set_sha256=$briefTargetSetSha256、轮内全局 operation_index=$briefFailureOperation 绑定到 target id=$briefFailureTargetId path=$briefFailureTargetPath kind=$briefFailureTargetKind lifecycle=$briefFailureTargetLifecycle。operation index 是跨文件全局顺序；Inspect 必须按顺序模拟全部前置 op，包括其他 target 上的 op。正式定义的 schemaVersion、targetFiles、defaultTarget 与冻结 target registry 均只读；只能编辑 candidate.json 中允许范围内的 operation/assertion，并且只能引用已注册 target。create-file 故障绝不授权直接创建或覆盖业务文件，只能修 candidate 中的 operation；already-exists 仍必须通过内容绑定、marker 和 target assertions。declaration、definition、caller 修改必须拆成多个单目标 op，并分别添加 target assertion。Validate、Promote 和 recovery 的 manifest/receipt 必须绑定同一 target_set_sha256。schema/target 事实缺失、registry 漂移或 receipt 不一致时必须 fail-close，禁止从日志或路径猜测。code-step 的创建、提交、journal 或回滚故障仍属于 noncode；rollback_status=incomplete 硬阻断 edit/promote/restart/resume。"
}
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    elseif ($roundIsD -and $isTaskStaticFault) {
        ...
        if ($taskStaticCrossRoundRepairEnabled) {
            $selfHealRuleSuffixEn += " After every edit, rerun the checker from ${roundTag} with -ChainRounds. It must replay converged rounds on the chained in-memory source and stop at the first unresolved round. Repair only that round, never skip it or run a later round independently, then rerun the same chain from ${roundTag} until D4 passes."
            $selfHealRuleSuffixZh += " 每次编辑后都从 ${roundTag} 带 -ChainRounds 重跑 checker，由链式内存源码重放已收敛轮并停在首个未收敛轮。只修该轮，禁止跳过或单独运行后续轮；随后仍从 ${roundTag} 重跑同一条链，直到 D4 通过。"
        }
        else {
    ...
    elseif ($roundIsV) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] This ticket failed in round ${roundTag}. V1-V4 are compile/verify phase names, NOT round keys in the JSON. Append the incremental patch as one or more new operations consecutively at the END of the D4 operations array. Do NOT create V1-V4 round entries, edit D1-D3, or modify/delete any existing D4 content. After appending, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce). A failure blocks restart/resume: repair within this allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] 本票故障发生轮次：${roundTag}。V1-V4 是编译/验证阶段名称，不是 JSON 中的轮次键名。仅可将增量补丁作为一个或多个新 op 连续追加到 D4 operations 数组末尾；不得创建 V1-V4 轮次条目，不得改 D1-D3，也不得修改/删除 D4 既有内容。追加后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
...
任何重启前必须运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    ...
    $selfHealRuleSuffixEn += $taskDefinitionTransactionSuffixEn
    $selfHealRuleSuffixZh += $taskDefinitionTransactionSuffixZh
    $selfHealRuleSuffixEn += $crossRoundTransactionSuffixEn
    $selfHealRuleSuffixZh += $crossRoundTransactionSuffixZh
    $selfHealRuleSuffixEn += $boundArtifactCorrectionEn
    $selfHealRuleSuffixZh += $boundArtifactCorrectionZh
    $selfHealRuleSuffixEn += $vxCodeFixSuffixEn
    $selfHealRuleSuffixZh += $vxCodeFixSuffixZh
}

$gitGuardSuffixEn = ' During unattended execution, do not run git checkout, git commit, or git push unless the user explicitly authorizes the specific command in the same turn.'
$gitGuardSuffixZh = ' 无人值守运行期间禁止执行 git checkout / git commit / git push；仅在用户同轮明确授权后才可提交或推送。'
$passiveWaitSuffixEn = ' Follow every ticket step without omission. When recovery_transaction_command is present, execute it exactly once as the recovery and closeout transaction; otherwise execute atomic_closeout_command exactly once at the end of the event ticket. Do not execute either closeout path again after success or failure. Claim closure only from successful machine facts; split receipt fields are audit-only and must not be run one by one. The final chat reply for the ticket must end with a standalone line exactly in the form `handled_at: YYYY-MM-DD HH:mm:ss`, copied from the successful machine output. After this ticket is handled, wait silently for the next event/status ticket delivered by the existing guard/trigger/dispatch chain. Do not create or run scheduled monitoring scripts, polling loops, background jobs, watchers, persistent PowerShell commands, or long-running/cross-round monitoring commands; these can interrupt closure when the next ticket arrives. The 3-minute closeout window is a target, not an agent-enforced wall-clock timeout. After starting recovery_transaction_command, wait for that synchronous command to exit naturally; never kill, Stop-Process, terminate its terminal, or cancel it merely because it exceeds 3 minutes or 240 seconds. The 240-second value belongs only to the transaction script internal stage-start verification; determine success or failure only from the command exit code and JSON machine facts.'
$passiveWaitSuffixZh = ' 严格按票据流程执行，不遗漏任何操作；当 brief 提供 recovery_transaction_command 时，将它作为“恢复+闭环事务”只执行一次；否则在事件票处理结束时只执行一次 atomic_closeout_command。无论成功或失败，任一闭环路径都不得再次执行。仅凭成功的机器事实声称闭环，旧分步回执字段只作审计兼容、不得逐条执行。本票最终聊天回复必须以独立末行 `handled_at: YYYY-MM-DD HH:mm:ss` 结束，时间从成功机器输出原样复制。本票闭环后，静默等待现有 guard/trigger/dispatch 链投送下一张事件票或状态票；不得创建或运行定时巡检脚本、轮询循环、后台 job、watcher、常驻 PowerShell 命令或长时间跨轮次巡检命令，这些命令可能在下一张事件票到达时中断收尾。3 分钟收尾窗口仅是目标，不是 Agent 执行的事务总墙钟超时。启动 recovery_transaction_command 后必须等待该同步命令自然退出；不得仅因超过 3 分钟或 240 秒而 kill、Stop-Process、终止终端或取消事务。240 秒仅属于事务脚本内部的 stage 启动验证；成功或失败只依据命令退出码和 JSON 机器事实。'
...
function Add-GitGuardConstraint {
    ...
    if ($normalized.Contains('git checkout') -and $normalized.Contains('git commit') -and $normalized.Contains('git push')) {
        return $Template
    }
...
elseif ($eventNormalized -eq 'task-definition-fix-required') {
    ...
    $taskDefinitionMandatorySuffix = if ($useChineseDispatchMessage) {
        $taskDefinitionSafetySuffixZh + $taskDefinitionTransactionSuffixZh + $boundArtifactCorrectionZh
    }
    else {
        $taskDefinitionSafetySuffixEn + $taskDefinitionTransactionSuffixEn + $boundArtifactCorrectionEn
    }
    $taskDefinitionRuleSuffix = if (-not [string]::IsNullOrWhiteSpace($selfHealRuleSuffixEn) -or -not [string]::IsNullOrWhiteSpace($selfHealRuleSuffixZh)) {
        if ($useChineseDispatchMessage) { $selfHealRuleSuffixZh } else { $selfHealRuleSuffixEn }
    }
    else {
        $taskDefinitionMandatorySuffix
    }
    $firstMessage = ("{0}`n`n{1}" -f $firstMessage.TrimEnd(), $taskDefinitionRuleSuffix.Trim())
}
else {
