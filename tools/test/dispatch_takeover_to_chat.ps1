...
After fixing a code-step fault, validate only the current failing op with `tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`; the checker simulates read-only preceding ops in order and checks op n as the target. Whole-round runtime dry-run remains sequential, so later ops may depend on earlier replacements. For helper forward-declaration repairs, preserve the helper definition, leave exactly one prototype before the first caller, and remove prototypes after the caller or duplicates. Treat any static-check failure as a hard restart block: use its diagnostics to continue repairing only within the allowed edit boundary, then rerun the check. Restart/resume is allowed only after the check passes. If it cannot pass without violating the boundary or another gate, report the blocker and do not restart. Restart only the correct stage main process for this ticket by procedure (A issue -> restart A, B issue -> restart B; never guess or switch stages), continue monitoring, and return chat_heartbeat plus a fix conclusion.'
...
Do not mix with script-only remediation. After fixing a code-step fault, validate only the current failing op with `tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`; the checker sequentially simulates read-only preceding ops and checks op n as the target. Whole-round runtime dry-run remains sequential. For helper forward-declaration repairs, preserve the helper definition, leave exactly one prototype before the first caller, and remove prototypes after the caller or duplicates. Treat any static-check failure as a hard restart block: use its diagnostics to continue repairing the task definition only within the allowed edit boundary, then rerun the check. Restart/resume is allowed only after the check passes. If it cannot pass without violating the boundary or another gate, report the blocker and do not restart. Restart only the correct stage main process for the ticketed phase (A issue -> restart A, B issue -> restart B); do not guess or switch phases. Complete bounded fix -> verify -> resume chain, then return handled_at with exact fix scope and validation evidence.'
...
$noticeInfraMessageEn = 'Please take over ticket {0} (event={1}) in KNOWN-INFRA-TRANSIENT notice flow: read {3} first, run route_guard_command, execute environment stabilization workflow, and return handled_at. Resume/restart is blocked until infra is confirmed stable.'
...
修复 code-step 故障后，只检查触发故障的当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`；checker 会按顺序模拟只读前置 op，仅将 op n 作为目标检查。整轮运行时 dry-run 仍按顺序执行，允许后续 op 依赖前序 replacement。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。静态检查失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有静态检测通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。重启时只能按本票目标阶段启动正确的主进程（A 问题重启 A，B 问题重启 B，禁止猜阶段、串阶段），然后继续监控，最后回传 chat_heartbeat 与修复结论。'
...
修复 code-step 故障后，只检查触发故障的当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`；checker 会按顺序模拟只读前置 op，仅将 op n 作为目标检查，整轮运行时 dry-run 仍按顺序执行。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。静态检查失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有静态检测通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
...
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    elseif ($roundIsD -and $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] Code-step phase fault in ${roundTag}: edit only this D round and later D rounds (D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4); never edit an earlier D round. The source has NOT yet been changed by ${roundTag}. In ${roundTag}, ops before the current failing op are read-only; modify, delete, insert, or append only from the failing op onward. After changes, check only that failing op with -RoundTag ${roundTag} -OperationIndex <n>; the checker simulates preceding ops as read-only prerequisites. Preserve helper definitions and keep exactly one prototype before the first caller, removing later/duplicate prototypes. A failure blocks restart/resume: repair within this allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} code-step 阶段故障：仅可修改当前 D 轮及后续 D 轮（D1 -> D1-D4，D2 -> D2-D4，D3 -> D3-D4，D4 -> D4），禁止改前置 D 轮。${roundTag} 源码尚未变更；该轮故障 op 之前的 op 为只读，仅可从故障 op 位置起修改、删除、插入或追加 op。变更后仅检查当前故障 op，使用 -RoundTag ${roundTag} -OperationIndex <n>；checker 将前置 op 作为只读前提顺序模拟。必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除后置或重复 prototype。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
    elseif ($roundIsV) {
    ...
    elseif ($roundIsD -and -not $isCompileVerifyFault -and -not $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] ${roundTag} fault (type=$kindTag): since the round is known, treat as code-step phase. Ops before the current failing op are read-only; modify, append, insert, or delete only from that op onward. Then check only that failing op with -RoundTag ${roundTag} -OperationIndex <n>; preceding ops are simulated as read-only prerequisites. A failure blocks restart/resume: repair within the allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} 故障（类型=$kindTag）：已知开发轮次，按 code-step 阶段处理。当前故障 op 之前只读，仅可从该 op 起修改、追加、插入或删除。随后使用 -RoundTag ${roundTag} -OperationIndex <n> 仅检查当前故障 op；前置 op 仅作为只读前提被模拟。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
    else {
    ...
    If D1-D4 code-step phase failed (code-edit-failure / task-definition-mismatch): source NOT yet changed; in the failing round, ops before the failing op are read-only and changes start at that op.
...
Always run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce) before any restart/resume. A failure blocks restart/resume: repair within the allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
    ...
    若 D1-D4 code-step 阶段失败（code-edit-failure / task-definition-mismatch）：源码尚未变更；当前故障轮中，故障 op 之前只读，变更从故障 op 开始。
...
elseif ($eventNormalized -eq 'task-definition-fix-required') {
    ...
    switch ($routeGuardExpected) {
        ...
        'incident-manual-code-fix' { $firstMessage = $codeFixRecoveryMessage -f $TicketId, $TicketEvent, $startFileRel, $dispatchReadContextText; break }
