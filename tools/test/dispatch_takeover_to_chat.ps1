...
After fixing a code-step fault, run SyntaxOnly first, then validate the current failing op with `tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>` when it is locatable; the checker simulates read-only preceding ops in order. Then run the same current round without -OperationIndex. Code-step invokes this checker as the shared full-round engine and writes target source only after operations, replay, and postApplyAssertions pass. The checker stops at the first failed op; later ops and rounds are not evaluated. Rerun checker as often as diagnostics require within this repair ticket; these local checks do not consume the identical-fingerprint main-process relaunch budget. For helper forward-declaration repairs, preserve the helper definition, leave exactly one prototype before the first caller, and remove prototypes after the caller or duplicates. Treat single-instance conflict or any regex/worker timeout as a hard restart block. Restart/resume is allowed only after the current failing round passes. Restart only the correct stage main process for this ticket by procedure (A issue -> restart A, B issue -> restart B; never guess or switch stages), continue monitoring, and return chat_heartbeat plus a fix conclusion.'
...
Do not mix with script-only remediation. After fixing a code-step fault, run SyntaxOnly first, then use `-RoundTag <Dn> -OperationIndex <n>` when the failed op is locatable, and finally check the current failing round without -OperationIndex. Code-step invokes the checker as the shared full-round engine and writes target source only after operations, replay, and postApplyAssertions pass. The checker sequentially advances only after each op passes and stops at the first failure; do not preflight later rounds. Rerun checker as often as diagnostics require within this repair ticket; these local checks do not consume the identical-fingerprint main-process relaunch budget. For helper forward-declaration repairs, preserve the helper definition, leave exactly one prototype before the first caller, and remove prototypes after the caller or duplicates. Treat single-instance conflict or regex/worker timeout as a hard restart block. Restart/resume is allowed only after the current failing round passes. Restart only the correct stage main process for the ticketed phase (A issue -> restart A, B issue -> restart B); do not guess or switch phases. Complete bounded fix -> verify -> resume chain, then return handled_at with exact fix scope and validation evidence.'
...
修复 code-step 故障后，先运行 SyntaxOnly；可定位时用 -OperationIndex 快检当前 op，再对当前故障轮运行不带 -OperationIndex 的递进严格检查。code-step 内部复用 checker 作为完整整轮执行引擎，仅在 operations、replay 与 postApplyAssertions 全部通过后写回目标源码；checker 首错即停，不检查后续 op 或轮。可在同一修复工单内按诊断反复调用 checker，这些本地检查不消耗相同指纹主进程重启预算。单实例冲突、正则或 worker 超时均硬阻断。只有当前故障轮通过才允许重启或 resume。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
...
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    elseif ($roundIsD -and -not $isCompileVerifyFault -and -not $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] ${roundTag} fault (type=$kindTag): since the round is known, treat as code-step phase. Ops before the current failing op are read-only; modify, append, insert, or delete only from that op onward. Then first check that failing op with -RoundTag ${roundTag} -OperationIndex <n>; preceding ops are simulated as read-only prerequisites. A failure blocks restart/resume: repair within the allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} 故障（类型=$kindTag）：已知开发轮次，按 code-step 阶段处理。当前故障 op 之前只读，仅可从该 op 起修改、追加、插入或删除。随后先使用 -RoundTag ${roundTag} -OperationIndex <n> 检查当前故障 op；前置 op 仅作为只读前提被模拟。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
    else {
