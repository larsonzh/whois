...
'@
$runningStatusShortMessageZh = '[SHORT-CARD] 票据 {0}（event={1}），先读 {2}。1）用一段话先报根因+修复路径；若本票是 healthy 的 running-status-report，根因应写“无活动故障/常规定时状态票”，修复路径应写“continue_watch only”。2）若可自愈且非不可恢复环境、且未触发预算/冷却耗尽，立即执行 business_resume（business_command -> continue_watch_command；business 为空仅 continue）。3）处置后回到只读盯盘（10 分钟心跳+轮询），直到“stop monitoring”，不要再询问是否继续轮询/监控。4）强制回执：本票当轮动作完成后必须回传 handled_at（YYYY-MM-DD HH:mm:ss）；running-status-report 在 business/continue_watch 后必须立即回传 handled_at；session_closed_at 仅在 stop monitoring 或 A/B 终态时回传。不得仅凭旧 exit 证据推断需要重启 B；不得手工创建 chat_heartbeat/handled 临时回执文件；不得在未获同意时创建非 tmp 新脚本。回传 chat_heartbeat：SESSION/A/B、run_dir、main_round、supervisor/companion/guard 最新心跳、B exit digest。状态：{3}。'
