...
修复后执行必要验证；如果改动了 task-definition，必须运行 `tools/test/check_task_definition_static.ps1`，并将失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有静态检测通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。重启时只能按本票目标阶段启动正确的主进程（A 问题重启 A，B 问题重启 B，禁止猜阶段、串阶段），然后继续监控，最后回传 chat_heartbeat 与修复结论。'
...
如果改动了 task-definition，必须运行 `tools/test/check_task_definition_static.ps1`，并将失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有静态检测通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
...
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    elseif ($roundIsD -and $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] Code-step phase fault in ${roundTag}: edit only this D round and later D rounds (D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4); never edit an earlier D round. The source has NOT yet been changed by ${roundTag}. In ${roundTag}, ops before the current failing op are read-only; modify, delete, insert, or append only from the failing op onward. After changes, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce). A failure blocks restart/resume: repair within this allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} code-step 阶段故障：仅可修改当前 D 轮及后续 D 轮（D1 -> D1-D4，D2 -> D2-D4，D3 -> D3-D4，D4 -> D4），禁止改前置 D 轮。${roundTag} 源码尚未变更；该轮故障 op 之前的 op 为只读，仅可从故障 op 位置起修改、删除、插入或追加 op。变更后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
    elseif ($roundIsV) {
    ...
    elseif ($roundIsD -and -not $isCompileVerifyFault -and -not $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] ${roundTag} fault (type=$kindTag): since the round is known, treat as code-step phase. You may modify, append, or delete ops; then run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce) on the most-forward changed op. A failure blocks restart/resume: repair within the allowed boundary using the diagnostics and rerun the check until it passes; if it cannot pass compliantly, report the blocker and do not restart."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} 故障（类型=$kindTag）：已知开发轮次，按 code-step 阶段处理。可修改/追加/删除 op，然后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce），优先检查最靠前变更 op。检查失败即阻断重启：必须依据诊断在允许边界内继续修复并重新检查，直至通过；若无法合规通过，报告阻塞且不得重启。"
    }
    else {
