...
$genericRecoveryMessageEn = 'Please take over ticket {0} (event={1}) and execute recovery by {2}: read {3} first, run route_guard_command from the brief and follow its classification before any action, report root cause plus remediation method, and if self-healable without budget/cooldown exhaustion or nonrecoverable environment, trigger business_resume immediately. After handling, keep read-only monitoring with 10-minute heartbeat + poll cadence until "stop monitoring", and apply D1 90/30/10/20 rules.'
...
$genericRecoveryMessageZh = '请接管票据 {0}（event={1}），按 {2} 执行恢复：先阅读 {3}，并先执行 brief 中的 route_guard_command，必须按其 classification 决定分支后再执行动作；先汇报根因与修复方法；若可自愈且未触发预算/冷却耗尽且非不可恢复环境，立即触发 business_resume。处置后进入只读盯盘，按 10 分钟心跳 + 轮询持续监控，直到“stop monitoring”，并执行 D1 90/30/10/20 规则。'
