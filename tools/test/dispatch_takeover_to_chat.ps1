...
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    if ($roundIsD -and $isCompileVerifyFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] Compile/verify phase fault in ${roundTag}: edit only this D round and later D rounds (D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4); never edit an earlier D round. The source has already been changed by ${roundTag} (code-step PASSED), so its existing ops are read-only: append one or more new ops consecutively only at the END of ${roundTag} operations. After changing, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce); static check must pass before any restart/resume."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} 编译/验证阶段故障：仅可修改当前 D 轮及后续 D 轮（D1 -> D1-D4，D2 -> D2-D4，D3 -> D3-D4，D4 -> D4），禁止改前置 D 轮。${roundTag} 源码已变更（code-step 已通过），其既有 op 均为只读，只能在 ${roundTag} operations 数组末尾连续追加一个或多个新 op。变更后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）；静态检查通过后才可重启。"
    }
    elseif ($roundIsD -and $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] Code-step phase fault in ${roundTag}: edit only this D round and later D rounds (D1 -> D1-D4, D2 -> D2-D4, D3 -> D3-D4, D4 -> D4); never edit an earlier D round. The source has NOT yet been changed by ${roundTag}. In ${roundTag}, ops before the current failing op are read-only; modify, delete, insert, or append only from the failing op onward. After changes, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce); static check must pass before any restart/resume."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} code-step 阶段故障：仅可修改当前 D 轮及后续 D 轮（D1 -> D1-D4，D2 -> D2-D4，D3 -> D3-D4，D4 -> D4），禁止改前置 D 轮。${roundTag} 源码尚未变更；该轮故障 op 之前的 op 为只读，仅可从故障 op 位置起修改、删除、插入或追加 op。变更后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）；静态检查通过后才可重启。"
    }
    elseif ($roundIsV) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] V1-V4 verify-phase fault: V1-V4 are compile/verify phase names, NOT round keys in the JSON. Append the incremental patch as one or more new operations consecutively at the END of the D4 operations array. Do NOT create V1-V4 round entries, edit D1-D3, or modify/delete any existing D4 content. After appending, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce); static check must pass before any restart/resume."
        $selfHealRuleSuffixZh = "[自愈修复规则] V1-V4 纯验证阶段故障：V1-V4 是编译/验证阶段名称，不是 JSON 中的轮次键名。仅可将增量补丁作为一个或多个新 op 连续追加到 D4 operations 数组末尾；不得创建 V1-V4 轮次条目，不得改 D1-D3，也不得修改/删除 D4 既有内容。追加后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce）；静态检查通过后才可重启。"
    }
