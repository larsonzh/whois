...
$taskDefinitionFixMessageZh = '请接管票据 {0}（event={1}），先阅读 {2}。请诊断根因是否为 task-static 阶段中 task-definition 与当前源码形态不匹配，并给出最小修复。只有 task-static 故障和经分类确认为代码故障的编译/验证故障可进入本流程；所有 code-step 故障均属于 noncode。编辑期间保持 testdata 下正式任务定义只读：先执行 task_definition_repair_transaction.ps1 -Mode Prepare，只允许使用 VS Code `apply_patch` 修改生成的 candidate.json，再按顺序执行 Validate 与 Promote；不得直接修改正式任务定义或业务源码。禁止用终端内联 Python/PowerShell、重定向、通用字符串替换或格式化器修改任务定义语义。诊断必须以第一个结构化子失败和编译器/测试首错为准，外层 wrapper 摘要不能覆盖结构化子证据。修复 task-static 故障后，依次执行 SyntaxOnly、故障目标 op 快检（可定位时）和当前故障 D 轮递进严格检查，再遵循 brief 中的 task_static_cross_round_repair_enabled：false 时当前轮通过即停止，true 时按顺序逐轮检查并修复后续 D 轮到 D4，全部通过后才恢复。若已知是某个阶段某一轮（例如 B D4），要把目标 stage / round / 文件名写清楚。
...
$codeFixRecoveryMessageZh = '请接管票据 {0}（event={1}），进入“代码修复专用流程”：先阅读 {3}，执行 route_guard_command，并严格按 route.allowed_actions 执行。仅处理源码/任务定义不匹配、编译或校验失败，不与脚本修复流程混用。诊断必须下钻到第一个结构化子失败和编译器/测试首错，不得用外层 wrapper 摘要覆盖子流程证据。如果这是自愈生成物修复，保持 testdata 下对应阶段正式任务定义只读：先执行 task_definition_repair_transaction.ps1 -Mode Prepare，只用 VS Code `apply_patch` 修改 candidate.json，再按顺序执行 Validate 与 Promote；不得直接修改正式任务定义或业务源码。禁止终端内联 Python/PowerShell、重定向、通用字符串替换或格式化器代改。验证顺序固定为 SyntaxOnly 装载检查、故障目标 op 快检（可定位时）、当前故障 D 轮递进严格检查。上述本地验证通过后不得停下：brief 提供 pre_restart_launch_ready_command 时先执行它；brief 提供 recovery_transaction_command 时必须只执行一次；否则只执行唯一允许的 atomic closeout 路径。若处理本代码修复票时发现脚本故障，必须停止代码修复流程并按脚本策略重新分类：仅当 LOCAL_GUARD_SCRIPT_SELF_HEAL_ENABLED 显式为 true 时进入“脚本自愈专用流程”，否则进入“脚本故障排查专用流程”，不得修改文件或控制进程。
