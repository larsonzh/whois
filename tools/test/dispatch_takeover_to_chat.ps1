...
Do not mix with script-only remediation. After fixing a code-step fault, first validate the current failing op with `tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`; the checker sequentially simulates read-only preceding ops and checks op n as the target. Whole-round runtime dry-run remains sequential. For helper forward-declaration repairs, preserve the helper definition, leave exactly one prototype before the first caller, and remove prototypes after the caller or duplicates. Treat any static-check failure as a hard restart block: use its diagnostics to continue repairing the task definition only within the allowed edit boundary, then rerun the check. Restart/resume is allowed only after the focused check and all affected full-round checks pass. If they cannot pass without violating the boundary or another gate, report the blocker and do not restart. Restart only the correct stage main process for the ticketed phase (A issue -> restart A, B issue -> restart B); do not guess or switch phases. Complete bounded fix -> verify -> resume chain, then return handled_at with exact fix scope and validation evidence.'
...
修复 code-step 故障后，先检查触发故障的当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`；checker 会按顺序模拟只读前置 op，仅将 op n 作为目标检查。整轮运行时 dry-run 仍按顺序执行，允许后续 op 依赖前序 replacement。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。静态检查失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有目标检查和所有受影响轮的整轮检查均通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。重启时只能按本票目标阶段启动正确的主进程（A 问题重启 A，B 问题重启 B，禁止猜阶段、串阶段），然后继续监控，最后回传 chat_heartbeat 与修复结论。'
...
修复 code-step 故障后，先检查触发故障的当前 op：`tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce -RoundTag <Dn> -OperationIndex <n>`；checker 会按顺序模拟只读前置 op，仅将 op n 作为目标检查，整轮运行时 dry-run 仍按顺序执行。修复 helper 前向声明时必须保留 helper definition，在首次 caller 前仅保留一个 prototype，并删除 caller 后或重复的 prototype。静态检查失败视为重启硬阻断：依据诊断仅在允许修改边界内继续修复任务定义，然后重新检查。只有目标检查和所有受影响轮的整轮检查均通过才允许任何重启或 resume；若无法在不违反边界或其他门禁的前提下通过检查，必须报告阻塞且不得重启。完成 fix -> verify -> resume 的闭环后回传 handled_at，并说明修复范围与验证证据。'
