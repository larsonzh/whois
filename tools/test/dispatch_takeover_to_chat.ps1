...
if ($routeGuardExpected -in @('incident-auto-resume-code-fix', 'incident-manual-code-fix')) {
    ...
    elseif ($roundIsD -and $isCodeStepFault) {
        $selfHealRuleSuffixEn = "[Self-Heal Rule] Code-step phase fault in ${roundTag}: the source has NOT yet been changed by this round. You may modify, append, or delete ops within this round. To avoid changing already-validated steps, do NOT modify any op before the current failing op; you may only modify, append, or delete ops starting from the current failing op position and onward. After changes, run static check (tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce) on the MOST-FORWARD modified/appended op; static check must pass before any restart/resume."
        $selfHealRuleSuffixZh = "[自愈修复规则] ${roundTag} code-step 阶段故障：源码尚未变更，可修改/追加/删除该轮次中的 op。为避免误改已验证步骤，禁止修改当前故障 op 之前的 op；仅允许从当前故障 op 位置开始，向后修改/追加/删除该轮次中的 op。变更后运行静态检查（tools/test/check_task_definition_static.ps1 -TaskDefinitionFile <file> -Policy enforce），优先检查该轮次排序最靠前的修改/追加 op；静态检查通过后才可重启。"
    }
    elseif ($roundIsV) {
