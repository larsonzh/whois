...
$scriptFixRecoveryMessageEn = 'Please take over ticket {0} (event={1}) in SCRIPT-FIX dedicated flow: read {3} first, run route_guard_command, and execute only route.allowed_actions. Focus on unattended script self-heal path (guard/trigger/dispatch/poll scripts), keep business source unchanged unless explicitly required by route. If eligible, prefer recovery_transaction_command after script fix verification; otherwise execute the brief-provided business_resume/continue_watch/closeout sequence. Keep evidence concise and deterministic.'
...
$scriptFixRecoveryMessageZh = '请接管票据 {0}（event={1}），进入“脚本自愈专用流程”：先阅读 {3}，执行 route_guard_command，并严格按 route.allowed_actions 执行。仅处理无人值守脚本链路（guard/trigger/dispatch/poll）问题，除非路由明确允许，不要混入业务源码改动。修复后做有界验证；满足恢复条件时优先执行 recovery_transaction_command，否则执行 brief 提供的 business_resume/continue_watch/closeout 分步链。'
...
$nonCodeRecoveryMessageZh = '请接管票据 {0}（event={1}），进入“非代码故障专用流程”：先阅读 {3}，执行 route_guard_command，并严格按 route.allowed_actions 执行。所有 code-step 故障都属于本流程，因为 code-step 只执行读取、验证、原子写入和写后验证；code-step 故障绝不允许修改源码或任务定义。编译/验证阶段经分类为环境、权限、磁盘、网络、锁、工具链或测试基础设施的故障也留在本流程。先稳定非代码原因，再仅执行路由允许的同阶段恢复并回传 handled_at。'
