...
$genericRecoveryMessageZh = '请接管票据 {0}（event={1}），按 {2} 执行恢复：先阅读 {3}，并先执行 brief 中的 route_guard_command，必须按其 classification 决定分支后再执行动作；仅允许执行“动作映射在 route.allowed_actions 内”的命令（硬白名单），route.blocked_actions 视为禁止项。先汇报根因与修复方法；若可自愈且未触发预算/冷却耗尽且非不可恢复环境，立即触发 business_resume。处置后进入只读盯盘，按 10 分钟心跳 + 轮询持续监控，直到“stop monitoring”，并执行 D1 90/30/10/20 规则。禁止创建非 tmp 新脚本或临时后台监控循环，仅允许使用 brief/poll 输出中已有仓库命令。'
