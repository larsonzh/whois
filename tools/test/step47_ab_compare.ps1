...
foreach ($q in $cases) {
    ...
    $trialRaw = & $BinaryPath --enable-step47-trial --step47-trial-scope $scopeNorm --debug --retry-metrics $q 2>&1
...
$eligibleCount = @($rows | Where-Object { $_.TrialAction -eq "step47-eligible" }).Count

Write-Output ("[STEP47-AB] out_dir={0}" -f $outDir)
Write-Output ("[STEP47-AB] scope={0} eligible={1} auth_changed={2} route_changed={3}" -f $scopeNorm, $eligibleCount, $authChangedCount, $routeChangedCount)
Write-Output ("[STEP47-AB] summary_csv={0}" -f $summaryCsv)
Write-Output ("[STEP47-AB] summary_txt={0}" -f $summaryTxt)

if ($authChangedCount -gt 0 -or $routeChangedCount -gt 0) {
