...
try {
    ...
    if ($existingPids.Count -gt 0) {
        if ($NoRestartIfRunning.IsPresent) {
            Write-Output ("[OPEN-AB-SUPERVISOR] restart_precheck existing_count={0} existing_pids={1} mode=reuse" -f $existingPids.Count, ($existingPids -join ','))
            $reuseExisting = $true
    ...
    if ([string]::IsNullOrWhiteSpace($supervisorLog) -or [string]::IsNullOrWhiteSpace($liveStatus)) {
        ...
        for ($attempt = 0; $attempt -lt 24; $attempt++) {
            if ($reuseExisting) {
                $supervisorDir = Get-LatestTimestampedDirectory -Root $supervisorRoot -After ([datetime]::MinValue)
            }
            else {
