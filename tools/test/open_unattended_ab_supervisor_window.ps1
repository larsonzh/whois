...
function Exit-LaunchMutex {
    ...
    if ([bool]$Context.Acquired) {
        try { $Context.Mutex.ReleaseMutex() | Out-Null } catch { $null = $_ }
    }

    try { $Context.Mutex.Dispose() } catch { $null = $_ }
}
...
$launchMutexContext = Enter-LaunchMutex -Role 'supervisor' -StartFilePath $startFilePath
try {
    $existingPids = @(Get-RunningMonitorProcessIdList -ScriptLeaf 'unattended_ab_supervisor.ps1' -StartFileIdentity $startFileIdentity -RepoRoot $repoRoot)
    $reuseExisting = $false
    $processId = 0

    if ($existingPids.Count -gt 0) {
        if ($NoRestartIfRunning.IsPresent) {
            $probeRoot = Join-Path $repoRoot 'out\artifacts\ab_supervisor'
            $probeDir = Get-LatestTimestampedDirectory -Root $probeRoot
            $probePaths = @()
            if ($null -ne $probeDir) {
                $probePaths += Join-Path $probeDir.FullName 'supervisor.log'
                $probePaths += Join-Path $probeDir.FullName 'live_status.json'
            }
            ...
            if ($reuseAlive) {
                Write-Output ("[OPEN-AB-SUPERVISOR] restart_precheck existing_count={0} existing_pids={1} mode=reuse stale_min={2}" -f $existingPids.Count, ($existingPids -join ','), $monitorReuseStaleMinutes)
                $reuseExisting = $true
                $processId = [int]$existingPids[0]
            }
            else {
                Write-Output ("[OPEN-AB-SUPERVISOR] restart_precheck existing_count={0} existing_pids={1} mode=restart-stale stale_min={2}" -f $existingPids.Count, ($existingPids -join ','), $monitorReuseStaleMinutes)
    ...
    if ([string]::IsNullOrWhiteSpace($supervisorLog) -or [string]::IsNullOrWhiteSpace($liveStatus)) {
        $supervisorRoot = Join-Path $repoRoot 'out\artifacts\ab_supervisor'
        $supervisorDir = $null
        for ($attempt = 0; $attempt -lt 24; $attempt++) {
            if ($reuseExisting) {
                $supervisorDir = Get-LatestTimestampedDirectory -Root $supervisorRoot
            }
            else {
        ...
        if ([string]::IsNullOrWhiteSpace($supervisorLog) -and $null -ne $supervisorDir) {
            $supervisorLog = Join-Path $supervisorDir.FullName 'supervisor.log'
        }

        if ([string]::IsNullOrWhiteSpace($liveStatus) -and $null -ne $supervisorDir) {
            $liveStatus = Join-Path $supervisorDir.FullName 'live_status.json'
        }
    }

    Write-Output ("[OPEN-AB-SUPERVISOR] pid={0} launcher_pid={1} script={2} start_file={3} start_from_stage={4} supervisor_log={5} live_status={6} reuse_existing={7}" -f $processId, $PID, $scriptPath, $StartFile, $StartFromStage, $supervisorLog, $liveStatus, [string]$reuseExisting)
}
finally {
