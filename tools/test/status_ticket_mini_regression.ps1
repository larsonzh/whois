...
if ($ContractGateOnly.IsPresent) {
    ...
    $repairTransactionPass = (
        ...
        $repairTransactionText.Contains('promote validation coverage incomplete') -and
        $repairTransactionText.Contains('promote chain coverage must end at D4') -and
        $repairTransactionText.Contains('[string]::Join('','', $actualValidatedRounds) -ne [string]::Join('','', $expectedValidatedRounds)') -and
    ...
    $operatorContractPass = (
        ...
        $copilotInstructionsText.Contains('`pattern_unmatched=0` 说明 JSON 已加载且正则已编译') -and
        $operationFlowText.Contains('checker 没有自制 JSON 解码器') -and
        $startTemplateText.Contains('`pattern_unmatched=0` 表示解析和正则编译均已完成') -and
        $dispatchText.Contains('modify only the emitted candidate.json with the VS Code apply_patch editing tool') -and
        $dispatchText.Contains('pattern_unmatched=0 means JSON loading and regex compilation already succeeded') -and
        $dispatchText.Contains('pattern_unmatched=0 表示 JSON 已加载且正则已编译') -and
        $takeoverTriggerText.Contains('use VS Code apply_patch only on candidate.json') -and
        $copilotInstructionsText.Contains('轮次归属不可转移（硬规则）') -and
        $operationFlowText.Contains('某个 D 轮自身的任务定义问题只能在该轮') -and
        $startTemplateText.Contains('某轮问题只能在该轮 operations 内修复') -and
        $dispatchText.Contains('A defect belonging to one round must be fixed only in that same round') -and
        $dispatchText.Contains('某轮问题只能在该轮内修复') -and
        $takeoverTriggerText.Contains("('task_definition_prepare_command={0}' -f `$repairPrepareCommand)") -and
        $takeoverTriggerText.Contains("('task_definition_inspect_command={0}' -f `$repairInspectCommand)") -and
        $takeoverTriggerText.Contains("('task_definition_checker_command={0}' -f `$repairCheckerCommand)") -and
        $takeoverTriggerText.Contains("('task_definition_validate_command={0}' -f `$repairValidateCommand)") -and
        $takeoverTriggerText.Contains("('task_definition_promote_command={0}' -f `$repairPromoteCommand)") -and
        $takeoverTriggerText.Contains("`$repairCheckerChainArgument = if (`$repairChainMode) { ' -ChainRounds' }") -and
        $takeoverTriggerText.Contains('after every edit rerun one -ChainRounds checker from the failing round')
    )
...
$atomicCloseoutContractChecks = [ordered]@{
    atomic_closeout_facts = $atomicCloseoutVerifiesFacts
    recovery_transaction_facts = $recoveryTransactionVerifiesFacts
    takeover_projection = $takeoverProjectsAtomicCloseout
    poll_projection = $pollProjectsAtomicCloseout
    closure_checker_projection = $closureCheckerProjectsAtomicCloseout
    prompt_split_closeout_rejected = $promptRejectsSplitCloseout
    dispatch_machine_facts = $dispatchRequiresMachineFacts
    dispatch_script_fault_routing = $dispatchRoutesDiscoveredScriptFault
    dispatch_compile_test_routing = $dispatchRecognizesCompileOrTestFailure
    guard_structured_classification = $guardClassifiesStructuredValidationBeforeScriptStack
    guard_script_fault_markers = $guardScriptFaultRequiresStrongMarkerOrNoStructuredExit
    guard_code_fault_delegation = $guardDelegatesCodeFaultRecovery
    copilot_inline_editing_forbidden = $copilotForbidsInlineEditing
}
$atomicCloseoutContractFailures = @($atomicCloseoutContractChecks.GetEnumerator() | Where-Object { -not [bool]$_.Value } | ForEach-Object { [string]$_.Key })
$atomicCloseoutContractPass = ($atomicCloseoutContractFailures.Count -eq 0)
$atomicCloseoutContractReason = if ($atomicCloseoutContractPass) { 'atomic-ticket-closeout-contract-present' } else { 'missing-atomic-ticket-closeout-contract:' + ($atomicCloseoutContractFailures -join ',') }
...
$copilotRequiresRepairPreview = $copilotInstructionsText.Contains('`operation-preview.json`、`operation-preview.txt` 与 `apply-patch-context.txt`') -and $copilotInstructionsText.Contains('推荐执行只读 `-Mode Inspect`') -and $copilotInstructionsText.Contains('Inspect 不修改 candidate、正式任务定义或业务源码，也不替代验证门禁')
...
$guardPersistsIncidentGeneration = $sessionGuardText.Contains('function Get-IncidentGenerationPersistedSignature') -and $sessionGuardText.Contains('function Set-IncidentGenerationPersistedSignature') -and $sessionGuardText.Contains('function Test-IncidentGenerationTicketAlreadyQueued') -and $sessionGuardText.Contains('LAST_INCIDENT_GENERATION_SIGNATURE = $signatureCompact') -and $sessionGuardText.Contains('$incidentGenerationAlreadyQueued = Test-IncidentGenerationTicketAlreadyQueued') -and $sessionGuardText.Contains('$incidentGenerationSignature -eq $persistedIncidentGeneration -or $incidentGenerationAlreadyQueued') -and $sessionGuardText.Contains('incident_generation_dedup_rehydrated source=queue-history') -and $sessionGuardText.Contains('reason=incident-generation-already-queued') -and $sessionGuardText.Contains('-DedupSuffix $incidentGenerationSignature') -and $sessionGuardText.Contains('Set-IncidentGenerationPersistedSignature -Signature $incidentGenerationSignature') -and $sessionGuardText.Contains('$statusRunDirSignature = (Convert-ToSingleLineText -Text $failureRunDirAnchor)') -and $sessionGuardText.Contains('$statusSignature = "{0}|{1}|{2}|{3}" -f $sessionStatus, $aStatus, $bStatus, $statusRunDirSignature')
...
$taskSafetyBriefProjectsSwitch = $takeoverTriggerText.Contains('task_static_cross_round_repair_enabled={0}') -and $takeoverTriggerText.Contains('check only the current failing D round before restart; later rounds remain runtime-gated') -and $takeoverTriggerText.Contains('after every edit rerun one -ChainRounds checker from the failing round')
$taskSafetyDispatchBranches = $dispatchText.Contains('[Cross-round repair enabled] Keep one candidate.') -and $dispatchText.Contains('Every checker run starts at the ticket failing round with -ChainRounds') -and $dispatchText.Contains('[Cross-round repair disabled] Use the failing-round source baseline') -and $dispatchText.Contains('[跨轮次修复已开启] 保持一个 candidate') -and $dispatchText.Contains('每次 checker 都从工单故障轮带 -ChainRounds 启动') -and $dispatchText.Contains('[跨轮次修复已关闭] 使用故障轮源码基线') -and $dispatchText.Contains('$taskStaticCrossRoundRepairEnabled = $briefTaskStaticCrossRoundRepairEnabled') -and $dispatchText.Contains('$taskDefinitionFixMessage -f $TicketId, $TicketEvent, $dispatchReadContextText, '''', $crossRoundRepairStatus') -and -not $dispatchText.Contains('$crossRoundRepairStatus, $firstMessage.TrimStart()')
$taskSafetyHasFailFast = $dispatchText.Contains('stop at the first failure') -and $dispatchText.Contains('首错即停') -and $dispatchText.Contains('checker defaults to one round') -and $dispatchText.Contains('checker 默认每次只验证一轮') -and $dispatchText.Contains('ChainRounds advances the same in-memory text across rounds') -and $dispatchText.Contains('ChainRounds 在轮次间继续推进同一内存文本')
...
$taskSafetyHasCrossRoundTransaction = $taskDefinitionRepairTransactionText.Contains('[string]$ValidateThroughRound') -and $taskDefinitionRepairTransactionText.Contains('[switch]$ChainRounds') -and $taskDefinitionRepairTransactionText.Contains('chain_rounds = $ChainRounds.IsPresent') -and $taskDefinitionRepairTransactionText.Contains('validated_rounds = @($roundSequence)') -and $taskDefinitionRepairTransactionText.Contains('validated_rounds = @($manifest.validated_rounds)') -and $recoveryTransactionText.Contains('task-static recovery validated round coverage gate failed') -and $recoveryTransactionText.Contains('task-static recovery receipt round coverage gate failed') -and $recoveryTransactionText.Contains("`$checkerArguments += '-ChainRounds'") -and $takeoverTriggerText.Contains('repair_validate_through_round={0}') -and $takeoverTriggerText.Contains('manifest/receipt validated_rounds must cover that exact sequence; Promote exactly once') -and $pollText.Contains('repair_validate_through_round = if ($taskStaticCrossRoundRepairEnabled') -and $dispatchText.Contains('[Cross-round transaction execution]') -and $dispatchText.Contains('[跨轮事务执行]') -and $dispatchText.Contains('Use the complete task_definition_*_command values from the brief') -and $dispatchText.Contains('Prepare one candidate with -ValidateThroughRound D4 -ChainRounds') -and $operationFlowText.Contains('Validate -ValidateThroughRound D4 -ChainRounds') -and $startTemplateText.Contains('Validate -ValidateThroughRound D4 -ChainRounds') -and $promptDocText.Contains('Validate -ValidateThroughRound D4 -ChainRounds') -and $copilotInstructionsText.Contains('Validate -ValidateThroughRound D4 -ChainRounds')
...
$taskSafetyChecks = [ordered]@{
    focused_en = $taskSafetyHasFocusedLimitEn
    focused_zh = $taskSafetyHasFocusedLimitZh
    switch_default = $taskSafetyHasSwitchDefault
    brief_switch = $taskSafetyBriefProjectsSwitch
    dispatch_branches = $taskSafetyDispatchBranches
    fail_fast = $taskSafetyHasFailFast
    assertion_boundary = $taskSafetyHasAssertionBoundary
    repair_transaction = $taskSafetyHasRepairTransaction
    diagnosis_integrity = $taskSafetyHasDiagnosisIntegrity
    hard_block_notice = $taskSafetyHasHardBlockNotice
    suffix_attached = $taskSafetySuffixAttached
    phase_boundary = $taskSafetyHasPhaseBoundary
    retry_scope = $taskSafetyHasRetryScope
    cross_round_transaction = $taskSafetyHasCrossRoundTransaction
}
$taskSafetyMissing = @($taskSafetyChecks.GetEnumerator() | Where-Object { -not [bool]$_.Value } | ForEach-Object { [string]$_.Key })
$taskSafetyReason = if ($taskSafetyPass) { 'task-definition-progressive-static-check-contract-present' } else { 'missing-task-definition-progressive-static-check-contract:' + ($taskSafetyMissing -join ',') }
...
$briefRequiresTaskDefinitionPromotion = $takeoverTriggerText.Contains('task_definition_promotion_gate={0}') -and $takeoverTriggerText.Contains('require same-ticket promoted manifest and successful receipt') -and $takeoverTriggerText.Contains('validated/promoted/official hashes must match') -and $takeoverTriggerText.Contains('manifest/receipt validated_rounds must cover that exact sequence') -and $takeoverTriggerText.Contains('only after task_definition_promotion_gate passes')
