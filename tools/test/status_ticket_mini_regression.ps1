...
$taskSafetyBriefProjectsSwitch = $takeoverTriggerText.Contains('task_static_cross_round_repair_enabled={0}') -and $takeoverTriggerText.Contains('check only the current failing D round before restart; later rounds remain runtime-gated') -and $takeoverTriggerText.Contains('after the failing round passes, check and repair each later D round through D4 in order before restart')
$taskSafetyDispatchBranches = $dispatchText.Contains('[Cross-round repair enabled] For task-static faults, keep one transaction candidate') -and $dispatchText.Contains('[Cross-round repair disabled] For task-static faults and compile/verify faults classified as code faults') -and $dispatchText.Contains('[跨轮次修复已开启] 对 task-static 故障，在同一事务 candidate 中') -and $dispatchText.Contains('[跨轮次修复已关闭] 对 task-static 故障，以及经分类确认为代码故障的编译/验证故障') -and $dispatchText.Contains('$taskStaticCrossRoundRepairEnabled = $briefTaskStaticCrossRoundRepairEnabled') -and $dispatchText.Contains('$taskDefinitionFixMessage -f $TicketId, $TicketEvent, $dispatchReadContextText, '''', $crossRoundRepairStatus') -and -not $dispatchText.Contains('$crossRoundRepairStatus, $firstMessage.TrimStart()')
