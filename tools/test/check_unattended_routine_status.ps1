...
$script:UnhandledExitTag = 'CHECK-UNATTENDED-ROUTINE-STATUS'
    ...
    commands = [ordered]@{
        ...
        selected_ticket = if ($null -ne $firstRow) {
            [ordered]@{
                ...
                route_guard_command = Convert-ToSingleLineText -Text ([string]$firstRow.route_guard_command)
                next_command_order = @($firstRow.next_command_order)
                atomic_closeout_command = Convert-ToSingleLineText -Text ([string]$firstRow.atomic_closeout_command)
                ticket_closure_check_command = Convert-ToSingleLineText -Text ([string]$firstRow.ticket_closure_check_command)
                event_dedup_health_check_command = Convert-ToSingleLineText -Text ([string]$firstRow.event_dedup_health_check_command)
                final_status_closeout_command = Convert-ToSingleLineText -Text ([string]$firstRow.final_status_closeout_command)
                final_status_closeout_apply_ack_command = Convert-ToSingleLineText -Text ([string]$firstRow.final_status_closeout_apply_ack_command)
...
if ($AsJson.IsPresent) {
    ...
    if ($null -ne $output.commands.selected_ticket) {
        ...
        Write-Output ('  route_guard_command={0}' -f [string]$output.commands.selected_ticket.route_guard_command)
        Write-Output ('  next_command_order={0}' -f [string](@($output.commands.selected_ticket.next_command_order) -join '|'))
        Write-Output ('  atomic_closeout_command={0}' -f [string]$output.commands.selected_ticket.atomic_closeout_command)
        Write-Output ('  ticket_closure_check_command={0}' -f [string]$output.commands.selected_ticket.ticket_closure_check_command)
        Write-Output ('  event_dedup_health_check_command={0}' -f [string]$output.commands.selected_ticket.event_dedup_health_check_command)
        Write-Output ('  final_status_closeout_command={0}' -f [string]$output.commands.selected_ticket.final_status_closeout_command)
        Write-Output ('  final_status_closeout_apply_ack_command={0}' -f [string]$output.commands.selected_ticket.final_status_closeout_apply_ack_command)
