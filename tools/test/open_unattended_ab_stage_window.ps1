...
function Add-StageTaskDefinitionBlockedTicket {
    param(
        ...
        [AllowEmptyString()][string]$HardBlockReason = '',
        [AllowEmptyString()][string]$FailureFingerprint = '',
        [int]$RetryCount = 0,
        [int]$RetryLimit = 0
    )
    ...
    $recommendedAction = 'Report root cause and remediation path first. Task-definition static precheck has exceeded the allowed retry limit, so do not continue automatic stage restarts. Review the task-definition file under testdata, decide whether to repair or reset baseline, rerun task static precheck manually, and only then resume the guarded workflow. Execute every ticket step without omission, then execute atomic_closeout_command exactly once and claim closure only from its successful machine facts. The 3-minute closeout window is a target, not an agent-enforced transaction timeout. Wait for recovery_transaction_command to exit naturally; never kill, Stop-Process, terminate its terminal, or cancel it merely because it exceeds 3 minutes or 240 seconds. Then wait silently for the next ticket; never run scheduled, persistent, or long-running cross-round monitoring commands.'
    ...
    switch ((Convert-ToSingleLineText -Text $BlockEvent).ToLowerInvariant()) {
        'recovery-await-confirmation' {
            ...
            $recommendedAction = 'Report root cause and remediation path first. Task-definition static precheck has exceeded the allowed retry limit, so pause automatic retries and require explicit restart approval after evidence review. Repair or reset the task-definition file under testdata, rerun task static precheck manually, and only then resume the guarded workflow. Execute every ticket step without omission, then execute atomic_closeout_command exactly once and claim closure only from its successful machine facts. The 3-minute closeout window is a target, not an agent-enforced transaction timeout. Wait for recovery_transaction_command to exit naturally; never kill, Stop-Process, terminate its terminal, or cancel it merely because it exceeds 3 minutes or 240 seconds. Then wait silently for the next ticket; never run scheduled, persistent, or long-running cross-round monitoring commands.'
        }
        'task-definition-fix-required' {
            ...
            $confirmationKey = ''
            $selfHealable = $true
            $recommendedAction = 'Report root cause and remediation path first. Task-definition static precheck has exceeded the allowed retry limit, so do not restart automatically; instead repair the task-definition file under testdata until static precheck passes, then resume via the standard business_resume path. Execute every ticket step without omission, then execute atomic_closeout_command exactly once and claim closure only from its successful machine facts. The 3-minute closeout window is a target, not an agent-enforced transaction timeout. Wait for recovery_transaction_command to exit naturally; never kill, Stop-Process, terminate its terminal, or cancel it merely because it exceeds 3 minutes or 240 seconds. Then wait silently for the next ticket; never run scheduled, persistent, or long-running cross-round monitoring commands.'
        }
    }
    ...
    $ticket = [ordered]@{
        ...
        hard_block_reason = (Convert-ToSingleLineText -Text $HardBlockReason)
        failure_fingerprint = (Convert-ToSingleLineText -Text $FailureFingerprint)
        retry_count = $RetryCount
        retry_limit = $RetryLimit
        auto_restart_allowed = (-not $HardBlock)
        task_definition = (Convert-ToAnchorPath -Path $TaskDefinitionRelative)
...
if ($Stage -eq 'A') {
    ...
    $aFailureCodeFault = if ($settings.Contains('A_FAILURE_CODE_FAULT')) { Convert-ToBooleanSetting -Value ([string]$settings.A_FAILURE_CODE_FAULT) -Default $false } else { $false }

    $aRetryGrantKey = 'A_CODEFIX_IDENTICAL_FP_RETRY_GRANTED_FOR'
    $aRetryGrantAtKey = 'A_CODEFIX_IDENTICAL_FP_RETRY_GRANTED_AT'
    $aRetryCountKey = 'A_CODEFIX_IDENTICAL_FP_RETRY_COUNT'
    $aRetryStateKey = 'A_CODEFIX_IDENTICAL_FP_STATE'
    $aRetryStateAtKey = 'A_CODEFIX_IDENTICAL_FP_STATE_AT'
    $aRetryMax = 3
    if ($settings.Contains('CODEFIX_IDENTICAL_FP_MAX_RETRIES')) {
        $parsedGlobalRetryMax = Get-ParsedPositiveInt -Value ([string]$settings.CODEFIX_IDENTICAL_FP_MAX_RETRIES)
        if ($parsedGlobalRetryMax -gt 0) { $aRetryMax = $parsedGlobalRetryMax }
    }
    if ($settings.Contains('A_CODEFIX_IDENTICAL_FP_MAX_RETRIES')) {
        $parsedStageRetryMax = Get-ParsedPositiveInt -Value ([string]$settings.A_CODEFIX_IDENTICAL_FP_MAX_RETRIES)
    ...
    if (-not [string]::IsNullOrWhiteSpace($aFailureMainRound) -and
        ...
        $aFailureFingerprint -eq $aPreviousFailureFingerprint -and
        $aFailurePhase -in @('compile', 'verify') -and
        $aFailureCodeFault) {
        ...
        if ($aCurrentState -eq 'hard_block' -and $aHasRepairEvidence) {
        ...
        if ($aRetryCount -lt $aRetryMax) {
            ...
            throw ("[OPEN-AB-STAGE] infinite-loop-protection: A identical code-fix fingerprint detected (main_round={0}, phase={1}, task_start_at={2}, fingerprint={3}). Retry granted {4}/{5}; apply code fix and relaunch." -f $aFailureMainRound, $aFailurePhase, $aFailureTaskStartAt, $aFailureFingerprint, $aNextAttempt, $aRetryMax)
        }
        ...
        Add-StageTaskDefinitionBlockedTicket -StartFilePath $startFilePath -Settings $settings -Stage $Stage -TaskDefinitionRelative $taskDefinitionRelative -FailCount ($aRetryCount + 1) -MaxFails $aRetryMax -PrecheckExitCode 0 -BlockEvent 'manual-wait-paused' -ExtraDetail ("fingerprint_duplicate=true retry_budget_exhausted=true retry_count={0} retry_limit={1} round={2} phase={3} task_start_at={4} fingerprint={5}" -f $aRetryCount, $aRetryMax, $aFailureMainRound, $aFailurePhase, $aFailureTaskStartAt, $aFailureFingerprint) -RecommendedActionOverride 'Code-fix fingerprint repeated after retry budget exhaustion. Change the repair strategy and produce validated repair evidence before relaunch.' -FailureCategoryOverride 'fingerprint-duplicate-manual-escalation' -FailureKindOverride 'fingerprint-duplicate-retry-exhausted' -HardBlock $true -HardBlockReason 'retry-budget-exhausted' -FailureFingerprint $aFailureFingerprint -RetryCount $aRetryCount -RetryLimit $aRetryMax
...
if ($Stage -eq 'B') {
    ...
    $bFailureCodeFault = if ($settings.Contains('B_FAILURE_CODE_FAULT')) { Convert-ToBooleanSetting -Value ([string]$settings.B_FAILURE_CODE_FAULT) -Default $false } else { $false }

    $bRetryGrantKey = 'B_CODEFIX_IDENTICAL_FP_RETRY_GRANTED_FOR'
    $bRetryGrantAtKey = 'B_CODEFIX_IDENTICAL_FP_RETRY_GRANTED_AT'
    $bRetryCountKey = 'B_CODEFIX_IDENTICAL_FP_RETRY_COUNT'
    $bRetryStateKey = 'B_CODEFIX_IDENTICAL_FP_STATE'
    $bRetryStateAtKey = 'B_CODEFIX_IDENTICAL_FP_STATE_AT'
    $bRetryMax = 3
    if ($settings.Contains('CODEFIX_IDENTICAL_FP_MAX_RETRIES')) {
        $parsedGlobalRetryMax = Get-ParsedPositiveInt -Value ([string]$settings.CODEFIX_IDENTICAL_FP_MAX_RETRIES)
        if ($parsedGlobalRetryMax -gt 0) { $bRetryMax = $parsedGlobalRetryMax }
    }
    if ($settings.Contains('B_CODEFIX_IDENTICAL_FP_MAX_RETRIES')) {
        $parsedStageRetryMax = Get-ParsedPositiveInt -Value ([string]$settings.B_CODEFIX_IDENTICAL_FP_MAX_RETRIES)
    ...
    if (-not [string]::IsNullOrWhiteSpace($bFailureMainRound) -and
        ...
        $bFailureFingerprint -eq $bPreviousFailureFingerprint -and
        $bFailurePhase -in @('compile', 'verify') -and
        $bFailureCodeFault) {
        ...
        if ($bCurrentState -eq 'hard_block' -and $bHasRepairEvidence) {
        ...
        if ($bRetryCount -lt $bRetryMax) {
            ...
            if ($bNextAttempt -gt 1 -and -not $bHasRepairEvidence) {
                ...
                Add-StageTaskDefinitionBlockedTicket -StartFilePath $startFilePath -Settings $settings -Stage $Stage -TaskDefinitionRelative $taskDefinitionRelative -FailCount $bNextAttempt -MaxFails $bRetryMax -PrecheckExitCode 0 -BlockEvent 'manual-wait-paused' -ExtraDetail ("fingerprint_duplicate=true retry_budget_exhausted=true retry_requires_evidence=true retry_count={0} retry_limit={1} round={2} phase={3} task_start_at={4} fingerprint={5}" -f $bRetryCount, $bRetryMax, $bFailureMainRound, $bFailurePhase, $bFailureTaskStartAt, $bFailureFingerprint) -RecommendedActionOverride 'Identical fingerprint retried without effective repair evidence. AI must change repair method (not same patch shape), adjust round operations/anchors, and produce evidence-changing edits before relaunch. Stop auto-retry, perform manual task-definition fix, pass static check, then relaunch.' -FailureCategoryOverride 'fingerprint-duplicate-manual-escalation' -FailureKindOverride 'fingerprint-duplicate-evidence-missing' -HardBlock $true -HardBlockReason 'repair-evidence-missing' -FailureFingerprint $bFailureFingerprint -RetryCount $bRetryCount -RetryLimit $bRetryMax
                throw ("[OPEN-AB-STAGE] infinite-loop-protection: B identical code-fix fingerprint detected without repair evidence (round={0}, phase={1}, task_start_at={2}, fingerprint={3}). Manual intervention required." -f $bFailureMainRound, $bFailurePhase, $bFailureTaskStartAt, $bFailureFingerprint)
            }
            ...
            throw ("[OPEN-AB-STAGE] infinite-loop-protection: B identical code-fix fingerprint detected (main_round={0}, phase={1}, task_start_at={2}, fingerprint={3}). Retry granted {4}/{5}; apply code fix and relaunch." -f $bFailureMainRound, $bFailurePhase, $bFailureTaskStartAt, $bFailureFingerprint, $bNextAttempt, $bRetryMax)
        }
        ...
        Add-StageTaskDefinitionBlockedTicket -StartFilePath $startFilePath -Settings $settings -Stage $Stage -TaskDefinitionRelative $taskDefinitionRelative -FailCount ($bRetryCount + 1) -MaxFails $bRetryMax -PrecheckExitCode 0 -BlockEvent 'manual-wait-paused' -ExtraDetail ("fingerprint_duplicate=true retry_budget_exhausted=true retry_count={0} retry_limit={1} round={2} phase={3} task_start_at={4} fingerprint={5}" -f $bRetryCount, $bRetryMax, $bFailureMainRound, $bFailurePhase, $bFailureTaskStartAt, $bFailureFingerprint) -RecommendedActionOverride 'Code-fix fingerprint repeated after retry budget exhaustion. Change the repair strategy and produce validated repair evidence before relaunch.' -FailureCategoryOverride 'fingerprint-duplicate-manual-escalation' -FailureKindOverride 'fingerprint-duplicate-retry-exhausted' -HardBlock $true -HardBlockReason 'retry-budget-exhausted' -FailureFingerprint $bFailureFingerprint -RetryCount $bRetryCount -RetryLimit $bRetryMax
