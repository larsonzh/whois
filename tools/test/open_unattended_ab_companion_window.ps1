...
try {
    ...
    if ($existingPids.Count -gt 0) {
        if ($NoRestartIfRunning.IsPresent) {
            $probeRoot = Join-Path $repoRoot 'out\artifacts\ab_companion'
            $probeDir = Get-LatestTimestampedDirectory -Root $probeRoot
            $probePaths = @()
            if ($null -ne $probeDir) {
                $probePaths += Join-Path $probeDir.FullName 'companion.log'
            }
            ...
            if ($reuseAlive) {
                Write-Output ("[OPEN-AB-COMPANION] restart_precheck existing_count={0} existing_pids={1} mode=reuse stale_min={2}" -f $existingPids.Count, ($existingPids -join ','), $monitorReuseStaleMinutes)
                $reuseExisting = $true
                $processId = [int]$existingPids[0]
            }
            else {
                Write-Output ("[OPEN-AB-COMPANION] restart_precheck existing_count={0} existing_pids={1} mode=restart-stale stale_min={2}" -f $existingPids.Count, ($existingPids -join ','), $monitorReuseStaleMinutes)
