...
function Get-StartFileState {
    ...
    $lines = @(Get-Content -LiteralPath $StartFilePath -ErrorAction Stop)
    $lineIndex = @{}
    $orderedKeys = New-Object 'System.Collections.Generic.List[string]'
    $map = [ordered]@{}
...
$defaultSelectorText = 'PRECHECK_*;NETWORK_PRECHECK_LAST_RESULT;NETWORK_PRECHECK_LAST_AT;NETWORK_PRECHECK_LAST_REASON;A_SUCCESS_SNAPSHOT_FINAL_STATUS;A_SUCCESS_SNAPSHOT_SUMMARY;A_SUCCESS_SNAPSHOT_SOURCE_STATE;A_FINAL_STATUS;B_FINAL_STATUS;SESSION_FINAL_STATUS;SESSION_FINAL_NOTES;AI_SESSION_BLOCKING_WATCH_NOTES;RESTART_EVIDENCE_NOTES'
