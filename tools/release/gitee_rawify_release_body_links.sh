...
#
# 用法：
#   ./tools/release/gitee_rawify_release_body_links.sh [-t v3.2.10] [-o owner] [-p repo] [-n] <file...>
# 说明：
...
if [[ -z "$tag" && $# -eq 1 ]]; then
  ...
  if [[ -n "$inferred_tag" ]]; then
    tag="$inferred_tag"
    echo "[gitee-rawify] inferred tag: $tag (from ${bn_candidate:+filename/content})"
  fi
fi
...
for f in "$@"; do
  ...
  for a in "${assets[@]}"; do
    if [[ $dry_run -eq 1 ]]; then
      ...
      echo "[gitee-rawify][dry-run] ${f}: ${a} (rel=${cnt_rel}, gh=${cnt_gh}) -> ${base}/${a}"
    else
      # 相对路径 -> Gitee raw
      sed -E -i "s#\]\(release/lzispro/whois/${a}\)#](${base}/${a})#g" "$tmp"
      # GitHub Release 直链 -> Gitee raw
  ...
  if [[ $dry_run -eq 1 ]]; then
    rm -f "$tmp"
    echo "[gitee-rawify] 预览完成（未改动）: $f"
  else
    mv "$tmp" "$f"
    echo "[gitee-rawify] 已处理: $f"
  fi
done

echo "[gitee-rawify] 完成：tag=${tag}, repo=${owner}/${repo}"
