...
BINDING_SENTENCE=进入事件驱动监控，后续动作由定时状态票/事件票指令触发，并仅执行既有仓库命令（禁止自建额外脚本或循环）。
