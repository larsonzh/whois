...
struct Config {
	...
	const char* preclass_action_list; // Step P1 action candidate query list (CSV)
	...
	int preclass_first_hop_enable;   // Enable Phase B classifier-preferred first hop (default on)
	int preclass_early_converge_enable; // Enable Phase C reserved/special early converge (default on)
