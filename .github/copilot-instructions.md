...
- **新建任务定义文件格式（硬规则）**：任何新建的正式 `testdata/autopilot_code_step_tasks_*.json` 在创建落盘时必须使用 `UTF-8 with BOM` 编码和 `LF` 行尾；不得保留 UTF-8 无 BOM、ANSI、UTF-16 或 CRLF。模板复制后、JSON 语义编辑前应立即完成该机械规范化，并在生成 start-file 前运行 `tools/dev/enforce_utf8_bom_lf.ps1` 编码门禁确认。编码/EOL 规范化不得改变 JSON 值、数组顺序或 operation 结构，JSON 语义修改仍必须使用 `apply_patch`。
...
- **可选远程快速编译（初始编制）**：本地全定义静态检查和 effective-C 语法门禁通过后，若任务涉及多个 C source/header、公共声明或调用链、平台条件编译、链接符号、新文件接入，或本地/远端工具链差异，AI 可运行 `tools/test/check_task_definition_remote_quick.ps1` 获取早期编译反馈。单定义使用 `-TaskDefinitionFiles "testdata/<TASK>.json"`，A/B 链按顺序使用一个逗号分隔值 `-TaskDefinitionFiles "testdata/<A>.json,testdata/<B>.json"`。脚本必须在 `tmp/` 隔离副本中顺序执行 D1-D4 checker artifact + code-step，默认只远程编译 `x86_64 win64`，不得修改主工作树；禁止临时拼接 effective-tree 或远程构建长命令。该操作按复杂度可选，不替代完整九架构 Strict、Golden/referral、Step47/preclass 和任务专项验证；跳过时应能说明任务简单或已有等价完整远程证据。
