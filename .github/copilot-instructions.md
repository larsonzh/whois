...
- **运行期被动收票（硬规则）**：A/B 无人值守运行期间，guard/trigger/dispatch 负责监控、产票和向会话投送消息。Agent 只需保持会话在线并静默等待事件驱动票或状态票；等待本身不执行任何命令。收到工单后，按工单指令与 `next_command_order` 执行其中无需用户确认的预授权操作并回执，然后继续静默等待。禁止 Agent 自行创建或运行任何定时巡检监控脚本、轮询循环、后台 job、watcher、常驻 PowerShell 命令，禁止自行周期性调用 heartbeat 或 `poll_agent_tickets.ps1`。一次性执行工单明确给出的 poll/heartbeat/回执命令不受此限制。
