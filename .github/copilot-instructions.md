...
## 7. 交流与输出语言约定
- 日常对话/解释：**使用中文** 与用户沟通。
- 代码与注释：**统一使用英文**，保持与现有代码库风格一致。
- Git 提交信息：**使用英文**，方便后续审阅与跨团队协作。

## 8. 构建与日终同步流程
- 本地无 `make` 环境，如需构建/冒烟测试，需先通知用户由其运行远程脚本 `tools/remote/remote_build_and_test.sh`（含 Golden 检查）。
- 每次远程冒烟测试通过后：
  - 立即在 `docs/RFC-whois-client-split.md` 中补充当次改动与测试结论；
  - 提出建议的英文提交信息，并提醒用户适时提交/推送代码与文档。
- 每当用户宣布当天工作结束时，需在 RFC 备忘录中记录：
  - 当日完成的具体内容；
  - 下一步计划/待办，确保次日启动 VS Code 时即便聊天记录丢失也能从 RFC 获得上下文。
