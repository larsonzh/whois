...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-16 ~ 2027-01-22，串行第 35 份，Checklist A，草案）**：
...
**八轮通用约束（开跑前确认）**：
1. [ ] 串行约束：仅在第 34 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
...
3. [ ] D3：Remove the unknown-hint wrapper helpers `wc_preclass_v6_unknown_hint_required`, `wc_preclass_unknown_hint_default_reason_literal` and `wc_preclass_apply_v4_unknown_hint_if_needed`, routing the v4 path through the public `wc_preclass_apply_allocated_hint_if_unknown_class`.
4. [ ] D4：Remove the rir-unknown compare wrapper literal `wc_preclass_rir_unknown_compare_literal` by routing its three call sites directly through the base `wc_preclass_rir_unknown_literal`.
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-23 ~ 2027-01-31，串行第 36 份，Checklist B，草案）**：
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [ ] D1：Finish the match-layer literal dedup by routing `wc_preclass_match_layer_from_query_kind` directly through the compare literals and removing `wc_preclass_match_layer_cidr_literal`/`wc_preclass_match_layer_ip_literal` definitions and forward declarations.
2. [ ] D2：Remove the pure-forward `wc_preclass_set_unknown_hint_tuple` by routing its two call sites directly through `wc_preclass_apply_classified_tuple`, and remove `wc_preclass_match_layer_from_query_kind_normalized` by routing its single call site to the direct `query_is_cidr` condition.
3. [ ] D3：Inline the special-result tuple setter for the 8 v4 call sites in `wc_preclass_classify_ip` directly through `wc_preclass_apply_none_confidence_tuple` with the class-special literal.
4. [ ] D4：Inline the special-result tuple setter for the 5 v6 call sites and remove the now-unused `wc_preclass_set_special_tuple` definition.
...
- 当前策略基线：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`、`AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`。
- 预检基线：（待生成启动文件后由 launch-ready 预检回填 `PRECHECK_*`；A 阶段 launch-ready DryRun 待预检）。
