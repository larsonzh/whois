...
- 2026-02-20：CIDR 正文契约烟测扩展：`tools/test/cidr_body_contract_smoke.ps1` 增补为 4 个稳定 case（first-marker 抑制、post-marker 一致性路径、no-marker 直达、IANA 链路尾行存在），本地复跑 `pass=4 fail=0`，日志 `out/artifacts/cidr_body_contract/20260220-124943`；VS Code 任务已提供 `Test: CIDR Body Contract` 与 `Test: CIDR Body Contract (prefilled)`。
- 2026-02-20：CIDR 草案矩阵扩展：`testdata/cidr_matrix_cases_draft.tsv` 新增 no-marker 跨 RIR、全空间 unknown 与 invalid-input 三类可执行用例；定向执行 `redirect_matrix_test.ps1 -CasesFile testdata/cidr_matrix_cases_draft.tsv` 结果 `pass=8 fail=0`，日志 `out/artifacts/redirect_matrix/20260220-125430`。VS Code 已新增一键任务 `Test: Redirect Matrix (CIDR Draft TSV)` 与 `Test: Redirect Matrix (CIDR Draft TSV, prefilled)`，按任务等价命令复跑同样 `pass=8 fail=0`（`out/artifacts/redirect_matrix/20260220-130044`）。
