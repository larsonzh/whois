...
- 本文中的旧“下一步”“下次开工”和未勾选模板均为历史过程记录；除下方“当前维护状态”明确列出的条目外，不代表当前待办。

**当前维护状态（唯一状态入口）**：

- 2026-09-03 v3.3.3 发布准备：源码版本号已置为 3.3.3（`src/core/meta.c`、`src/whois_client.c`，提交 `0202efc6`），创建 `docs/release_bodies/v3.3.3.md`，并同步 README 标题/发布列表/速览、RELEASE_NOTES 3.3.3 段与 `docs/RFC-post-3.3.0-development-plan.md`（WP-12 状态、12.3 发布顺序）。正式 release 制品由 One-Click Release 以 `WHOIS_FORCE_VERSION=v3.3.3` 强制重建；当前 `release/lzispro/whois` 为同步中间态，不作为发布产物。
...
- 2026-08-25 WP-09 已完成：修复 title 与 grep line 模式在共享 workbuf 向前压缩时的五处重叠 `memcpy`，并建立 native Linux GCC ASan/UBSan 确定性门禁（64 个正向场景与受控负例 PASS）。最终 Strict Version `lto-auto` 默认轮无编译/LTO 告警，九架构 build/hash、Golden、三起点 referral、Linux/QEMU `18` 组与 Windows `6` 组查询冒烟及双目录 release sync 全 PASS（`out/artifacts/20260825-171420`，285s）；x86_64/win32/win64 聚焦 grep selftest `9/9` PASS（`out/artifacts/20260825-165446`）。
- 2026-08-28 v3.3.2 发布候选验证已完成：Strict `lto-auto` 九架构无编译/LTO 告警、9/9 SHA 一致、Golden 与三起点 referral PASS（`out/artifacts/20260828-081629`，212s）；build+sync dry-run `exit_code=0`（`20260828-061931`，含 Step47 preclass preflight）、D6 双轮 `RoundPass=True`（`20260828-065758`）、CIDR `4/4+9/9`、Redirect Matrix `authMismatch=0 error=0`（`20260828-075458`）、Step47 预发布 7/7（`20260828-080030`）、写盘核心守卫 `pass=True` 全部通过；IANA 快照刷新无上游变化。该轮产物为中间版本串（`v3.3.1-9-g23d061d3-dirty`），正式 v3.3.2 产物由提交后的 One-Click Release 强制重建。
- 2026-08-25 WP-10 已完成：确认 `--buffer-size` 已直接控制主查询实际接收上限，新增 `--max-bytes` 没有独立语义；native Linux GCC 的 3 个真实接收案例与 IPv4/IPv6/CIDR/批量 4 个分类案例全部 PASS，实证当前上限耗尽会静默成功并造成权威、referral、ERX/IANA、拒绝访问分类漂移（`out/artifacts/response_limit_audit/20260825-183151`）。决定不新增公开 CLI、不改变网络行为；截断可观测与 fail-close 作为独立内部 bugfix 候选另行评审。WP-08/WP-09 轻量复审未发现 WP-11 所需用户证据，WP-11 保持 proposed。
- 核心模块局部治理与性能优化不设常驻优先级或排期；仅在测试覆盖和可复现实测基线证明收益后，以新事项启动。

**快速索引（轻整理，摘要版）**：
- 2026-09-01：串行 WP-13C SOCKS4/4a“无人值守 A/B”已完成执行回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`（A run=`out/artifacts/dev_verify_multiround/20260831-150933`，B run=`out/artifacts/dev_verify_multiround/20260831-205121`，A/B 合计 `0d 12:01:40`，session start=2026-08-31 15:08:58）；A/B 均 8/8 一轮通过、全程无事故/自愈/重启；事件票 `T20260831-205129509-bb7b885e`（handled_at 2026-08-31 20:54:38）、`chat-final-20260901-031037`（handled_at 2026-09-01 03:12:26）闭环；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260901-121843`，755s；Local hash verify/Golden/referral 全 PASS）（详见 `docs/RFC-proxy-access.md` 13.x 执行回填）。
...
- 2026-08-31：WP-13B-2 收尾编辑器 IntelliSense 兼容修正：`src/core/proxy.c` `wc_proxy_selftest` 的 `UINT64_MAX` 在 VS Code C/C++ IntelliSense 下报 code 19（“编号的预期结尾后有多余文本”），改为自测局部 `no_deadline = ~(uint64_t)0`（数值等价、不依赖实现特定后缀）；真实 GCC/Clang 与远程九架构构建不受影响，生产行为零变化。修正后 Strict Version `lto-auto` 复核无告警 + Local hash verify/Golden/referral 全 PASS（`out/artifacts/20260831-095325`，270s；本地 9/9 SHA-256 与 `SHA256SUMS-static.txt` 一致）。
...
- 2026-08-10：落地实验性 `schemaVersion=vx-draft` 多文件任务定义链：共享 target registry、target-bound task-static/replay/assertion、`TASK_STATIC_VALIDATED_ARTIFACT_VX1`、code-step journal/整组回滚/reset、wrapper 全目标 source-delta、repair preview/Validate/Promote target-set 绑定及独立 Vx 模板；V1 单文件回归保持通过。真实无人值守 A/B 启用仍需具体 TODO-free 定义完成完整 launch-ready、编译、黄金与 Step47 验收。
- 2026-08-10：串行第 41/42 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2027-03-02 ~ 2027-03-15`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260809-114503，B run=out/artifacts/dev_verify_multiround/20260809-201456）；全程 0 起事故、无自愈、无重启，一次通过；A 阶段于 2026-08-09 20:15:07 完成（0d 08:30:42），B 阶段于 2026-08-10 02:52:12 完成（0d 06:37:48），会话结束 2026-08-10 02:52:12；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260810-053425`，212s）（详见文内第 41/42 份清单段与启动文件段）。
- 2026-08-09：新增串行第 41/42 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2027-03-02 ~ 2027-03-15`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20270302_20270308.json`、`testdata/autopilot_code_step_tasks_20270309_20270315.json`）与 active 启动文件（`testdata/unattended_start/active/unattended_ab_start_20270302-20270315.md`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、链式有效源码编译验证均通过（详见文内第 41/42 份清单段与启动文件段）。
- 2026-08-09：串行第 39/40 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2027-02-16 ~ 2027-03-01`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260808-064045，B run=out/artifacts/dev_verify_multiround/20260809-004856）；全程 0 起事故、无自愈、无重启，一次通过；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260809-090641`，279s）（详见文内第 39/40 份回填段与启动文件段）。
- 2026-08-08：新增串行第 39/40 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2027-02-16 ~ 2027-03-01`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20270216_20270222.json`、`testdata/autopilot_code_step_tasks_20270223_20270301.json`）与 active 启动文件（`testdata/unattended_start/active/unattended_ab_start_20270216-20270301.md`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、专项安全回归与有效源码编译验证均通过（详见文内第 39/40 份清单段与启动文件段）。
- 2026-08-08：串行第 37/38 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2027-02-01 ~ 2027-02-15`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260807-135744，B run=out/artifacts/dev_verify_multiround/20260807-193345）；全程 0 起事故、无自愈、无重启，一次通过；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260808-053433`，240s）（详见文内第 37/38 份回填段与启动文件段）。
- 2026-08-07：新增串行第 37/38 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2027-02-01 ~ 2027-02-15`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20270201_20270207.json`、`testdata/autopilot_code_step_tasks_20270208_20270215.json`）与 active 启动文件（`testdata/unattended_start/active/unattended_ab_start_20270201-20270215.md`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、专项安全回归与有效源码编译验证均通过（详见文内第 37/38 份清单段与启动文件段）。
- 2026-08-07：串行第 35/36 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2027-01-16 ~ 2027-01-31`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260807-011902，B run=out/artifacts/dev_verify_multiround/20260807-061242）；全程 0 起事故、无自愈、无重启，一次通过；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260807-121102`，289s）（详见文内第 35/36 份回填段与启动文件段）。
- 2026-08-07：新增串行第 35/36 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2027-01-16 ~ 2027-01-31`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20270116_20270122.json`、`testdata/autopilot_code_step_tasks_20270123_20270131.json`）与 active 启动文件（`testdata/unattended_start/active/unattended_ab_start_20270116-20270131.md`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、专项安全回归与有效源码编译验证均通过（详见文内第 35/36 份清单段与启动文件段）。
- 2026-08-07：串行第 33/34 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2027-01-01 ~ 2027-01-15`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260806-065714，B run=out/artifacts/dev_verify_multiround/20260806-120749）；全程 0 起事故、无自愈、无重启，一次通过；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260807-001732`，293s）（详见文内第 33/34 份回填段与启动文件段）。
- 2026-08-06：新增串行第 33/34 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2027-01-01 ~ 2027-01-15`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20270101_20270107.json`、`testdata/autopilot_code_step_tasks_20270108_20270115.json`）与 active 启动文件（`testdata/unattended_start/active/unattended_ab_start_20270101-20270115.md`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、专项安全回归与有效源码编译验证均通过（详见文内第 33/34 份清单段与启动文件段）。
- 2026-08-06：串行第 31/32 份“无人值守超高密度 A/B”已完成回填：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`；窗口 `2026-12-16 ~ 2026-12-31`，A/B 各 8/8 轮通过（A run=out/artifacts/dev_verify_multiround/20260805-161948，B run=out/artifacts/dev_verify_multiround/20260805-220426）；全程 0 起事故、无自愈、无重启，一次通过；最终 Strict 远程构建冒烟同步 + 黄金校验（`lto-auto`）无告警通过（`out/artifacts/20260806-050216`，314s）（详见文内第 31/32 份回填段与启动文件段）。
- 2026-08-05：新增串行第 31/32 份“无人值守超高密度 A/B 下次开工清单（草案）”（窗口 `2026-12-16 ~ 2026-12-31`），并同步起草配套任务定义（`testdata/autopilot_code_step_tasks_20261216_20261222.json`、`testdata/autopilot_code_step_tasks_20261223_20261231.json`）；A 全定义静态验收、B 以 A 为前置的链式静态验收、专项安全回归与有效源码编译验证均通过（详见文内第 31/32 份清单段与启动文件段）。
...
- 2026-07-23：retry-budget minimal regression 复用一次 session-floor seed，并通过 ack-only fast path 覆盖 yes/missing/no，移除每 case 重复 select；poll 子进程从 9 次降为 4 次，本机实测由 92.2 秒降至 46.9 秒，ledger 结果契约不变。
- 2026-07-23：恢复事务时限语义跨层归一：3 分钟只作为收尾目标，不再表述为 Agent 可执行的事务总墙钟上限；代理指令、流程/提示词/启动模板、guard/stage-window/dispatch 提示和 takeover brief 均要求 `recovery_transaction_command` 启动后等待自然退出，禁止按 3 分钟或 240 秒主动终止事务。脚本内部 stage 启动验证继续使用 240 秒预算，atomic closeout acknowledge 保持 120 秒；完整票据回归锁定该禁杀契约。
- 2026-07-23：修复 stage-window 调用 launch-ready 时 stderr 进度行在 PowerShell 5.1 `ErrorActionPreference=Stop` 下触发提前 catch，表现为首个 static START 后空 `result=`。子步骤改用独立进程重定向捕获，调用方局部容纳诊断流但继续按退出码与 PASS marker 硬门禁；受控 mutex 冲突可完整报告 exit 4 与 `single_instance_conflict`，快速 contract gate 增加诊断保留断言。
- 2026-07-23：修复 ticket dispatch 对纯业务长消息无条件施加 4000 字符头尾截断的问题。历史日志确认 1476 次实际命中均为 `incident-captured`，没有 transcript 删除证据，也未发现行数截断；其他票据类型虽未在历史中实际超限，但共享同一入口。格式化器默认不再截断业务正文，显式上限能力保留，快速 contract gate 动态验证长消息完整性、显式 cap 与 transcript 噪声过滤。
...
- 2026-02-20：CIDR 正文契约烟测扩展：`tools/test/cidr_body_contract_smoke.ps1` 增补为 4 个稳定 case（first-marker 抑制、post-marker 一致性路径、no-marker 直达、IANA 链路尾行存在），本地复跑 `pass=4 fail=0`，日志 `out/artifacts/cidr_body_contract/20260220-130633`；VS Code 任务已提供 `Test: CIDR Body Contract` 与 `Test: CIDR Body Contract (prefilled)`。
- 2026-02-20：CIDR 草案矩阵扩展：`testdata/cidr_matrix_cases_draft.tsv` 新增 no-marker 跨 RIR、全空间 unknown 与 invalid-input 三类可执行用例；定向执行 `redirect_matrix_test.ps1 -CasesFile testdata/cidr_matrix_cases_draft.tsv` 结果 `pass=8 fail=0`，日志 `out/artifacts/redirect_matrix/20260220-125430`。VS Code 已新增一键任务 `Test: Redirect Matrix (CIDR Draft TSV)` 与 `Test: Redirect Matrix (CIDR Draft TSV, prefilled)`；组合任务提供参数化与预填两种入口：`Test: CIDR Contract Bundle` / `Test: CIDR Contract Bundle (prefilled)`（脚本入口：`tools/test/run_cidr_contract_bundle.ps1`），参数化入口已验证通过，两套校验复跑 `pass=4/8, fail=0/0`（`out/artifacts/cidr_body_contract/20260220-135818`，`out/artifacts/redirect_matrix/20260220-135910`），并生成汇总 `out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-135818.txt`。
- 2026-02-20：按《IPv4/IPv6 地址 WHOIS 查询规则契约》实施工作计划（执行中）：Step 1（低风险）先做“响应分类入口统一 + 非权威 marker 表驱动化”；Step 2（中风险）收敛非 CIDR/CIDR 状态机（含 referral 优先、轮询兜底、visited 约束与 CIDR 一次性闭环）；Step 3（治理）处理 `--no-cidr-erx-recheck`（先 deprecated 再移除）；Step 4（优化）仅接入 IANA 首跳候选优化，不介入最终权威裁决。每步完成门禁：`Remote: Build (Strict Version)` + `Test: CIDR Contract Bundle (prefilled)` + `Test: Redirect Matrix (10x6)` 全绿后再进入下一步。
- 2026-02-20：Step 1 首轮落地（进行中）：`src/core/lookup_exec_redirect.c` 与 `src/core/lookup_exec_decision.c` 已将多处散落 marker 判定替换为 `lookup_exec_rules` 统一谓词入口，并统一语义空响应判定调用路径；门禁复跑通过：CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-144025.txt`）与 Redirect Matrix 10x6（`authMismatchFiles=0 errorFiles=0`，`out/artifacts/redirect_matrix_10x6/20260220-144112`）。
- 2026-02-20：编译校验闭环：按远程快速构建命令执行 `remote_build_and_test.sh`（`x86_64+win64`，`lto-auto`）后，`Local hash verify PASS` + `Golden PASS`（产物：`out/artifacts/20260220-145703`，已同步到 `release/lzispro/whois`）；随后使用新编译 `whois-win64.exe` 复跑 Redirect Matrix 10x6（脚本当前为 12x6）全绿：`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`，日志 `out/artifacts/redirect_matrix_10x6/20260220-145801`。
- 2026-02-20：Step 1 第二轮收敛（进行中）：在 `src/core/lookup_exec_rules.h/.c` 新增 APNIC/RIPE/LACNIC 与 IPv6-root 的统一 marker 谓词封装，并将 `src/core/lookup_exec_redirect.c` 残余直写 marker 判断切换为规则入口（等价替换，无语义扩展）；随后按新编译产物复跑门禁通过：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-151042`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-151116.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-151217`）。
- 2026-02-20：Step 1 收尾完成：`src/core/lookup_exec_redirect.c` 剩余两处 `current_rir_guess==arin` 直写比较已统一为 `wc_lookup_exec_is_effective_current_rir()` 入口；随后按同口径完成远程快速构建与回归门禁：`Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-152140`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-152157.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-152256`）。基于 Step 1 目标（分类入口统一 + marker 判定收敛）已达成，下一步进入 Step 2（非 CIDR/CIDR 状态机收敛）。
- 2026-02-20：Step 2 第一轮（最小改动）已开始并通过门禁：在 `src/core/lookup_exec_next.c` 引入 `effective_rir = wc_lookup_exec_rule_effective_rir(current_rir_guess, current_host)` 作为统一当前 RIR 判定来源，收敛 next-hop 选择中的散落 RIR 直写比较（含 ARIN/RIPE/AFRINIC/LACNIC/APNIC 分支与 referral confidence 入参）；策略顺序与 fallback 动作未变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-154029`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-154054.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-154159`）。
- 2026-02-20：Step 2 第二轮（分支门控收敛）通过门禁：在 `src/core/lookup_exec_next.c` 提取统一 helper `wc_lookup_exec_try_pick_rir_cycle()`，将 `no-match / force_rir_cycle / allow_cycle / gate-recover / referral-low-confidence` 五处重复 cycle 分支收敛为单入口，统一 `rir_cycle_exhausted` 写回路径；策略优先级与 fallback tag 保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-155424`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-155454.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-155531`）。
- 2026-02-20：Step 2 第三轮（visited/循环判定去重）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_has_visited_host()`，收敛 `whois.arin.net` 与 `whois.iana.org` 的重复 visited 扫描；将 `wc_lookup_exec_pick_cycle_when_referral_rir_visited()` 改为复用 `wc_lookup_exec_try_pick_rir_cycle()`，进一步统一 cycle 选择路径。该轮仅做等价重构，未改变 referral/cycle 策略顺序。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-160545`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-160605.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-160645`）。
- 2026-02-20：Step 2 第四轮（referral 归一化路径收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_fill_referral_host()` 与 `wc_lookup_exec_accept_referral_host()`，统一 explicit referral 分支与 selftest IANA pivot 回落分支中的 referral host 归一化与 `next_port` 写回逻辑；该轮为等价重构，不调整 referral/cycle 判定顺序。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-161830`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-161915.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-162029`）。
- 2026-02-20：Step 2 第五轮（IANA pivot 资格判定收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_can_use_iana_pivot()`，将“当前 RIR 非已知 RIR 且非 ARIN 且未禁用 pivot”的资格判定从内联分支收敛为单函数；该轮保持 IANA pivot 触发条件与日志动作不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-163418`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-163435.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-163539`）。
- 2026-02-20：Step 2 第六轮（IANA pivot 目标写入收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_set_iana_pivot_next()`，统一常规分支与 selftest `force_iana_pivot` 分支中的 IANA 目标 host 写入与 fallback flag 置位，减少重复分支代码；策略顺序与 fallback 行为保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-165013`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-165027.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-165124`，含一次限流重试后通过）。
- 2026-02-20：Step 2 第七轮（referral 阻断判定收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_should_block_referral_host()`，统一 explicit referral 分支下的三类阻断条件（host 已访问、referral RIR 已访问、APNIC ERX IANA 链路下 ARIN→APNIC 特例）并减少散落条件判断；策略顺序与 fallback 行为保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-170724`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-170820.txt`），Redirect Matrix 10x6（脚本当前为 12x6）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-171110`）。
- 2026-02-20：Step 2 第八轮（force-stop 入口收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_effective_force_stop_authoritative()`，统一 `force_stop_authoritative` 与 `need_redir_eval` 的入口计算，去除内联覆写分支；该轮为等价重构，不调整 next-hop 策略。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-172154`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-172221.txt`）；Redirect Matrix 10x6（脚本当前为 12x6）首轮受 APNIC 限流影响出现 `errorFiles=3`（`out/artifacts/redirect_matrix_10x6/20260220-172343`），同参数即时重跑恢复全绿：`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-173737`）。
- 2026-02-20：Step 2 第九轮（ARIN 直连分支收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_try_pick_arin_direct()`，将首跳 `rir-direct`（`whois.arin.net`）选择与日志写入从主流程内联分支抽离为单入口 helper；策略触发条件、优先级与 fallback tag 保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-183258`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-183312.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-183410`）。
- 2026-02-20：Step 2 第十轮（header-hint 分支收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_try_pick_header_hint()`，将 header hint 的有效性检查、visited 约束、next-host 写入与 fallback 日志从主流程内联分支抽离为单入口 helper；触发条件、优先级与 tag（`manual/header-hint`）保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-184230`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-184245.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-184352`）。
- 2026-02-20：Step 2 第十一轮（ambiguous-stop-apnic 分支收敛）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_try_handle_ambiguous_stop_apnic()`，将 `query terms are ambiguous` 场景下的 APNIC 回访终止判定、标记写入与 fallback 日志从主流程内联分支抽离为单入口 helper；触发条件、优先级与 tag（`manual/ambiguous-stop-apnic`）保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-185621`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-185659.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-185757`）。
- 2026-02-20：Step 2 第十二轮（多分支小包收敛）通过门禁：在 `src/core/lookup_exec_next.c` 一次收敛 3 处分支：`wc_lookup_exec_try_pick_iana_pivot()`（IANA pivot 选择与日志）、`wc_lookup_exec_try_force_iana_pivot_once()`（selftest `force_iana_pivot` 一次性路径）、`wc_lookup_exec_write_apnic_erx_ref_host_if_needed()`（APNIC ERX 回写尾段）；触发条件、优先级与 fallback tag 保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-191210`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-191225.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-191422`）。
- 2026-02-20：Step 2 第十三轮（多分支小包收敛）通过门禁：在 `src/core/lookup_exec_next.c` 一次收敛 3 处分支：`wc_lookup_exec_try_pick_gate_recover_cycle()`（gate-recover 轮询兜底）、`wc_lookup_exec_try_pick_referral_low_confidence_cycle()`（低置信 referral 轮询分流）、`wc_lookup_exec_try_accept_or_cycle_referral()`（referral 接受/visited 回退合流）；触发条件、优先级与 fallback tag 保持不变。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-192448`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-192550.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-192825`）。
- 2026-02-20：Step 2 第十四轮（收口轮）通过门禁：在 `src/core/lookup_exec_next.c` 新增 `wc_lookup_exec_try_pick_pre_pivot_cycle_paths()`，将 pre-pivot 的三段 cycle 路径（`no-match`、`force_rir_cycle`、`allow_cycle`）按原顺序收敛为单入口 helper，未改变触发条件与 fallback tag。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-194512`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-194720.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-194936`，含 lacnic 单点限流重试后通过）。
- 2026-02-20：Step 2 收口结论：`lookup_exec_next.c` 主要状态机分支已完成等价收敛，且连续多轮门禁全绿；Step 2 视为完成，下一阶段进入 Step 3（`--no-cidr-erx-recheck` 治理：先 deprecated 再移除）。
- 2026-02-20：Step 3 第一轮（deprecated 标记）通过门禁：保留 `--no-cidr-erx-recheck` 运行时语义不变，仅在 `src/core/meta.c` help 文案、`include/wc/wc_opts.h` 选项注释与 `docs/USAGE_CN.md`/`docs/USAGE_EN.md` 标注 deprecation 与“下个主版本计划移除”；同时在 `RELEASE_NOTES.md` 追加迁移提示，作为移除前过渡窗口。回归结果：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260220-200848`），CIDR Bundle（body `pass=4 fail=0`，matrix `pass=8 fail=0`，`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-201008.txt`），Redirect Matrix 10x6（脚本当前为 12x6，稳定化参数 `600/2/2500`）`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260220-201101`）。
- 2026-02-20：Step 3 第二轮（runtime deprecation warning）已落地：在 `src/core/opts.c` 解析 `--no-cidr-erx-recheck` 时输出一次性 stderr 告警 `[DEPRECATED] --no-cidr-erx-recheck is deprecated and planned for removal in the next major release.`，不改变选项语义（`cidr_erx_recheck=0` 仍保持原行为）。本地重复传参验证 `deprecated_count=1` 通过。
- 2026-02-20：Step 3 第二轮门禁复验（win64）：Redirect Matrix 10x6（脚本当前为 12x6）复跑 `authMismatchFiles=0`，但同时间窗出现外部限流/拒绝与连接超时噪声，`errorFiles=9`（`tmp/logs/redirect_matrix_12x6_step3r2_win64/20260220-203743`）与 `errorFiles=15`（稳定参数 `600/2/2500`，`tmp/logs/redirect_matrix_12x6_step3r2_win64_stable/20260220-205820`）。
- 2026-02-20：CIDR Contract Bundle（win64）在同一固定 case 连续两次失败：`cidr_with_marker_first_marker_hit_direct_stop_first_marker_rir`（`8.8.0.0/16 @ apnic`）期望 `whois.apnic.net`，实得 `error`（`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-212400.txt`、`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-212823.txt`）。失败样例日志显示 RIPE `access denied`/限流触发并最终落 `error @ error`（`out/artifacts/redirect_matrix/20260220-212924/cases/case_8.8.0.0_16_apnic.log`），当前判定为环境门禁阻塞，待网络窗口恢复后再复核全绿。
...
- 2026-02-20：Step 4 第二轮（仅可观测性）已落地：在 `src/core/dns.c` 新增通用候选汇总标签 `[DNS-CAND-SUM]`（按 hop/host 输出 `mode/start/count` 与来源分解），输出门控与第一轮一致（仅 `--debug` 或 `--retry-metrics`），不改变候选顺序与 authority 语义。
- 2026-02-20：Step 4 第二轮验证：`whois-win64.exe --debug -h whois.iana.org 8.8.8.8` 命中 `[DNS-CAND-SUM] hop=0 host=whois.iana.org ...` 与后续 hop 汇总；非 debug 同查询统计 `dns_cand_sum_count=0`、`dns_cand_iana_count=0`，门控正确。
- 2026-02-20：Step 4 第二轮构建与门禁复验：远程构建同步通过（`out/artifacts/20260220-232930`，`Local hash verify: PASS (some entries missing)`）；CIDR bundle 仍固定失败同一 case（`out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-233117.txt`，matrix `pass=7 fail=1`）；矩阵（稳定参数 `600/2/2500`）结果 `authMismatchFiles=0 errorFiles=13`（`tmp/logs/redirect_matrix_12x6_step4r2_win64/20260220-233322`），继续符合“外部限流阻塞、authority 语义稳定”的临时豁免口径。
- 2026-02-21：Step 4 第三轮（仅可观测性）已落地：在 `src/core/dns.c` 新增 `[DNS-CAND-RATIO]`，按 hop 输出候选来源占比（`input/cache/resolver/known/canonical`）；与前两轮一致，仅在 `--debug` 或 `--retry-metrics` 输出，不改变候选顺序与 authority 语义。
- 2026-02-21：Step 4 第三轮标签验证：`whois-win64.exe --debug -h whois.iana.org 8.8.8.8` 命中 `[DNS-CAND-SUM]` / `[DNS-CAND-RATIO]` / `[DNS-CAND-IANA]`；非 debug 同查询统计标签计数为 0。
- 2026-02-21：Step 4 第三轮构建与门禁复验：远程构建同步通过（`out/artifacts/20260221-000657`，`Local hash verify: PASS (some entries missing)`）；CIDR bundle 仍固定失败同一 case（`out/artifacts/cidr_bundle/cidr_bundle_summary_20260221-000750.txt`，matrix `pass=7 fail=1`）。矩阵两次同参复跑出现 authority mismatch（`tmp/logs/redirect_matrix_12x6_step4r3_win64/20260221-000911/authority_20260221-000911.txt` 与 `tmp/logs/redirect_matrix_12x6_step4r3_win64_rerun/20260221-004158/authority_20260221-004158.txt`），对应样例均含 `Connection error`（`lacnic_45.121.52.0_22.txt`、`arin_45.113.52.0_22.txt`）。
- 2026-02-21：风险结论：由于本轮出现 `authMismatchFiles>0`，临时豁免口径进入“待人工复核”状态；在确认 mismatch 为环境性抖动前，暂停 Step 4 后续轮次，仅保留当前 round-3 观测改动与证据记录。
- 2026-02-21：根因复核与参数化复验（win64）：确认 RIPE 对当前 IPv4 出口存在稳定拒绝（非随机抖动），据此将矩阵参数调整为 `-RirIpPref "arin=ipv6,ripe=ipv6"` 后复跑通过：`preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=0`，`(no errors found)`（`tmp/logs/redirect_matrix_12x6_step4r3_win64_ripe_ipv6/20260221-*`）。结论：authority 语义链路稳定，失败主要由 RIPE IPv4 出口可达性策略触发。
- 2026-02-21：Step 4 第四轮（仅可观测性）已落地：在 `src/core/dns.c` 新增 `[DNS-CAND-UNIQ]`，按 hop 输出候选唯一性统计（`total/unique/duplicate`），用于观测候选池重复度；与前几轮一致，仅在 `--debug` 或 `--retry-metrics` 输出，不改变候选顺序与 authority 语义。
- 2026-02-21：Step 4 第四轮构建复验：远程快速构建与同步通过（`out/artifacts/20260221-024636`），结论 `Local hash verify PASS + Golden PASS`。
- 2026-02-21：Step 4 第四轮矩阵复验（win64，稳定参数 `600/2/2500` + `-RirIpPref arin=ipv6,ripe=ipv6`）两次同参结果均为 `preMissingFiles=0 postHasBodyFiles=0 authMismatchFiles=0 errorFiles=1`，错误分别为 `iana_45.113.52.0_22` 与 `lacnic_45.113.52.0_22` 的单点 `connect timeout`（`tmp/logs/redirect_matrix_12x6_step4r4_win64_ripe_ipv6/20260221-024940`、`tmp/logs/redirect_matrix_12x6_step4r4_win64_ripe_ipv6_rerun/20260221-031813`）。结论：authority 语义稳定，残余为环境性网络噪声。
- 2026-02-21：输出契约修复（显示层，非裁决）：修复“首跳失败且默认正文裁剪时重定向提示行缺失”与“首跳失败标题误显示失败候选 IP”两处显示问题（`src/core/lookup_exec_tail.c`、`src/core/lookup_exec_loop.c`）；验证：远程快速构建 `Local hash verify PASS + Golden PASS`（`out/artifacts/20260221-035818`），并本地复验失败样例 `-h 192.0.2.1 8.8.8.8` 输出 `=== Query: ... @ unknown ===`。
- 2026-02-21：环境恢复复验（同参重跑）：VM 重启并完成系统更新后，按同一远程命令再次执行 `tools/remote/remote_build_and_test.sh`（`x86_64+win64`，`lto-auto`，`-G 1`）恢复全流程通过：`Local hash verify PASS + Golden PASS`，日志 `out/artifacts/20260221-053557`；对应上一轮失败窗口为 `out/artifacts/20260221-052125`，归因为 VM 外网异常导致 smoke/golden 非稳态。
...
- 2026-02-18：Round 38 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-190218`。
- 2026-02-18：`lookup_exec_redirect.c` Round 37（10 刀）完成：将 `saw_rate_limit_or_denied` 的双层条件合并为单层门控，保持置位时机与语义不变。
- 2026-02-18：Round 37 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-185920`。
...
- 2026-02-18：Round 30 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-183357`。
...
- 2026-02-18：Round 17 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-163448`。
...
- 2026-02-18：Round 10 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-154734`。
- 2026-02-18：`lookup_exec_redirect.c` Round 9（6 刀）完成：继续做单次调用 helper 等价收敛（redirect-flag 更新链中的 allow-cycle 初始化/authoritative-stop 处理、APNIC suppress 与 RIPE non-managed 标记链）并内联到主编排，保持判定顺序不变。
- 2026-02-18：Round 9 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-154240`。
- 2026-02-18：`lookup_exec_redirect.c` Round 8（6 刀）完成：继续做单次调用 helper 等价收敛（header authority/match 判定链、LACNIC ERX hint 与 known-rir internal non-auth 判定链、access-state 初始化链、header-hint 初始化链）并内联到主编排，保持判定顺序不变。
- 2026-02-18：Round 8 回归通过：远程 Strict（`x86_64+win64`，`lto-auto`）`无告警 + Golden PASS`，日志 `out/artifacts/20260218-153823`。
...
- 2026-02-18：该规则回归通过：Strict Version（lto-auto 默认）`无告警 + lto 无告警 + Golden PASS + referral check PASS`（`out/artifacts/20260218-133904`）；重定向矩阵 11x6（节流参数 `500/2/2500`）`authMismatchFiles=0 errorFiles=0`（`out/artifacts/redirect_matrix_10x6/20260218-134359`）。
...
#### 执行结果（2026-02-26，P2 版本治理规划）

- `--no-cidr-erx-recheck` 移除窗口评估
  - 当前状态：v3.2.x 已 deprecated（help/USAGE/RELEASE_NOTES/运行时一次性告警均已落地）。
  - 建议窗口：
    - 当前与后续一个 patch 窗口继续保留兼容（仅告警，不改语义）；
    - next-major 执行移除（删除选项解析与 help 暴露，保留迁移说明）。
  - 迁移建议：脚本与任务中立即移除该参数，默认 CIDR 路径保持 ERX/IANA 基准复查。

- next-major 兼容公告草稿
  - 已新增：`docs/release_bodies/next-major-compat-announcement-draft.md`
  - 内容包含：影响面、替代路径、建议时序、回滚建议、中英短通知模板。
...
#### 执行记录模板（2026-02-28，Step 4.5 仅观测接入）
...
#### 执行结果（2026-03-16，Step 4.7 受控 early-unknown 试验入口）

- 本轮代码改动（默认关闭）
  - 新增参数：`--enable-step47-early-unknown`（默认关闭）。
  - 生效条件：必须同时满足
    - `--enable-step47-trial`
    - `--step47-trial-scope reserved`
    - 候选命中 `255.0.0.0`
    - 未显式指定 `-h`
  - 动作：触发 `action=step47-short-circuit-unknown`，并输出 `unknown` 尾行。

- 本轮门禁与验证
  - Remote Strict（lto-auto）：PASS，目录 `/out/artifacts/20260316-023011`。
  - A/B + early-unknown（reserved）：`eligible=4 short_circuit=1 auth_changed=1 route_changed=1`，目录 `.\out\artifacts\step47_ab\20260316-023241`。
  - A/B + early-unknown（minimal）：`eligible=1 short_circuit=0 auth_changed=0 route_changed=0`，目录 `.\out\artifacts\step47_ab\20260316-023249`。
  - A/B + early-unknown（all）：`eligible=5 short_circuit=0 auth_changed=0 route_changed=0`，目录 `.\out\artifacts\step47_ab\20260316-023257`。

- 结论
  - 试验入口已受控接入：仅在 `reserved` scope 触发单点 short-circuit；其余 scope 仍保持“仅观测不改收敛”。
  - 默认行为未变化（不开开关时与 Step 4.6 一致）。
...
**进展速记（2026-02-19，invalid CIDR 收口）**：
...
- 评估并最小化 `lookup_text` 与 `lookup_exec_empty` 的重叠判定，统一“先非权威 marker、后语义空响应”的优先级说明，补到 RFC 规则段。
- 继续按“单轮单目标 + 双闸回归（Strict + matrix）”推进，新增规则前先补文档清单再落实现。
...
- 拆分续作（2026-02-18，第四十轮）：在 `src/core/lookup_exec_tail.c` 做输出参数命名一致性整理，将 `wc_lookup_exec_run_tail_guard_loop_capture_check` 中的 `force_original_next` 出参语义统一为 `next_state_force_original_out`，并同步局部变量名 `computed_force_original_next -> next_state_force_original`；仅命名调整，行为保持不变。
- 拆分回归（2026-02-18，第四十轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-094217`。
- 拆分续作（2026-02-18，第四十一轮）：在 `src/core/lookup_exec_tail.c` 做小范围语义化命名整理，新增 `WC_LOOKUP_EXEC_TAIL_FLAG_REDIRECT_CAP` 常量替代 `fallback_flags` 中的字面量 `0x10`，并将 `wc_lookup_exec_run_redirect_cap_check` 对齐命名为 `wc_lookup_exec_run_tail_redirect_cap_check`，同步调用点；行为保持不变。
- 拆分回归（2026-02-18，第四十一轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-094551`。
- 拆分续作（2026-02-18，第四十二轮）：继续在 `src/core/lookup_exec_tail.c` 做 pre-check 链路拆分，将 `wc_lookup_exec_run_tail_pre_checks` 拆为 4 个步骤 helper：`wc_lookup_exec_run_tail_pre_authority_check`、`wc_lookup_exec_run_tail_pre_clear_referral`、`wc_lookup_exec_run_tail_pre_guard_no_next_check`、`wc_lookup_exec_run_tail_pre_mark_pending_referral`，主流程仅保留编排；行为保持不变。
- 拆分回归（2026-02-18，第四十二轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-095005`。
- 拆分续作（2026-02-18，第四十三轮）：继续在 `src/core/lookup_exec_tail.c` 做 post-check 链路拆分，将 `wc_lookup_exec_run_tail_post_checks` 拆为 3 个步骤 helper：`wc_lookup_exec_run_tail_post_guard_loop_check`、`wc_lookup_exec_run_tail_post_append_redirect_header`、`wc_lookup_exec_run_tail_post_write_next_state`，主流程仅保留编排；行为保持不变。
...
- 拆分续作（2026-02-18，第四十六轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做同主题多点等价细化，新增 `wc_lookup_exec_run_tail_redirect_cap_reached` / `wc_lookup_exec_run_tail_redirect_cap_mark_hit` / `wc_lookup_exec_run_tail_redirect_cap_set_unknown_authority`、`wc_lookup_exec_run_tail_pre_should_mark_pending_referral`、`wc_lookup_exec_run_tail_post_should_append_redirect_header`，将 redirect-cap、pending_referral、redirect-header 三处路径统一为“条件判断 + 动作执行”编排；行为保持不变。
...
- 拆分续作（2026-02-18，第四十九轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做“条件下沉”同主题多点细化，新增 `wc_lookup_exec_run_tail_pre_should_clear_referral` 与 `wc_lookup_exec_run_tail_post_should_write_next_host` / `wc_lookup_exec_run_tail_post_should_write_next_port` / `wc_lookup_exec_run_tail_post_should_write_force_original_query`，并将 referral 清理与三类 next-state 写回统一为 `should_* + action` 编排；行为保持不变。
...
- 拆分续作（2026-02-18，第五十二轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 对 guard-loop capture 路径做同主题多点阶段化，新增 `wc_lookup_exec_run_tail_prepare_next_state_force_original`、`wc_lookup_exec_run_tail_apply_apnic_force_ip_if_needed`、`wc_lookup_exec_run_tail_execute_guard_loop_check`、`wc_lookup_exec_run_tail_finalize_guard_loop_capture`，将计算/应用/执行/收尾四段显式分层；行为保持不变。
- 拆分回归（2026-02-18，第五十二轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-103549`。
- 拆分续作（2026-02-18，第五十三轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做 tail pipeline 与入口就绪判定的同主题多点细化，新增 `wc_lookup_exec_run_tail_post_finalize_append_stage` / `wc_lookup_exec_run_tail_post_finalize_write_state_stage` / `wc_lookup_exec_run_tail_post_finalize_pipeline` 以及 `wc_lookup_exec_is_tail_context_has_out` / `wc_lookup_exec_is_tail_context_has_opts` / `wc_lookup_exec_is_tail_context_has_net_info`，进一步将 post 收尾与 runtime-ready 判定压缩为显式阶段编排；行为保持不变。
- 拆分回归（2026-02-18，第五十三轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-103851`。
- 拆分续作（2026-02-18，第五十四轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做 pipeline 与入口 gating 的同主题细化，新增 `wc_lookup_exec_run_tail_checks_should_continue_post` / `wc_lookup_exec_run_tail_checks_post_execute_stage`、`wc_lookup_exec_is_tail_context_dependencies_ready`、`wc_lookup_exec_should_handle_tail` / `wc_lookup_exec_execute_tail_handle`，将 pre/post 短路与 handle 入口进一步压缩为显式条件+执行阶段编排；行为保持不变。
- 拆分回归（2026-02-18，第五十四轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-104209`。
- 拆分续作（2026-02-18，第五十五轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 对 post-checks 与 checks-pipeline 做“prepare/execute/decision”同主题细化，新增 `wc_lookup_exec_run_tail_post_prepare_next_state_force_original` / `wc_lookup_exec_run_tail_post_guard_loop_execute_stage` / `wc_lookup_exec_run_tail_post_finalize_execute_stage` 以及 `wc_lookup_exec_run_tail_checks_pre_execute_stage` / `wc_lookup_exec_run_tail_checks_pre_stage_stopped`，将 post 与 pre/post 串接阶段进一步显式分层；行为保持不变。
- 拆分回归（2026-02-18，第五十五轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-104621`。
- 拆分续作（2026-02-18，第五十六轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做 checks-pipeline 与 handle 入口的同主题细化，新增 `wc_lookup_exec_run_tail_checks_pre_stop_stage` / `wc_lookup_exec_run_tail_checks_pre_allows_post_stage` / `wc_lookup_exec_run_tail_checks_post_or_stop_stage`，并新增 `wc_lookup_exec_handle_tail_invalid_result`，将 pre/post 决策与入口失败返回显式化为条件+执行编排；行为保持不变。
- 拆分回归（2026-02-18，第五十六轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-104926`。
- 拆分续作（2026-02-18，第五十七轮，加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做 checks 决策与入口执行的同主题细化，新增 `wc_lookup_exec_run_tail_checks_post_blocked_result` / `wc_lookup_exec_run_tail_checks_post_allowed_execute_stage` / `wc_lookup_exec_run_tail_checks_post_decision_stage`、`wc_lookup_exec_is_tail_context_core_dependencies_ready` / `wc_lookup_exec_is_tail_context_network_dependency_ready`，并新增 `wc_lookup_exec_handle_tail_should_reject` / `wc_lookup_exec_handle_tail_execute_or_reject`；行为保持不变。
- 拆分回归（2026-02-18，第五十七轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-113249`。
- 拆分续作（2026-02-18，第五十八轮，双加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 对 pre-checks / post-checks / context-valid 三条链做同主题高密度分层，新增 `wc_lookup_exec_run_tail_pre_authority_stopped_result` / `wc_lookup_exec_run_tail_pre_guard_no_next_stopped_result` / `wc_lookup_exec_run_tail_pre_finalize_execute_stage`、`wc_lookup_exec_run_tail_post_guard_loop_stopped_result`、`wc_lookup_exec_is_tail_context_not_present_result` / `wc_lookup_exec_is_tail_context_runtime_ready_stage`，将停止结果与执行阶段进一步显式化；行为保持不变。
- 拆分回归（2026-02-18，第五十八轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-113635`。
- 拆分续作（2026-02-18，第五十九轮，双加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 对 checks 与 handle 决策链做同主题细化，新增 `wc_lookup_exec_run_tail_checks_pre_stopped_result` / `wc_lookup_exec_run_tail_checks_pre_decision_stage` / `wc_lookup_exec_run_tail_checks_post_decision_execute_stage`，以及 `wc_lookup_exec_handle_tail_reject_result` / `wc_lookup_exec_handle_tail_execute_result`，进一步将 pre/post 与入口 reject/execute 分支结果显式化；行为保持不变。
- 拆分回归（2026-02-18，第五十九轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-114029`。
- 拆分续作（2026-02-18，第六十轮，双加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做 checks/result 与 runtime-ready 决策的同主题细化，新增 `wc_lookup_exec_run_tail_checks_pre_decision_result` / `wc_lookup_exec_run_tail_checks_post_decision_result`、`wc_lookup_exec_is_tail_context_runtime_ready_result` / `wc_lookup_exec_is_tail_context_runtime_ready_decision_stage`，并新增 `wc_lookup_exec_handle_tail_final_result`，将 checks、runtime-ready、handle 最终返回进一步显式化；行为保持不变。
- 拆分回归（2026-02-18，第六十轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-114423`。
- 拆分续作（2026-02-18，第六十一轮，双加倍多刀）：继续在 `src/core/lookup_exec_tail.c` 做收口型细化，新增 `wc_lookup_exec_run_tail_checks_prepare_pre_stage_result` / `wc_lookup_exec_run_tail_checks_pre_stage_stop_result` / `wc_lookup_exec_run_tail_checks_continue_result`，以及 `wc_lookup_exec_handle_tail_is_rejected` / `wc_lookup_exec_handle_tail_continue_result`，将 checks 与 handle 最终分支结果再做一层显式编排；行为保持不变。
- 拆分回归（2026-02-18，第六十一轮）：使用同一条远程命令复跑 `x86_64+win64`（`lto-auto`）构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-114803`。
- 收官清理（2026-02-18）：对 `src/core/lookup_exec_tail.c` 执行反向收敛，移除 `wc_lookup_exec_handle_tail_final_result` 及同类纯转发/常量返回 wrapper，`wc_lookup_exec_handle_tail` 直接保留“上下文校验 + checks pipeline 调度”最小等价路径；行为保持不变。
- 收官清理回归（2026-02-18）：使用固定远程命令（`-H 10.0.0.199 -u larson -k '/c/Users/妙妙呜/.ssh/id_rsa' -t 'x86_64 win64' -w 0 -r 1 -q '8.8.8.8 1.1.1.1 10.0.0.8' -a '' -G 1 -E '' -O 'lto-auto' -L 0`）复跑构建 + 双端 smoke + golden，全部 PASS，日志 `out/artifacts/20260218-120252`。
...
**进展速记（2026-03-28，Address-Space 前置分类器 P1 第三刀）**：
- candidate 治理：新增 `--preclass-action-tier r0|r1`（默认 `r0`），用于 P1 动作候选分层。
  - `r0`：仅 `255.0.0.0` 允许受控短路。
  - `r1`：扩展到 `255.0.0.0`、`10.0.0.1`、`fc00::1`、`fe80::1`。
- 兼容约束保持不变：仍需 `--enable-preclass-actions + --enable-step47-trial` 双门控，显式 `-h` 继续旁路。
- 观测增强：`[PRECLASS-DECISION]` 增加 `p1_tier=r0|r1` 字段。
- 验证：
  - 远程 Strict（lto-auto）PASS：`out/artifacts/20260328-015504`（hash/golden/referral 全通过）。
  - P1 门控矩阵 PASS：`out/artifacts/preclass_p1_matrix/20260328-015001`（`pass=30 fail=0`，含 r0/r1 分层断言）。
  - 默认最小矩阵 PASS：`out/artifacts/preclass_matrix/20260328-015033`（`pass=12 fail=0`）。
...
**阶段完成标记（2026-03-28）**：
- P2 收口判定已满足：参数透传闭环、三闸全绿、Step47 双链路全绿、strict `-K` 人工透传验证完成。
...
3. [x] 若全绿：先贴 PASS 单段快报，再补完整复盘块；若有失败：先贴 FAIL 单段快报，并回填 `run_ts/failed_gate/evidence_path/cause_next`。
4. [x] 若出现外部拒绝/限流噪声（如 `%ERROR:201`），按 `docs/RELEASE_FLOW_CN.md` 的网络窗口复验建议执行参数化复验并保留双份证据（本轮未触发，已按模板记录）。
5. [x] 完成后同步更新 `docs/RFC-whois-client-split.md` 与 `RELEASE_NOTES.md` 当日增量条目，并确认工作区干净。
6. [x] 产出一版“Address-Space 前置分类器可执行设计骨架”（数据模型、生成脚本输入输出、查表 API、门禁断言），并在 RFC 中明确落地顺序与回退点。
...
**下次开工清单（2026-04-02）**：
1. [x] 组合链路验证（D6 合流）：执行 Remote Strict（`-K 1 -N 1`）验证 preflight 与 table guard 同时启用时全链路 PASS，并记录 `STRICT_TS/PREFLIGHT_TS/TABLE_GUARD_TS`。
2. [x] One-Click dry-run 全链路验证：通过任务入口执行 `oneClickDryRun=true` + `rbPreflight=1` + `rbPreclassTableGuard=1`，确认“跳过 tag/release 更新 + 不自动提交 statics”的无副作用语义。
...
- 第 2 条 dry-run 全链路复验：
  - 首轮 preflight 失败：`out/artifacts/step47_preclass_preflight/20260403-022527`（`pass=3 fail=1`，失败点为 rollback 子步骤）。
  - 复跑（`-RbCflagsExtra ''`）PASS：`out/artifacts/20260403-023609`、`out/artifacts/step47_preclass_preflight/20260403-023618`、`out/artifacts/preclass_table_guard/20260403-024219`。
  - dry-run 无副作用语义命中：`statics changes detected but commit/push skipped`、`skipping GitHub release update`、`skipping Gitee release update`、`one-click done: dry-run mode; tag=v3.2.12`。
...
**进展速记（2026-04-03，UI 入口串行补证完成）**：
- `Test: One-Click DryRun Guard (build+sync, prefilled)` 已按串行触发并完成，证据 `out/artifacts/oneclick_dryrun_guard/20260403-064550`。
  - 结果：`exit_code=0`、`guard_result=pass`，但在 `RequireStaticsDetectedIfBuildSync=true` 下 `statics_detected=false`，因此 `smoke_result=fail`。
  - 解释：本轮无新的 static delta，触发了“必须检测到静态变更”的严格断言；该结果与脚本口径一致，属于可解释失败。
- `Gate: D6 Double-Round Consistency (prefilled)` 已按串行触发并 PASS，证据 `out/artifacts/d6_consistency_double_round/20260403-065703`。
  ...
  - 判定：两轮 `RoundPass=True`，关键闸项全 `True`（见 `summary.csv`）。

**下次开工清单（2026-04-04）**：
1. [x] 任务入口实跑（UI 口径）：从 VS Code 任务面板执行 `Gate: D6 Double-Round Consistency`、`Test: One-Click DryRun Guard (local)`、`Test: One-Click DryRun Guard (build+sync)`，确认任务入口与脚本直跑结果一致。（已完成 prefilled 串行补证：local PASS、D6 PASS；build+sync 在 `RequireStaticsDetectedIfBuildSync=true` 下因 `statics_detected=false` 产生可解释失败）
...
**进展速记（2026-04-05，继续推进 Daily 链路）**：
- UI 入口 Daily 串行链路 PASS：`local=20260405-013507`、`build+sync no-delta-ok=20260405-013515`、`D6=20260405-014305`。
- no-delta 口径验证：`build+sync` 本轮 `statics_detected=false` 且 `smoke_result=pass`，链路健康判定成立。
...
**进展速记（2026-04-05，继续推进第二轮）**：
- Daily 三任务（UI 串行）PASS：`local=20260405-032635`、`build+sync no-delta-ok=20260405-032642`、`D6=20260405-033428`（两轮 `RoundPass=True`）。
- strict/no-delta 并排复验 PASS：`strict=20260405-035747`、`no-delta-ok=20260405-040554`，本轮 strict 仍为 `statics_detected=true`。
- D6 非默认样本（`1.0.0.1 45.113.52.0/22 2404:6800:4008::200e`）首轮出现“Round2 单轮异常”：`out/artifacts/d6_consistency_double_round/20260405-041520`；按分流规则立即重跑后恢复全 PASS：`out/artifacts/d6_consistency_double_round/20260405-043523`。
- 模板抽测（PowerShell + bash+grep）继续命中：
  - PowerShell：`out/artifacts/oneclick_dryrun_guard/20260405-040554/{summary.txt,oneclick_dryrun.log}`、`out/artifacts/d6_consistency_double_round/20260405-043523/summary.csv`。
  - bash+grep：命中 `summary.txt` 的 `key: value` 与 log 的 `key=value` 两类行，验证兼容正则有效。

**进展速记（2026-04-05，按 2026-04-08 清单执行）**：
- Daily 三任务（UI 串行）PASS：`local=20260405-050330`、`build+sync no-delta-ok=20260405-050338`、`D6=20260405-051137`（两轮 `RoundPass=True`）。
- strict/no-delta 并排复验 PASS：`strict=20260405-053514`、`no-delta-ok=20260405-054315`，本轮 strict 为 `statics_detected=true`。
- D6 非默认样本（`9.9.9.9 43.227.220.0/22 2606:4700:4700::1111`）PASS：`out/artifacts/d6_consistency_double_round/20260405-055041`，Round1 `20260405-055404/055412/055854`，Round2 `20260405-060602/060610/061036`。
- 模板抽测（PowerShell + bash+grep）PASS：
  - PowerShell：`out/artifacts/oneclick_dryrun_guard/20260405-054315/{summary.txt,oneclick_dryrun.log}`、`out/artifacts/d6_consistency_double_round/20260405-055041/summary.csv`。
  - bash+grep：同目录命中 `key: value` 与 `key=value` 双格式字段。

**进展速记（2026-04-05，按 2026-04-09 清单执行）**：
- Daily 三任务（UI 串行）PASS：`local=20260405-062047`、`build+sync no-delta-ok=20260405-062056`、`D6=20260405-062756`（两轮 `RoundPass=True`）。
- strict/no-delta 并排复验 PASS：`strict=20260405-064857`、`no-delta-ok=20260405-065628`；本轮 strict 为 `statics_detected=true`。
- D6 非默认样本（`208.67.222.222 203.26.12.0/24 2620:119:35::35`）PASS：`out/artifacts/d6_consistency_double_round/20260405-070422`，Round1 `20260405-070803/070811/071140`，Round2 `20260405-071846/071854/072243`。
- 模板抽测（PowerShell + bash+grep）PASS：
  - PowerShell：`out/artifacts/oneclick_dryrun_guard/20260405-065628/{summary.txt,oneclick_dryrun.log}`、`out/artifacts/d6_consistency_double_round/20260405-070422/summary.csv`。
  - bash+grep：同目录命中 `key: value` 与 `key=value` 双格式字段。

**进展速记（2026-04-05，按 2026-04-10 清单执行）**：
- Daily 三任务（UI 串行）PASS：`local=20260405-090255`、`build+sync no-delta-ok=20260405-090304`、`D6=20260405-091112`（两轮 `RoundPass=True`）。
- strict/no-delta 并排复验 PASS：`strict=20260405-093833`、`no-delta-ok=20260405-094715`；本轮 strict 为 `statics_detected=true`。
- D6 非默认样本（`149.112.112.112 45.236.136.0/22 2001:4860:4860::8844`）首跑出现“Round2 单轮异常”：`out/artifacts/d6_consistency_double_round/20260405-095722`；按分流规则立即重跑后恢复全 PASS：`out/artifacts/d6_consistency_double_round/20260405-101930`。
- 模板抽测（PowerShell + bash+grep）PASS：
  - PowerShell：`out/artifacts/oneclick_dryrun_guard/20260405-094715/{summary.txt,oneclick_dryrun.log}`、`out/artifacts/d6_consistency_double_round/20260405-101930/summary.csv`。
  - bash+grep：同目录命中 `key: value` 与 `key=value` 双格式字段。

**进展速记（2026-04-05，按 2026-04-11 清单执行）**：
- Daily 三任务（UI 串行）PASS：`local=20260405-132622`、`build+sync no-delta-ok=20260405-132637`、`D6=20260405-133529`（两轮 `RoundPass=True`）。
- strict/no-delta 并排复验 PASS：`strict=20260405-140407`、`no-delta-ok=20260405-141319`；本轮 strict 为 `statics_detected=true`。
- D6 非默认样本（`64.6.64.6 103.53.144.0/22 2620:fe::fe`）PASS：`out/artifacts/d6_consistency_double_round/20260405-142232`，Round1 `20260405-142619/142628/143120`，Round2 `20260405-143854/143903/144528`。
- 模板抽测（PowerShell + bash+grep）PASS：
...
**进展速记（2026-04-05，按 2026-04-13 开发切片执行）**：
- in-scope：仅增强 `PRECLASS-DECISION` 观测字段（`action_src` / `match_layer` / `fallback`），不改变路由与终态语义。
- out-of-scope：不调整 `title/grep/fold` 输出契约，不变更重定向判定与默认开关语义。
- 代码实现：`src/core/whois_query_exec.c` 在 `wc_preclass_emit_observation()` 增补稳定观测字段，并保持 `route_change` 与决策入口逻辑不变。
- 条件专项门禁（因触及预分类观测逻辑而触发）PASS：
  - Remote Strict（lto + smoke + sync + golden）：`out/artifacts/20260405-234432`（`Local hash verify PASS`、`[golden] PASS`、`referral check PASS`）。
...
**下次开工清单（2026-04-13）**：
1. [x] 开发切片定义：明确本轮要落地的功能点/修复点（含影响模块与验收条件），并在段落内记录“in-scope / out-of-scope”。
2. [x] 代码实现与本地自检：完成最小改动实现，执行对应自测（含必要的 `--selftest*` 或相关脚本）并记录结果。
3. [x] 最小回归门禁（串行 1 轮）：`local prefilled` -> `build+sync no-delta-ok` -> `D6 prefilled`，记录 `TASK_ONECLICK_TS/TASK_D6_TS`。（本轮以更严格的 Remote Strict + golden + referral 作为替代门禁留证）
4. [x] 条件专项门禁：仅当触及输出契约/DNS/重定向/预分类逻辑时，补跑对应专项（如 CIDR Contract / Redirect Matrix / Preclass Guard）并留证。
5. [x] 文档回填：同步更新 `docs/RFC-address-space-preclassifier.md`、`docs/RFC-whois-client-split.md`、`RELEASE_NOTES.md`，说明“变更点 + 最小门禁结果 + 是否触发专项门禁”。
6. [x] 收尾清理：若出现 static delta，统一提交推送；若无则记录 `no static delta` 并确认工作区干净。
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，2026-04-26 ~ 2026-05-03，草案）**：
...
2. [x] 完成 RFC 回填（至少 `docs/RFC-whois-client-split.md` 与相关证据路径），发布说明按提交流程补齐。
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，2026-05-04 ~ 2026-05-11，草案，串行第 2 份）**：
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，2026-05-12 ~ 2026-05-19，草案，串行第 3 份）**：

> 注：本清单为新一轮 A 清单，目标是与下一份（2026-05-20 ~ 2026-05-27）形成 A -> B 严格串行执行。D1~D3 任务类型必须为 `regex-patch` 或 `builtin`，不得为 `noop`。
> 回填状态：2026-04-10 已执行并一次性收敛（`rounds_total=8`、`rounds_pass=8`、`result=pass`）。

**八轮通用约束（开跑前确认）**：
1. [x] 与下一份 B 清单串行执行，本清单先执行。
2. [x] strict 刷新链路保持开启（`-K 1 -N 1`），目标为全轮 `D6Pass=True` 且 `RoundPass=True`。
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，2026-05-20 ~ 2026-05-27，草案，串行第 4 份）**：

> 注：本清单为新一轮 B 清单，仅在上一份（2026-05-12 ~ 2026-05-19）完成后启动。D1~D3 任务类型必须为 `regex-patch` 或 `builtin`，不得为 `noop`。
> 回填状态：2026-04-10 已执行并一次性收敛（`rounds_total=8`、`rounds_pass=8`、`result=pass`）。

**八轮通用约束（开跑前确认）**：
1. [x] 与上一份 A 清单串行执行，不并行；A 完成后再执行本清单。
2. [x] strict 刷新链路保持开启（`-K 1 -N 1`），目标为全轮 `D6Pass=True` 且 `RoundPass=True`。
...
**两份清单串行入口（A -> B，均为无人值守提速口径，2026-05-12 ~ 2026-05-27）**：
...
> 说明（2026-04-15）：若 `CodeStepCommand` 调用 `tools/test/autopilot_code_step_rounds.ps1` 且轮次覆盖 D1（`-StartRound <= 1`），命令内必须显式包含 `-Reset` 或 `-ResetStateOnly`；若遗漏且检测到历史 `invocationCount > 0`，入口将 fail-fast 终止。
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，2026-05-28 ~ 2026-06-04，草案，串行第 5 份，Checklist A）**：
...
**执行回填（2026-04-16，A/B 无人值守连续运行首次完整收口）**：

- 运行总览：按 A（`restore-source`）-> B（`state-only`）串行执行，整体为近 10 小时量级连续无人值守链路（从 A 开始到 B 收口约 11 小时）。
- A 清单结论：`out/artifacts/dev_verify_multiround/20260415-175235/final_status.json` 为 `Result=pass`、`CompletedRoundCount=8`。
- B 首次结论：`out/artifacts/dev_verify_multiround/20260415-232520/final_status.json` 为 `Result=fail`（`FailedRoundTags=["D1"]`）。
- B 故障处理：按既有规程完成本地/远端链路清理后，以 `-ResetCodeStepState -CodeStepResetPolicy state-only` 重启 B。
- B 重启收口：`out/artifacts/dev_verify_multiround/20260416-003754/final_status.json` 为 `Result=pass`、`CompletedRoundCount=8`；V4 证据见 `out/artifacts/dev_verify_multiround/20260416-003754/V4.log` 与 `out/artifacts/autopilot_dev_recheck_8round/20260416-042520/V4_d6_attempt1/20260416-042520/summary.csv`（双轮均 `RoundPass=True`）。
- 里程碑结论：这是首次在“B 触发网络波动故障处理后”仍成功完成 A/B 全链路无人值守串行收口，具备发布侧复用价值。

**下次开工清单（A/B 串行，2026-04-17，已完成回填）**：

1. [x] 固化本次证据索引：将 A/B 最终目录（`20260415-175235`、`20260415-232520`、`20260416-003754`）登记到当日执行台账与 release 凭证清单。
2. [x] 运行前健康检查：执行 SSH 连通、远端磁盘空间、`RbPreflight/RbPreclassTableGuard` 开关与任务文件存在性四项预检。
3. [x] 严格串行执行口径：A 固定 `restore-source`，B 固定 `state-only`，两者都显式携带 `-ResetCodeStepState`，禁止并行。
4. [x] B 故障重启标准化：若出现 D1 失败，先完成“本地+远端进程清理 -> 证据快照 -> state-only 重启”，不得直接二次启动。
5. [x] V3 非默认样本保留：继续固定混合样本（v4 + CIDR + v6）用于发布前最小异构复核，不降级为默认查询集。
6. [x] 回填同步：收口后同步更新 `docs/RFC-address-space-preclassifier.md` 与 `docs/OPERATIONS_CN.md`（含一段式日报/发布记录文本）。

**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-06-13 ~ 2026-06-20，串行第 7 份，Checklist A，已完成回填）**：
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-06-21 ~ 2026-06-28，串行第 8 份，Checklist B，已完成回填）**：
...
**模板复用项（本轮建议显式保留，供后续任务直接复用）**：

1. [x] `AUTO 会话预置模板`：沿用上一版结构与变量命名，仅更新任务文件与步幅：
  - `AUTO_TASK_FILE_A=testdata/autopilot_code_step_tasks_20260613_20260620.json`
  - `AUTO_TASK_FILE_B=testdata/autopilot_code_step_tasks_20260621_20260628.json`
  - `AUTO_DEV_VERIFY_STRIDE_A=2`、`AUTO_DEV_VERIFY_STRIDE_B=2`
2. [x] `AUTO 映射口径（A/B 入口）`：沿用上一版，不改映射关系（变量仍映射到 `start_dev_verify_8round_multiround.ps1` 同名参数）。
3. [x] `工作前置清单（A/B 开跑前）`：沿用上一版，唯一需要更新的是“两份任务文件路径”。
4. [x] `两份清单串行入口`：沿用 `A=restore-source`、`B=state-only`、都显式 `-ResetCodeStepState`、禁止并发。
5. [x] `固定运行策略（D1 监控容忍窗口）`：沿用 90 分钟窗口、30 分钟只观测、30~90 分钟每 10 分钟活性判定、连续 20 分钟三条件才判挂起。
6. [x] `D4 轮口径`：本轮与上一版不同，D4 明确为“实改码轮”，不再使用冻结/noop；任务定义确保 D4 regex 唯一命中（避免 `expected exactly one match, actual>1`）。

```powershell
# Sequence A (single-arg fastmode wrapper)
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/start_dev_verify_fastmode_A.ps1 autopilot_code_step_tasks_20260613_20260620.json

# Sequence B (single-arg fastmode wrapper)
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/start_dev_verify_fastmode_B.ps1 autopilot_code_step_tasks_20260621_20260628.json
```

**最新版 AUTO 预置模板（本次任务文件 + stride=2，独立节，执行优先）**：
...
**进展速记（2026-04-16，无人值守白名单自愈策略落地）**：
- 目标范围：仅启用 3 条白名单策略，不引入额外自动修复动作。
...
| 任务名 | grep 关键字（优先） | 用途 |
| --- | --- | --- |
| `Test: One-Click DryRun Guard (local, prefilled)` | `\[ONECLICK-DRYRUN-SMOKE\] result=|git_state_unchanged=|guard_result=` | 快速判断本地 dry-run 是否无副作用且 guard 生效 |
| `Test: One-Click DryRun Guard (build+sync, prefilled)` | `\[ONECLICK-DRYRUN-SMOKE\] result=|statics_detected=|guard_result=` | 快速判断 build+sync 断言是否通过、是否产生 static delta |
...
- **性能优化技术路线备忘（2026-01-15）**：
  ...
  - 地址选择优化：记录连接 RTT；按 RTT 排序优先；失败地址短期冷却避免反复尝试。
  - 本地轻缓存：缓存 DNS 解析、RIR 选择结果、失败/RTT 历史；启动加载、TTL 清理；提供开关禁用/清空。
  - 观测与验证：stderr 增补 RTT/失败统计标签（debug-only），保持 stdout 契约不变；对比官方 whois 的 P50/P95/失败率；跑 default+debug 冒烟、批量四向、自检四向黄金。
...
- 覆盖验证：
  ...
  - workbuf 压力自检：`--selftest-workbuf` 长行/CRLF + dense 续行两轮（含 fold+unique 与 grep-block 场景）均 `[WORKBUF] ... result=PASS`，日志目录 `out/artifacts/workbuf_stress/20260109-133951`。退出码 1 原因是内置 `force-suspicious` 场景预期触发，非 workbuf 失败。
  - Phase 1 主控拆分进展：`wc_client_flow` 入口切分为 mode 准备 / 分发 helper，批量入口 banner/penalty/signal 预检封装，渲染配置集中为 `wc_client_render_opts` 并复用到单查询路径，无行为改动。
  - 新一轮覆盖验证（拆分后）：
    - 远程冒烟 + 黄金（默认）：无告警 PASS，日志 `out/artifacts/20260109-145129`。
    ...
    - 四轮黄金回归（批量渲染 crash 修复后）：
      ...
      - 渲染入口下沉后复跑：
        - 远程冒烟 + 黄金（默认）：无告警 `[golden] PASS`，日志 `out/artifacts/20260109-225321`。
        - 远程冒烟 + 黄金（`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）：无告警 `[golden] PASS`，日志 `out/artifacts/20260109-225816`。
        - 批量策略黄金 raw/health-first/plan-a/plan-b：全 PASS，日志 `out/artifacts/batch_raw/20260109-230239/build_out/smoke_test.log`、`out/artifacts/batch_health/20260109-230638/build_out/smoke_test.log`、`out/artifacts/batch_plan/20260109-230940/build_out/smoke_test.log`、`out/artifacts/batch_planb/20260109-231213/build_out/smoke_test.log`（报告同目录）。
        - 自检黄金（`--selftest-force-suspicious 8.8.8.8` raw/health-first/plan-a/plan-b）：plan-b referral 校验失败（缺少 whois.afrinic.net 尾行与 redirect 提示），日志 `out/artifacts/batch_planb/20260109-232630/build_out/smoke_test.log`；referral_checks 见同目录 143.128.0.0/。需复核 AF* referral 输出与黄金差异。
      - 自检黄金复跑（计划 B referral 复核后）：raw/health-first/plan-a/plan-b 全部 `[golden-selftest] PASS`，日志 `out/artifacts/batch_raw/20260109-234646/build_out/smoke_test.log`、`out/artifacts/batch_health/20260109-234931/build_out/smoke_test.log`、`out/artifacts/batch_plan/20260109-235158/build_out/smoke_test.log`、`out/artifacts/batch_planb/20260109-235420/build_out/smoke_test.log`。
      - 四轮黄金再验证（渲染尾部集中化后）：
        - 远程冒烟 + 黄金（默认）：无告警 `[golden] PASS`，日志 `out/artifacts/20260110-000925`。
      ...
      - 批量错误分支瘦身后四轮黄金：远程冒烟 + 黄金（默认）无告警 PASS，日志 `out/artifacts/20260110-020138`；`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first` 同样 PASS，日志 `out/artifacts/20260110-020851`。批量策略 raw/health-first/plan-a/plan-b 全 PASS，日志 `out/artifacts/batch_raw/20260110-021336/build_out/smoke_test.log`、`out/artifacts/batch_health/20260110-021724/build_out/smoke_test.log`、`out/artifacts/batch_plan/20260110-021950/build_out/smoke_test.log`、`out/artifacts/batch_planb/20260110-022212/build_out/smoke_test.log`（报告同目录）；自检黄金 raw/health-first/plan-a/plan-b 全 PASS，日志 `out/artifacts/batch_raw/20260110-022514/build_out/smoke_test.log`、`out/artifacts/batch_health/20260110-022750/build_out/smoke_test.log`、`out/artifacts/batch_plan/20260110-023009/build_out/smoke_test.log`、`out/artifacts/batch_planb/20260110-023227/build_out/smoke_test.log`。
...
- 覆盖验证：
  ...
  - 批量策略黄金（raw/health-first/plan-a/plan-b）：无告警，PASS，日志分别见 `out/artifacts/batch_raw/20251231-203950/build_out/smoke_test.log`、`out/artifacts/batch_health/20251231-204330/build_out/smoke_test.log`、`out/artifacts/batch_plan/20251231-204730/build_out/smoke_test.log`、`out/artifacts/batch_planb/20251231-205124/build_out/smoke_test.log`（各自 golden_report 同目录）。
  - 自检黄金（raw/health-first/plan-a/plan-b，`--selftest-force-suspicious 8.8.8.8`，含 `--selftest-workbuf` + `-DWC_WORKBUF_ENABLE_STATS`）：无告警，PASS，日志 `out/artifacts/batch_raw/20251231-205426/build_out/smoke_test.log`、`out/artifacts/batch_health/20251231-205630/build_out/smoke_test.log`、`out/artifacts/batch_plan/20251231-205841/build_out/smoke_test.log`、`out/artifacts/batch_planb/20251231-210058/build_out/smoke_test.log`（stderr 含 `[WORKBUF]`/`[WORKBUF-STATS]`）。
...
- **2025-12-25（title/grep/fold 全链路复用工作缓冲 + 全矩阵复跑）**  
  - 代码：新增 `wc_workbuf` 模块；title/grep/fold 提供 `_wb` 接口并复用查询级 workbuf，`wc_apply_response_filters`/batch/single 调用链改为全程零拷贝写入同一缓冲；旧 API 兼容保留。  
  - 验证（均无告警，全部 PASS）：
    1) 默认远程冒烟 + 黄金：[out/artifacts/20251225-114602/build_out/smoke_test.log](out/artifacts/20251225-114602/build_out/smoke_test.log)；
    2) `--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`：[out/artifacts/20251225-114822/build_out/smoke_test.log](out/artifacts/20251225-114822/build_out/smoke_test.log)；
    3) 批量 raw/health-first/plan-a/plan-b `[golden] PASS`：raw [out/artifacts/batch_raw/20251225-115011/build_out/smoke_test.log](out/artifacts/batch_raw/20251225-115011/build_out/smoke_test.log)，health-first [out/artifacts/batch_health/20251225-115232/build_out/smoke_test.log](out/artifacts/batch_health/20251225-115232/build_out/smoke_test.log)，plan-a [out/artifacts/batch_plan/20251225-115454/build_out/smoke_test.log](out/artifacts/batch_plan/20251225-115454/build_out/smoke_test.log)，plan-b [out/artifacts/batch_planb/20251225-115718/build_out/smoke_test.log](out/artifacts/batch_planb/20251225-115718/build_out/smoke_test.log)；
...
#### 冻结期守护清单（执行版，2026-02-18）

- 规则 1（变更准入）：`src/core/lookup_exec_tail.c` 仅允许 bugfix、诊断可观测性、小范围命名修正；禁止新增“单层纯转发/常量返回”包装函数。
- 规则 2（提交门槛）：凡改动 tail 执行链路（pre/post/pipeline/handle 任一段），必须使用固定远程命令完成 `x86_64+win64` + `lto-auto` + smoke+golden 全量门禁。
...
- 1) 先拉取最新主线并确认工作区干净：`git pull --ff-only`、`git status --short`。
- 2) 复跑固定门禁（`x86_64+win64`、`lto-auto`、smoke+golden），记录新 artifact 路径并核对 `Local hash verify`、`[golden]`、`Golden check`。
- 3) 对“弱证据 + referral 可信度分级”做一次样例抽查（重点：`158.60.0.0/16`、`45.113.52.0/22`、`8.8.0.0/16`），确认链路稳定且权威尾行符合预期。
- 4) 复跑矩阵：`redirect_matrix_10x6` 与参数化 `redirect_matrix_test`（`-RirIpPref none -PreferIpv4 false`），确认 `authMismatchFiles=0` 且无新增系统性 errors。
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-07-15 ~ 2026-07-22，串行第 11 份，Checklist A，已完成回填）**：
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-07-23 ~ 2026-07-30，串行第 12 份，Checklist B，已完成回填）**：
...
**对应任务启动文件（2026-04-22，已回填）**：
- 路径：`tmp/unattended_ab_start_20260422-2300.md`
- 绑定文件：
...
**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-07-31 ~ 2026-08-07，串行第 13 份，Checklist A，草案，已放弃）**：

> 绑定启动文件：`tmp/unattended_ab_start_20260425-2300.md`。

**放弃结论回填（2026-04-25）**：
1. [x] D1 判定 `D-NOP`，后续 D2~D4 虽进入 `EXECUTE`，但 code-step 证据为 `already-applied`。
2. [x] `summary_partial.csv` 显示未形成有效源码增量，无法满足“本轮真实改动”目标。
3. [x] 已执行人工收口并转入“重起草 + 重生成”流程（不再复用本草案）。

**证据锚点**：
- `out/artifacts/dev_verify_multiround/20260425-032557/summary_partial.csv`
- `out/artifacts/dev_verify_multiround/20260425-032557/D4_code_step.log`


**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-08-08 ~ 2026-08-15，串行第 14 份，Checklist B，草案，已放弃）**：

> 绑定启动文件：`tmp/unattended_ab_start_20260425-2300.md`。

**放弃结论回填（2026-04-25）**：
1. [x] Checklist B 依赖 Checklist A 的有效源码改动与成功快照。
2. [x] Checklist A 本轮被判定为“无源码增量并放弃”，因此 B 不具备开跑前提。
3. [x] 串行第 14 份草案归档保留，不执行。


**对应任务启动文件（2026-04-25，已回填：本轮放弃）**：
...
- 终态字段：`A_FINAL_STATUS=BLOCKED`、`B_FINAL_STATUS=BLOCKED`、`SESSION_FINAL_STATUS=BLOCKED`。
- 放弃原因：`no-source-delta-and-already-applied`。
- 关键锚点：
  - `run_dir=out\artifacts\dev_verify_multiround\20260425-032557`
  - `summary_partial=out\artifacts\dev_verify_multiround\20260425-032557\summary_partial.csv`
  - `d4_log=out\artifacts\dev_verify_multiround\20260425-032557\D4_code_step.log`
  - `stop_evidence=out\artifacts\ab_manual_stop\20260425_041502`


**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-08-16 ~ 2026-08-22，串行第 15 份，Checklist A，已完成回填）**：

> 注：本清单为替代性重起草版本，替换旧第 13 份草案。
> 绑定启动文件：`testdata/unattended_start/active/unattended_ab_start_20260504-1123.md`。
> 回填状态：2026-05-17 已完成（A 轮 `rounds_total=8`、`rounds_pass=8`、`result=pass`）。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在上一串行批次收口后启动，A 期间禁止并发跑 B。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260816_20260822.json`。
3. [x] Reset 策略固定：A 使用 `-ResetCodeStepState -CodeStepResetPolicy restore-source`。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
...
**开发四轮（D1~D4，执行目标）**：
1. [x] D1：新增 confidence token-level helper，统一 confidence code/rank 解析。
2. [x] D2：新增 special tuple helper，统一 v4/v6 special setter 赋值路径。
3. [x] D3：新增 v4 分支 tuple helper，收敛 future-use/private/loopback/link-local/multicast 分支。
4. [x] D4：新增 v6 分支 special helper，并统一 global-unicast confidence literal 路径。

**复检四轮（V1~V4）**：
1. [x] V1 基线复检：`EXECUTE + RoundPass=True`。
2. [x] V2 噪声窗口复检：记录 skip/no-op 分类证据并保持 `RoundPass=True`。
3. [x] V3 混合样本复检：固定查询集 `64.6.64.6 103.53.144.0/22 2620:fe::fe`。
4. [x] V4 收口复检：目标 `rounds_total=8`、`rounds_pass=8`、`result=pass`。

**执行回填（Checklist A，2026-05-17）**：
- 任务定义文件：`testdata/autopilot_code_step_tasks_20260816_20260822.json`
- 执行目录：`out/artifacts/dev_verify_multiround/20260517-041528`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 均为 `EXECUTE + RoundPass=True`。

**任务定义文件（已执行）**：
- `testdata/autopilot_code_step_tasks_20260816_20260822.json`


**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-08-23 ~ 2026-08-29，串行第 16 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（新第 15 份）`result=pass` 且固化 A 成功快照后启动。
> 绑定启动文件：`testdata/unattended_start/active/unattended_ab_start_20260504-1123.md`。
> 回填状态：2026-05-17 已完成（B 轮 `rounds_total=8`、`rounds_pass=8`、`result=pass`）。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在 Checklist A `result=pass` 后启动，禁止并发。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260823_20260829.json`。
3. [x] Reset 策略固定：B 使用 `-ResetCodeStepState -CodeStepResetPolicy state-only`。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
...
**开发四轮（D1~D4，执行目标）**：
1. [x] D1：新增 token helper，统一 match-layer 与 decision-action 的空值/等值判断。
2. [x] D2：将 route-change 与 normalize 分支判断统一到 token helper。
3. [x] D3：新增 unknown-class 与 guessed-rir 判定 helper，统一 v4/v6 hint 条件路径。
4. [x] D4：新增 unknown-hint tuple setter，统一 unknown fallback 赋值并收敛 allocated literal 路径。

**复检四轮（V1~V4）**：
1. [x] V1 基线复检：`EXECUTE + RoundPass=True`。
2. [x] V2 噪声窗口复检：允许 `V-SKIP`，但需保留 `SkipReason` 且 `RoundPass=True`。
3. [x] V3 混合样本复检：`EXECUTE + RoundPass=True`。
4. [x] V4 收口复检：目标 `rounds_total=8`、`rounds_pass=8`、`result=pass`。

**执行回填（Checklist B，2026-05-17）**：
- 任务定义文件：`testdata/autopilot_code_step_tasks_20260823_20260829.json`
- 执行目录：`out/artifacts/dev_verify_multiround/20260517-075336`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 均为 `EXECUTE + RoundPass=True`。
- 会话终态：`out/artifacts/ab_supervisor/20260517-041534/live_status.json` 为 `status=pass`、`session_final_status=PASS`。

**任务定义文件（已执行）**：
- `testdata/autopilot_code_step_tasks_20260823_20260829.json`


**对应任务启动文件（2026-05-17，已完成回填）**：

- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20260504-1123.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20260816_20260822.json`
  - B：`testdata/autopilot_code_step_tasks_20260823_20260829.json`
- 策略重点：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`。
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_OPERATOR=Copilot`、`PRECHECK_START_GATE=READY`。
- 当前终态基线（按 live status 回填）：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 关键锚点：
  - `a_run_dir=out/artifacts/dev_verify_multiround/20260517-041528`
  - `b_run_dir=out/artifacts/dev_verify_multiround/20260517-075336`
  - `supervisor_live_status=out/artifacts/ab_supervisor/20260517-041534/live_status.json`


**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-08-30 ~ 2026-09-05，串行第 17 份，Checklist A，已完成回填）**：

> 注：本清单承接上一轮 A/B 收口结果，采用稳定命中优先策略（每个 D 轮 1 个确定性 regex-patch 操作）。
> 回填状态：2026-05-19 已完成；绑定启动文件 `testdata/unattended_start/active/unattended_ab_start_20260517-1315.md`。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在上一串行批次收口后启动，A 期间禁止并发跑 B。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260830_20260905.json`。
3. [x] Reset 策略固定：A 使用 `-ResetCodeStepState -CodeStepResetPolicy restore-source`。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=stable-high`，每个 D 轮 `minOperationsPerDRound=1`。
7. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
8. [x] 全程保持人工提交口径（`AUTO_COMMIT=0`、`AUTO_PUSH=0`），仅允许产物刷新。
...
**复检四轮（V1~V4）**：
1. [x] V1 基线复检通过（`EXECUTE + RoundPass=True`）。
2. [x] V2 噪声窗口复检通过（`RoundPass=True`）。
3. [x] V3 混合样本复检通过（`EXECUTE + RoundPass=True`）。
4. [x] V4 收口复检通过（`rounds_total=8`、`rounds_pass=8`、`result=pass`）。

**任务定义文件（已执行并回填）**：
- `testdata/autopilot_code_step_tasks_20260830_20260905.json`

**执行回填（Checklist A，2026-05-18）**：
- 执行目录：`out/artifacts/dev_verify_multiround/20260518-132102`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 全部 `EXECUTE + RoundPass=True`（8 行，`fail=0`、`skip_or_nop=0`）。


**下次开工清单（无人值守稳妥档：开发四轮 + 复检四轮，提速模式，2026-09-06 ~ 2026-09-12，串行第 18 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（串行第 17 份）`result=pass` 且固化 A 成功快照后启动。
> 回填状态：2026-05-19 已完成；绑定启动文件 `testdata/unattended_start/active/unattended_ab_start_20260517-1315.md`。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在 Checklist A `result=pass` 后启动，禁止并发。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260906_20260912.json`。
3. [x] Reset 策略固定：B 使用 `-ResetCodeStepState -CodeStepResetPolicy state-only`。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=stable-high`，每个 D 轮 `minOperationsPerDRound=1`。
7. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
8. [x] 保持 B 阶段 state-only 承接并留存 A 快照锚点。
...
**任务定义文件（已执行并回填）**：
- `testdata/autopilot_code_step_tasks_20260906_20260912.json`

**执行回填（Checklist B，2026-05-18）**：
- 执行目录：`out/artifacts/dev_verify_multiround/20260518-182148`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 全部 `EXECUTE + RoundPass=True`（8 行，`fail=0`、`skip_or_nop=0`）。


**对应任务启动文件（2026-05-19，已完成回填）**：

- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20260517-1315.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20260830_20260905.json`
  - B：`testdata/autopilot_code_step_tasks_20260906_20260912.json`
- 预检状态：`PRECHECK_STATUS=PASS`、`PRECHECK_OPERATOR=Copilot`、`PRECHECK_START_GATE=READY`。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-05-18 23:29:42`。
- 关键锚点：
  - `a_run_dir=out\artifacts\dev_verify_multiround\20260518-132102`
  - `b_run_dir=out\artifacts\dev_verify_multiround\20260518-182148`
  - `supervisor_log=out\artifacts\ab_supervisor\20260518-132601\supervisor.log`
  - `live_status=out\artifacts\ab_supervisor\20260518-132601\live_status.json`
- 状态票回填：`running-status-report` 共 30 张（首票 `T20260518-183128910-176af38b@2026-05-18 18:31:28`，末票 `T20260518-232238198-63b7b900@2026-05-18 23:22:38`）。
...
**下次开工清单（A/B 无人值守聊天分发策略 V1 收口，2026-05-25，已完成回填）**：

> 注：本清单对应 2026-05-25 最近一次 A/B 无人值守聊天分发策略收口任务；已按执行结果完成回填。

1. [x] 源键收敛落地：统一以 `AI_CHAT_POLICY_*` 驱动 stage/resume 派生键回写，不再依赖 trigger 命令硬编码 sender 开关。
2. [x] dispatch 主备链路收敛：支持 `sender_primary + sender_fallback_enabled` 解析与双向跨 sender 收底。
3. [x] trigger final gate 扩展：新增 `sender-sent` 守门（以 relay state 的 `sender_sent=true` 为 auto-stop 前置）。
4. [x] 最小联调闭环验证：先复现 `state-missing` defer，再注入 sender 确认态命中 `final_dispatch_sender_confirmed` 与 `auto_stop reason=session-final`。
5. [x] 文档回填：新增独立 RFC `docs/RFC-unattended-chat-dispatch-policy-v1.md`，并同步索引到 `docs/README.md` 与本 RFC 快速索引。
6. [x] 发布记录回填：`RELEASE_NOTES.md` 已补充 2026-05-25 中英文摘要。

**执行回填（A/B 聊天分发策略，2026-05-25）**：
- 关键证据日志：`out/artifacts/ab_agent_queue/takeover_trigger_unattended_ab_start_sender_gate_smoke.log`（`final_dispatch_sender_confirmed`、`auto_stop reason=session-final`）。
- relay state 证据：`out/artifacts/ab_agent_queue/chat_dispatch/latest_relay_unattended_ab_start_sender_gate_smoke.json`（`event=chat-session-final-status`、`sender_sent=true`）。
- 代码提交：`9199c60`（策略编译器 + 文档 RFC）、`1ce2395`（sender fallback + sender-sent gate）、`baec23e`（发布说明回填）。


**下次开工清单（无人值守高密度档：开发四轮 + 复检四轮，提速模式，2026-09-13 ~ 2026-09-20，串行第 19 份，Checklist A，已完成回填）**：

> 注：本清单参照第 17/18 份执行骨架，升级为“所有开发轮次高密度改动”，即每个 D 轮至少 2 个确定性 regex-patch 操作。
> 状态：2026-05-25 已执行并回填（`Result=pass`，`ExpectedRoundCount=8`，`CompletedRoundCount=8`）。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在串行第 18 份收口后启动，A 期间禁止并发跑 B。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260913_20260920.json`。
3. [x] D1 reset 要求固定（按任务定义）：当运行范围包含 D1 时必须显式携带 `-ResetCodeStepState`（直跑 code-step 入口可用 `-Reset` 或 `-ResetStateOnly`）。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=high`，每个 D 轮 `minOperationsPerDRound=2`（全部 D 轮高密度）。
7. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
8. [x] 全程保持人工提交口径（`AUTO_COMMIT=0`、`AUTO_PUSH=0`），仅允许产物刷新。

任务定义文件校对：`rounds` 仅定义 D1~D4（均为 `regex-patch`，且每轮 2 个 operations）；V1~V4 为门禁复检流程，不在任务定义文件中声明。

**开发四轮（D1~D4，全部高密度）**：
1. [x] D1：观测原因/置信度路径重构（2 个操作：reason prefix helper + confidence rank helper）。
2. [x] D2：match-layer 标注与 query-kind 映射重构（2 个操作：output label helper + normalized query-kind helper）。
3. [x] D3：route-change 阻断策略重构（2 个操作：enabled-for-decision helper + block-required helper）。
4. [x] D4：special-result 分发路径重构（2 个操作：special apply helper + reason guard helper）。
...
**执行回填（Checklist A，2026-05-25）**：
- 执行目录：`out/artifacts/dev_verify_multiround/20260525-070357`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 全部 `RoundPass=True`（`rows=8`、`pass=8`、`fail=0`）。
- 开发轮证据：D1~D4 均为 `CodeStepAction=applied` 且 `SourceDeltaAfterCodeStep=changed`。
- D 轮精确映射：`wc_preclass_reason_has_observe_prefix`、`wc_preclass_confidence_rank_from_level`、`wc_preclass_match_layer_output_label`、`wc_preclass_match_layer_from_query_kind_normalized`、`wc_preclass_route_change_enabled_for_decision`、`wc_preclass_route_change_block_required`、`wc_preclass_apply_special_result`、`wc_preclass_special_result_has_reason`。


**下次开工清单（无人值守高密度档：开发四轮 + 复检四轮，提速模式，2026-09-21 ~ 2026-09-28，串行第 20 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（串行第 19 份）`result=pass` 且 A 成功快照固化后启动，保持 B 的 state-only 承接策略。
> 状态：2026-05-25 已执行并回填（`Result=pass`，`ExpectedRoundCount=8`，`CompletedRoundCount=8`）。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在 Checklist A `result=pass` 后启动，禁止并发。
2. [x] 任务定义文件固定：`testdata/autopilot_code_step_tasks_20260921_20260928.json`。
3. [x] D1 reset 要求固定（按任务定义）：当运行范围包含 D1 时必须显式携带 `-ResetCodeStepState`（直跑 code-step 入口可用 `-Reset` 或 `-ResetStateOnly`）。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=high`，每个 D 轮 `minOperationsPerDRound=2`（全部 D 轮高密度）。
...
**开发四轮（D1~D4，全部高密度）**：
1. [x] D1：v4 reserved/v6 unknown tuple 抽象（2 个操作：reserved tuple helper + v6 reason helper）。
2. [x] D2：unknown-hint 默认与 allocated-hint 应用重构（2 个操作：default reason helper + allocated hint apply helper）。
3. [x] D3：decision action 归一化拆分（2 个操作：missing helper + fallback helper）。
4. [x] D4：default non-ip 标签路径重构（2 个操作：default input label helper + default match-layer helper）。
...
**执行回填（Checklist B，2026-05-25）**：
- 执行目录：`out/artifacts/dev_verify_multiround/20260525-115550`
- 汇总结论：`final_status.json` 显示 `Result=pass`、`ExpectedRoundCount=8`、`CompletedRoundCount=8`、`ExitCode=0`。
- 轮次结果：`summary.csv` 显示 D1~D4、V1~V4 全部 `RoundPass=True`（`rows=8`、`pass=8`、`fail=0`）。
- 开发轮证据：D1~D4 均为 `CodeStepAction=applied` 且 `SourceDeltaAfterCodeStep=changed`。
- D 轮精确映射：`wc_preclass_set_reserved_none_tuple`、`wc_preclass_v6_unknown_hint_reason_literal`、`wc_preclass_unknown_hint_default_reason_literal`、`wc_preclass_apply_allocated_hint_from_rir`、`wc_preclass_decision_action_missing`、`wc_preclass_apply_decision_none_fallback`、`wc_preclass_default_non_ip_label`、`wc_preclass_default_match_layer_non_ip`。


**对应任务启动文件（2026-05-25，已执行回填）**：
...
- 预检状态：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`、`PRECHECK_AT=2026-05-25 07:02:47`。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-05-25 16:59:40`。
...
**下次开工清单（无人值守高密度档：开发四轮 + 复检四轮，提速模式，2026-09-29 ~ 2026-10-06，串行第 21 份，Checklist A，已完成回填）**：
...
> 回填状态：2026-06-02 已按本期 A/B 会话结论完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在串行第 20 份已收口且会话状态稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时必须显式携带 `-ResetCodeStepState`（直跑 code-step 入口可用 `-Reset` 或 `-ResetStateOnly`）。
...
5. [x] 开发轮密度固定：`dRoundChangeDensity=high`，每个 D 轮 `minOperationsPerDRound=2`。
6. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
7. [x] 保持人工提交口径：`AUTO_COMMIT=0`、`AUTO_PUSH=0`，仅允许产物刷新。
8. [x] 保持 A 失败阻断 B：`A_FAILURE_BLOCKS_B=true`。

**开发四轮（D1~D4，全部高密度）**：
1. [x] D1：non-ip 默认与 v4 private tuple 抽象（5 个操作：`wc_preclass_set_non_ip_defaults`、`wc_preclass_set_v4_private_range_result` 及三处调用替换）。
2. [x] D2：v6 loopback 与 unique-local 分支 helper 对齐（3 个操作：`wc_preclass_set_v6_loopback_result` + `wc_preclass_set_v6_branch_special_result` 路径替换）。
3. [x] D3：v4/v6 reason literal helper 引入（5 个操作：`wc_preclass_v4_limited_broadcast_reason_literal`、`wc_preclass_v4_this_network_reason_literal`、`wc_preclass_v4_link_local_reason_literal`、`wc_preclass_v6_loopback_reason_literal` + 调用替换）。
4. [x] D4：private reason literal 收口与 special tuple 复用（4 个操作：`wc_preclass_v4_private_172_reason_literal`、`wc_preclass_v4_private_192_reason_literal`、`wc_preclass_set_v4_private_range_result` 复用 `wc_preclass_set_special_tuple`）。
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段已完成，且与会话总收口结论一致：`A_FINAL_STATUS=PASS`。
- 运行链路保持稳定：状态票接管、continue_watch/business_resume 分流、心跳写入均可持续执行。
- 关键口径：本轮 IPC 投送稳定，未观察到会话级阻塞异常。


**下次开工清单（无人值守高密度档：开发四轮 + 复检四轮，提速模式，2026-10-07 ~ 2026-10-14，串行第 22 份，Checklist B，已完成回填）**：
...
2. [x] D1 reset 要求固定：运行范围包含 D1 时必须显式携带 `-ResetCodeStepState`。
3. [x] Reset 策略固定：B 使用 `-CodeStepResetPolicy state-only`。
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=high`，每个 D 轮 `minOperationsPerDRound=2`。
7. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
8. [x] 保持 B 阶段 `A_SUCCESS_SNAPSHOT_*` 锚点可追溯。

**开发四轮（D1~D4，全部高密度）**：
1. [x] D1：decision default 赋值抽象（2 个操作：`wc_preclass_set_decision_defaults` 提取 + 默认赋值调用替换）。
2. [x] D2：query 归一化匹配路径抽象（2 个操作：`wc_preclass_normalized_query_for_match` 提取 + `normalized` 赋值调用替换）。
3. [x] D3：decision-action 与 route-change finalize helper 收口（3 个操作：`wc_preclass_should_apply_decision_action`、`wc_preclass_apply_route_change_finalize` + 两处调用替换）。
4. [x] D4：query-missing 与 preclass-disabled 判定归一（3 个操作：`wc_preclass_query_missing`、`wc_preclass_preclass_disabled_enabled` + 判定调用替换）。
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段已完成，且与会话总收口结论一致：`B_FINAL_STATUS=PASS`。
- 会话终态：`SESSION_FINAL_STATUS=PASS`，并已收口关闭。
- 关键终态证据（本期会话）：`B stage_final result=pass exit_code=0`，终态时间戳 `2026-06-01 23:30:16`；会话关闭时间 `2026-06-01 23:30:25`。


**对应任务启动文件（2026-06-02，已执行回填）**：
...
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-06-01 23:30:25`。
- 本期运行锚点：`run_dir=out\artifacts\dev_verify_multiround\20260601-181337`。
- Strict 远程链路锚点：`strict_run_dir=out\artifacts\20260602-022441`（`lto-auto`，远程编译+冒烟+同步+黄金+重定向校验）。
- 本期链路结论映射：证据键 `AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`、`run_dir=out\artifacts\dev_verify_multiround\20260601-181337`、`running-status-report` 持续接管、`chat_session_heartbeat` 连续更新、IPC 高优先级投送事件驱动票持续可达 -> 结论：IPC 消息投送链路的建立，使无人值守任务运行通过脚本与 AI 之间稳定可靠的对话，收敛为稳定可靠闭环。
...
**本期运行结论（回填到对应清单）**：
1. [x] 终态收口映射：证据键 `A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`、`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass` -> 结论：本期 A/B/SESSION 已按预期收口并完成会话关闭。
2. [x] 链路稳定映射：证据键 `AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`、`run_dir=out\artifacts\dev_verify_multiround\20260601-181337`、`running-status-report` 持续接管、`chat_session_heartbeat` 连续更新、IPC 高优先级投送事件驱动票持续可达 -> 结论：IPC 消息投送链路的建立，使无人值守任务在状态票/事件票协同下，通过脚本与 AI 之间稳定可靠的对话，收敛为稳定可靠闭环。
3. [x] 模式策略映射：证据键 `RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`（本期生产链路） -> 结论：生产继续以 Visible 为主；动态模型与 thinking effort 的运行中切换保持仅在 Silent 路径治理，后续按计划推进 Silent 工作脚本改造。
4. [x] Strict 远程校验映射：证据键 `strict_run_dir=out\artifacts\20260602-022441`、`lto warning=0`、`Local hash verify=PASS`、`Golden=PASS`、`referral check=PASS`、`duration=338s` -> 结论：Strict 远程构建冒烟同步与黄金/重定向链路全通过，当前发布门禁状态稳定。
...
**下次开工清单（无人值守更高密度档：开发四轮 + 复检四轮，提速模式，2026-10-15 ~ 2026-10-22，串行第 23 份，Checklist A，已完成回填）**：
...
> 回填状态：2026-06-04 已按本期 A/B 会话结论与补充验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 22 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=4`。
6. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
7. [x] 保持人工提交口径：`AUTO_COMMIT=0`、`AUTO_PUSH=0`。
8. [x] 保持 A 失败阻断 B：`A_FAILURE_BLOCKS_B=true`。

**开发四轮（D1~D4，更高密度）**：
1. [x] D1：non-ip 默认与 v4 private tuple 抽象（5 个操作）。
2. [x] D2：v6 loopback/link-local/unique-local 分支 helper 收口（4 个操作）。
3. [x] D3：v4/v6 reason literal helper 引入与调用替换（5 个操作）。
4. [x] D4：private reason literal 收口与 special tuple 复用（4 个操作）。
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段已完成并满足 B 启动前提，最终会话与本清单收口口径一致：`A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话级运行链路保持稳定：`running-status-report` 接管、自愈分流（`business_resume -> continue_watch_command`）、`chat_session_heartbeat write_ok=true` 均正常。
- 本期补充验证已覆盖 Strict 远程构建冒烟同步 + 黄金、debug/metrics 变体、Batch Golden 四策略、Selftest Golden 四策略与 Redirect Matrix 12x6，未见新的契约偏移。


**下次开工清单（无人值守更高密度档：开发四轮 + 复检四轮，提速模式，2026-10-23 ~ 2026-10-30，串行第 24 份，Checklist B，已完成回填）**：
...
4. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
5. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
6. [x] 开发轮密度固定：`dRoundChangeDensity=high`，每个 D 轮 `minOperationsPerDRound=3`。
7. [x] 轮次范围固定：`-StartRound 1 -EndRound 8`（D1~D4 + V1~V4）。
8. [x] 保持 B 阶段 `A_SUCCESS_SNAPSHOT_*` 锚点可追溯。

**开发四轮（D1~D4，更高密度）**：
1. [x] D1：decision default 赋值抽象 + null-guard 归一（3 个操作）。
2. [x] D2：query 归一化匹配路径抽象 + match-layer helper（3 个操作）。
3. [x] D3：decision-action 与 route-change finalize helper 收口（3 个操作）。
4. [x] D4：query-missing 与 preclass-disabled 判定归一（3 个操作）。
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段完成后以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status-pass` 正常关闭。
- 终态关键锚点：`run_dir=out/artifacts/dev_verify_multiround/20260603-130542`、`main_round=B/D4`、`B exit=0`，supervisor/companion/guard 终态链路收口正常。
- 本期额外验证继续覆盖 Strict 远程构建冒烟同步 + 黄金、Step47 preclass preflight/table guard、四策略 Batch Golden 与四策略 Selftest Golden，结果均为 PASS。


**对应任务启动文件（2026-06-04，已完成回填）**：
...
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-06-03 17:53:35`。
- 关键锚点：`run_dir=out/artifacts/dev_verify_multiround/20260603-130542`、`main_round=B/D4`、`supervisor_log=out/artifacts/ab_supervisor/20260603-130556/supervisor.log`、`live_status=out/artifacts/ab_supervisor/20260603-130556/live_status.json`。
...
**本期额外验证（2026-06-04）**：
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260604-001805`，`Local hash verify=PASS`、`Golden=PASS`、`referral check=PASS`，无 warning / 无 LTO warning，`duration=183s`。
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，透传 preflight + table guard）：`out/artifacts/20260604-002356`，在默认 PASS 基础上新增 `Step47 preclass preflight=PASS`、`Preclass table guard=PASS`，`duration=918s`。
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto` + `--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）：`out/artifacts/20260604-004605`，`Local hash verify=PASS`、`Golden=PASS`、`referral check=PASS`，无 warning / 无 LTO warning，`duration=184s`。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-10-31 ~ 2026-11-07，串行第 25 份，Checklist A，已完成回填）**：
...
> 状态：已回填（A 阶段 D1~D4 与 V1~V4 已完成；A -> B 因脚本缺陷未在同一日期连续执行，修复后转入分阶段收口并成功完成 A 阶段目标）。

**本期回填（2026-06-14，25A 实际执行）**：
1. [x] A 阶段开发四轮完成：D1~D4 全部执行并收口。
2. [x] A 阶段复检四轮完成：V1~V4 全部执行并通过。
3. [x] A -> B 连续执行过程中暴露脚本缺陷，未能同日串行衔接；缺陷已定位并修复。
4. [x] 修复后改为分阶段推进，A 阶段执行链条完整闭环，产出与状态满足无人值守收口要求。
5. [x] 为 B 阶段衔接完成必要脚本改进与链路完善，确保后续无人值守继续执行。

**轮次执行情况（25A）**：
1. [x] D1~D4 已完成。
2. [x] V1~V4 已完成。
3. [x] 轮次任务内容调整：A -> B 由“同日连续”改为“缺陷修复后分阶段执行”，但 A 阶段目标完整达成。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 24 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Extract direct v4/v6 special reason literals into dedicated helpers and replace hot-path string uses.
2. [x] D2：Introduce v4 branch special-result helper and route common v4 hot-path calls through it.
3. [x] D3：Extract known/unknown RIR tuple application helpers for global-unicast and allocated-hint paths.
4. [x] D4：Extract hot-path predicate helpers for inline v4/v6 special-branch conditions.
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-11-08 ~ 2026-11-15，串行第 26 份，Checklist B，已完成回填）**：
...
> 状态：已回填（26B 分阶段完成 D1~D4 与 V1~V4；B/D4 发生两次自愈修复且任务定义已随过程调整，最终在无人值守状态下成功收口并达成目标）。

**本期回填（2026-06-14，26B 实际执行）**：
1. [x] B 阶段开发四轮完成：D1~D4 全部执行并收口。
2. [x] B 阶段复检四轮完成：V1~V4 全部执行并通过。
3. [x] B/D4 轮次出现两次自愈修复，均在无人值守流程内完成并恢复通过。
4. [x] 执行过程中任务定义按实际问题进行了增删改并重新生效，最终产出物源代码修复成功。
5. [x] 尽管 A/B 未同日连续自动化串跑，但两阶段执行过程均完整且目标全部达成。
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Add entry helpers for query-missing, match-layer update and input-label projection.
2. [x] D2：Introduce reusable unknown-hint helpers for default, v4 and v6 hint paths.
3. [x] D3：Extract generic classified-tuple helper and route special/reserved/unknown tuple writers through it.
4. [x] D4：引入命名 special-result helper 并将 v4/v6 结果 helper 路由到该路径（5 个操作：新增 `wc_preclass_apply_v4_named_special_result`、`wc_preclass_apply_v6_named_special_result`，并改造 `set_v4_special_result`、`set_v4_private_range_result`、`set_v6_branch_special_result`、`set_v6_loopback_result` 调用链；含两次自愈修复）。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20261031_20261107.json
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20261108_20261115.json
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-11-16 ~ 2026-11-22，串行第 27 份，Checklist A，已完成回填）**：
...
> 回填状态：2026-08-04 已按本期 A/B 会话结论与收尾验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 26 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=5`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Extract multicast reason literals into dedicated helpers and replace hot-path inline call-site uses.
2. [x] D2：Introduce decision-action comparison helpers and replace inline observe/hint/disabled token checks.
3. [x] D3：Consolidate confidence token matching into reusable helper predicates and replace inline string comparisons.
4. [x] D4：Extract inline action category comparison helpers and replace hardcoded token checks in route-change policy.
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段已完成并满足 B 启动前提，最终 `A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- A 运行目录：`out/artifacts/dev_verify_multiround/20260803-193007`，8/8 轮（D1-D4 + V1-V4）全部通过，`final_status.json` 于 2026-08-04 01:32:36 生成。
- 期间 1 起 task-static 自愈：A-D1 `task-definition-mismatch`（ticket `T20260803-190723028`），经跨轮 D1-D4 候选事务修复后原子提升，A 自项目基线重启并全绿。


**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-11-23 ~ 2026-11-30，串行第 28 份，Checklist B，已完成回填）**：
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Add rule-table lookup helpers and route inline row-assignment logic through them.
2. [x] D2：Introduce reusable CIDR/non-CIDR query-kind resolution helpers and replace inline parse-cidr result checks.
3. [x] D3：Extract inline decision action mapping helpers and replace duplicated action-category checks.
4. [x] D4：Consolidate remaining route-change block-apply helpers and replace inline conditional policy path.
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段完成后以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status` 正常关闭（`SESSION_CLOSED_AT=2026-08-04 06:40:15`）。
- B 运行目录：`out/artifacts/dev_verify_multiround/20260804-015535`，8/8 轮全部通过，`B exit=0`，`final_status.json` 于 2026-08-04 06:39:17 生成。
- 期间 1 起 task-static 自愈：B-D1 `task-definition-mismatch`（ticket `T20260804-013737343`），经跨轮 D1-D4 候选事务修复后原子提升，B 自 A 成功快照重启并全绿。


**对应任务启动文件（2026-08-04，已完成回填）**：
...
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status`、`SESSION_CLOSED_AT=2026-08-04 06:40:15`。
- 关键锚点：`A run_dir=out/artifacts/dev_verify_multiround/20260803-193007`、`B run_dir=out/artifacts/dev_verify_multiround/20260804-015535`、`main_round=B/D4`、`live_status=out/artifacts/ab_session_guard/20260803-190316/live_status.json`。
- 阶段耗时：`A elapsed=0d 06:32:00（start 2026-08-03 19:02:32 / end 2026-08-04 01:34:32）`、`B elapsed=0d 05:05:50（start 20260804-013425 / end 2026-08-04 06:40:15）`、`A/B total=0d 11:37:43`。

**任务定义静态体检（已完成）**：
- [x] A/B 正式任务定义经运行期候选事务修复后均原子提升（Promote），`validated_rounds=[D1,D2,D3,D4]`，promotion receipt 与正式文件哈希校验通过。
- [x] 修复后对正式文件从故障轮执行 `-ChainRounds` 严格复检：`errors=0 warnings=0`。

**本期额外验证（2026-08-04，收尾）**：
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260804-101426`（修复前，`有告警`：7 条未使用 static 函数 warning，构建仍 PASS、`Golden PASS`、`referral check PASS`，`duration=283s`）。
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260804-103402`（修复后），`无告警 + lto 无告警 + Local hash verify=PASS + Golden PASS + referral check=PASS`，`duration=280s`。
...
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20261201_20261207.json`
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-12-16 ~ 2026-12-22，串行第 31 份，Checklist A，已完成回填）**：
...
> 回填状态：2026-08-06 已按本期 A/B 会话结论与收尾验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 30 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=5`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Route remaining inline V6 reason-name literals (V6_UNIQUE_LOCAL_FC00_7, V6_LINK_LOCAL_FE80_10, V6_MULTICAST_FF00_8) through existing literal helpers with forward declarations before reason_name.
2. [x] D2：Consolidate duplicate v4 special-result setter wrappers by removing branch and private-range setters and routing their call sites through the single v4 special-result setter.
3. [x] D3：Consolidate v6 special-result setters by removing the dedicated loopback setter and routing its call site through the single v6 branch special-result setter.
4. [x] D4：Unify the v6 loopback predicate to byte-array style consistent with other v6 predicates and update its call site.
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段一次通过并满足 B 启动前提，最终 `A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- A 运行目录：`out/artifacts/dev_verify_multiround/20260805-161948`，8/8 轮（D1-D4 + V1-V4）全部通过，`final_status.json` 于 2026-08-05 22:02:24 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2026-12-23 ~ 2026-12-31，串行第 32 份，Checklist B，已完成回填）**：
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Consolidate the v4 reserved-future-use setter chain by removing the reserved-none-tuple middle layer and routing directly to the none-confidence tuple setter.
2. [x] D2：Remove the duplicate unknown-hint reason helper and route its call site to the base no-rir-hint value literal.
3. [x] D3：Remove the rir-hint/no-rir-hint value wrapper literals and route their call sites through the base reason literal helpers.
4. [x] D4：Remove the rir-unknown-hint wrapper literal and route the compare literal directly to the base rir unknown literal.
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段一次通过并以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status` 正常关闭（`SESSION_CLOSED_AT=2026-08-06 03:56:31`）。
- B 运行目录：`out/artifacts/dev_verify_multiround/20260805-220426`，8/8 轮全部通过，`B exit=0`，`final_status.json` 于 2026-08-06 03:55:46 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`（A/B 阶段 launch-ready 均 PASS）。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-08-06 03:56:31`。
- 关键锚点：`A run_dir=out/artifacts/dev_verify_multiround/20260805-161948`、`B run_dir=out/artifacts/dev_verify_multiround/20260805-220426`、`main_round=B/D4`、`guard_log=out/artifacts/ab_session_guard/20260805-161958/guard.log`、`live_status=out/artifacts/ab_session_guard/20260805-161958/live_status.json`。
- 阶段耗时：`A elapsed=0d 05:45:17（start 2026-08-05 16:19:16 / end 2026-08-05 22:04:33）`、`B elapsed=unknown（B start 未记录 / end 2026-08-06 03:56:31）`、`A/B total=0d 11:37:16`。

**运行期票据（本期）**：
- [x] `a-pass-conclusion-b-started`（ticket `T20260805-220433408-075c2fdd`）：经唯一恢复事务原子闭环（handled 2026-08-05 22:06:36）。
- [x] `chat-session-final-status`（ticket `chat-final-20260806-035631`）：经唯一恢复事务原子闭环（handled 2026-08-06 03:57:37）。

**任务定义静态体检（本期）**：
- [x] 运行期 0 起 task-static 事故，A/B 正式任务定义无需运行期修复；编制期全定义静态检查（A 全定义 / B 以 A 为前置链式）`errors=0 warnings=0` 保持有效。

**本期额外验证（2026-08-06，收尾）**：
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260806-050216`，`无告警 + lto 无告警 + Local hash verify=PASS + Golden PASS + referral check=PASS`，`duration=314s`。

**本期收尾链路改进（2026-08-06）**：
- [x] 修复无故障 B 阶段缺少启动时间锚点：`open_unattended_ab_stage_window.ps1` 在 B 主进程通过启动探测后以 `$stageLaunchTime` 原子写入 `B_TASK_FIRST_START_AT`（仅字段为空时写入，重启不覆盖）；模板新增该字段并纳入 A 新会话 reset；`status_ticket_mini_regression.ps1 -ContractGateOnly` 与 `check_unattended_start_field_sync.ps1` 契约回归均 PASS（14/14），使 `chat-session-final-status` 终态票对零故障会话也能输出确定的 `B start` / `B elapsed`。


**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-01 ~ 2027-01-07，串行第 33 份，Checklist A，已完成回填）**：

> 注：本清单在第 32 份 B 的 very-high 基础上继续推进 `preclass.c` 的 special-result setter 层移除、hint wrapper 字面量移除与 reserved-future-use setter 移除。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270101_20270107.json`。
> 回填状态：2026-08-07 已按本期 A/B 会话结论与收尾验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 32 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=5`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Remove the v4 special-result setter wrapper and route all 8 v4 call sites in `wc_preclass_classify_ip` directly through the base `wc_preclass_set_special_tuple` setter.
2. [x] D2：Remove the v6 special-result setter wrapper and the now-unneeded `wc_preclass_set_special_tuple` forward declaration, routing all 5 v6 call sites directly through `wc_preclass_set_special_tuple`.
3. [x] D3：Remove the four hint-wrapper literals (class/confidence hint wrappers) and route their call sites directly through the base class/confidence literals.
4. [x] D4：Remove the v4 reserved-future-use setter and its reason literal wrapper, routing the single call site directly through `wc_preclass_apply_none_confidence_tuple` with the preserved class-special reason value.
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段一次通过并满足 B 启动前提，最终 `A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- A 运行目录：`out/artifacts/dev_verify_multiround/20260806-065714`，8/8 轮（D1-D4 + V1-V4）全部通过，`final_status.json` 于 2026-08-06 12:05:47 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。

**编制期验收（已完成，2026-08-06）**：
- [x] `-SyntaxOnly` 装载检查：PASS。
- [x] 全定义静态检查（无 RoundTag）：`errors=0 warnings=0`，D1~D4 全部 op 唯一命中、marker 自有、replacement 收敛、replay 稳定、断言精确。
- [x] 专项安全回归 `task_definition_safety_regression.ps1`：全 case PASS。
- [x] 链式检查 `-RoundTag D1 -ChainRounds`：`errors=0 warnings=0`。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-08 ~ 2027-01-15，串行第 34 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（串行第 33 份）`result=pass` 且 A 成功快照固化后启动；保持 `state-only` 承接策略，重点放在 match-layer 字面量去重、default/disabled decision wrapper 层移除与 route-change wrapper 层移除。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270108_20270115.json`。
> 回填状态：2026-08-07 已按本期 A/B 会话结论与收尾验证结果完成回填。
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Deduplicate the match-layer cidr/ip literal pair by routing `wc_preclass_match_layer_cidr_literal`/`wc_preclass_match_layer_ip_literal` directly through the compare literals and removing the cidr/ip output wrapper literals plus the input-label unknown wrapper literal.
2. [x] D2：Remove the default-decision wrapper layer by routing the six `wc_preclass_set_decision_defaults` call sites directly through the base literals/helpers and deleting the default_action, default_action_source, default_match_layer, default_fallback_reason, default_input_label, default_route_change, default_action_literal, default_action_source_literal, default_non_ip_label, default_match_layer_non_ip and normalize_action_source wrappers.
3. [x] D3：Remove the disabled-decision wrapper layer by routing the four `wc_preclass_apply_disabled_decision_fields` call sites directly through the base literals/helpers and deleting the hint_disabled_action_literal, hint_disabled_action_source, disabled_fallback_reason, disabled_route_change_reset and policy_action_source wrappers.
4. [x] D4：Remove the route-change wrapper layer by routing the `wc_preclass_route_change_policy_apply` and `wc_preclass_apply_route_change_finalize` call sites directly through the base helpers and deleting the route_change_block_reset, route_change_fallback_apply, apply_route_change_value and apply_route_change_policy wrappers.
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段一次通过并以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status` 正常关闭（`SESSION_CLOSED_AT=2026-08-06 17:48:16`）。
- B 运行目录：`out/artifacts/dev_verify_multiround/20260806-120749`，8/8 轮全部通过，`B exit=0`，`final_status.json` 于 2026-08-06 17:47:28 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
**对应任务启动文件（2026-08-07，已完成回填）**：

- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20270101-20270115.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20270101_20270107.json`
  - B：`testdata/autopilot_code_step_tasks_20270108_20270115.json`
- 当前窗口：`WINDOW=2027-01-01 ~ 2027-01-15`
- 当前策略基线：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`、`AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`。
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`（A/B 阶段 launch-ready 均 PASS）。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-08-06 17:48:16`。
- 关键锚点：`A run_dir=out/artifacts/dev_verify_multiround/20260806-065714`、`B run_dir=out/artifacts/dev_verify_multiround/20260806-120749`、`main_round=B/D4`、`guard_log=out/artifacts/ab_session_guard/20260806-065724/guard.log`、`live_status=out/artifacts/ab_session_guard/20260806-065724/live_status.json`。
- 阶段耗时：`A elapsed=0d 05:11:14（start 2026-08-06 06:56:42 / end 2026-08-06 12:07:55）`、`B elapsed=0d 05:40:58（start 2026-08-06 12:07:18 / end 2026-08-06 17:48:16）`、`A/B total=0d 10:51:34`。

**运行期票据（本期）**：
- [x] `a-pass-conclusion-b-started`（ticket `T20260806-120755804-498994bf`）：经唯一原子收尾闭环（handled 2026-08-06 12:09:21）。
- [x] `chat-session-final-status`（ticket `chat-final-20260806-174816`）：经唯一原子收尾闭环（handled 2026-08-06 17:49:24）。

**任务定义静态体检（本期）**：
- [x] 运行期 0 起 task-static 事故，A/B 正式任务定义无需运行期修复；编制期全定义静态检查（A 全定义 / B 以 A 为前置链式）`errors=0 warnings=0` 保持有效。

**本期额外验证（2026-08-07，收尾）**：
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260807-001732`，`无告警 + lto 无告警 + Local hash verify=PASS + Golden PASS + referral check=PASS`，`duration=293s`。


**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-16 ~ 2027-01-22，串行第 35 份，Checklist A，已完成回填）**：

> 注：本清单在第 34 份 B 的 very-high 基础上继续推进 `preclass.c` 的 decision-action-field 包装链移除、“none” 字面量三合一与 rir/unknown-hint 小包装移除。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270116_20270122.json`。
> 回填状态：2026-08-07 已按本期 A/B 会话结论与收尾验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 34 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=5`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Remove the decision-action-field wrapper chain by routing the two `wc_preclass_apply_decision_action_fields` call sites directly through the base literals/helpers and deleting the decision_action_source_literal, decision_action_source, decision_none_fallback_reason and apply_decision_none_fallback wrappers.
2. [x] D2：Unify the three none literals by routing their call sites through `wc_preclass_fallback_none_value_literal` and deleting `wc_preclass_fallback_none_literal` and `wc_preclass_decision_none_literal`; remove the rir-hint wrapper helpers `wc_preclass_has_rir_hint_value` and `wc_preclass_apply_allocated_hint_from_rir` by routing directly through `wc_preclass_rir_hint_present`.
3. [x] D3：Remove the unknown-hint wrapper helpers `wc_preclass_v6_unknown_hint_required`, `wc_preclass_unknown_hint_default_reason_literal` and `wc_preclass_apply_v4_unknown_hint_if_needed`, routing the v4 path through the public `wc_preclass_apply_allocated_hint_if_unknown_class`.
4. [x] D4：Remove the rir-unknown compare wrapper literal `wc_preclass_rir_unknown_compare_literal` by routing its three call sites directly through the base `wc_preclass_rir_unknown_literal`.
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段一次通过并满足 B 启动前提，最终 `A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- A 运行目录：`out/artifacts/dev_verify_multiround/20260807-011902`，8/8 轮（D1-D4 + V1-V4）全部通过，`final_status.json` 于 2026-08-07 06:10:45 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。

**编制期验收（已完成，2026-08-07）**：
- [x] `-SyntaxOnly` 装载检查：PASS。
- [x] 全定义静态检查（无 RoundTag）：`errors=0 warnings=0`，D1~D4 全部 op 唯一命中、marker 自有、replacement 收敛、replay 稳定、断言精确。
- [x] 专项安全回归 `task_definition_safety_regression.ps1`：全 case PASS。
- [x] 链式检查 `-RoundTag D1 -ChainRounds`：`errors=0 warnings=0`。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-01-23 ~ 2027-01-31，串行第 36 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（串行第 35 份）`result=pass` 且 A 成功快照固化后启动；保持 `state-only` 承接策略，重点放在 match-layer 字面量收尾、纯转发 tuple/query-kind helper 移除与 special-result setter 内联。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270123_20270131.json`。
> 回填状态：2026-08-07 已按本期 A/B 会话结论与收尾验证结果完成回填。
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Finish the match-layer literal dedup by routing `wc_preclass_match_layer_from_query_kind` directly through the compare literals and removing `wc_preclass_match_layer_cidr_literal`/`wc_preclass_match_layer_ip_literal` definitions and forward declarations.
2. [x] D2：Remove the pure-forward `wc_preclass_set_unknown_hint_tuple` by routing its two call sites directly through `wc_preclass_apply_classified_tuple`, and remove `wc_preclass_match_layer_from_query_kind_normalized` by routing its single call site to the direct `query_is_cidr` condition.
3. [x] D3：Inline the special-result tuple setter for the 8 v4 call sites in `wc_preclass_classify_ip` directly through `wc_preclass_apply_none_confidence_tuple` with the class-special literal.
4. [x] D4：Inline the special-result tuple setter for the 5 v6 call sites and remove the now-unused `wc_preclass_set_special_tuple` definition.
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段一次通过并以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status` 正常关闭（`SESSION_CLOSED_AT=2026-08-07 11:22:01`）。
- B 运行目录：`out/artifacts/dev_verify_multiround/20260807-061242`，8/8 轮全部通过，`B exit=0`，`final_status.json` 于 2026-08-07 11:21:11 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20270116-20270131.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20270116_20270122.json`
  - B：`testdata/autopilot_code_step_tasks_20270123_20270131.json`
- 当前窗口：`WINDOW=2027-01-16 ~ 2027-01-31`
- 当前策略基线：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`、`AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`。
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`（A/B 阶段 launch-ready 均 PASS）。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-08-07 11:22:01`。
- 关键锚点：`A run_dir=out/artifacts/dev_verify_multiround/20260807-011902`、`B run_dir=out/artifacts/dev_verify_multiround/20260807-061242`、`main_round=B/D4`、`guard_log=out/artifacts/ab_session_guard/20260807-011914/guard.log`、`live_status=out/artifacts/ab_session_guard/20260807-011914/live_status.json`。
- 阶段耗时：`A elapsed=0d 04:52:14（start 2026-08-07 01:18:31 / end 2026-08-07 06:10:45）`、`B elapsed=0d 05:09:50（start 2026-08-07 06:12:12 / end 2026-08-07 11:22:01）`、`A/B total=0d 10:03:31`。

**运行期票据（本期）**：
- [x] `a-pass-conclusion-b-started`（ticket `T20260807-061250330-d82a1f5c`）：经唯一原子收尾闭环（handled 2026-08-07 06:13:41）。
- [x] `chat-session-final-status`（ticket `chat-final-20260807-112201`）：经唯一原子收尾闭环（handled 2026-08-07 11:23:31）。

**任务定义静态体检（本期）**：
- [x] 运行期 0 起 task-static 事故，A/B 正式任务定义无需运行期修复；编制期全定义静态检查（A 全定义 / B 以 A 为前置链式）`errors=0 warnings=0` 保持有效。

**本期额外验证（2026-08-07，收尾）**：
- [x] Strict 远程编译冒烟同步 + 黄金校验（`lto-auto`，默认）：`out/artifacts/20260807-121102`，`无告警 + lto 无告警 + Local hash verify=PASS + Golden PASS + referral check=PASS`，`duration=289s`。


**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-02-01 ~ 2027-02-07，串行第 37 份，Checklist A，已完成回填）**：

> 注：本清单在第 36 份 B 的 very-high 基础上继续推进 `preclass.c` 的 decision-field 小包装/常量归一化：match-layer 谓词内联、non-ip 标签纯转发移除、route-change 常量内联与 decision-action 谓词移除。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270201_20270207.json`。
> 回填状态：2026-08-08 已按本期 A/B 会话结论与收尾验证结果完成回填。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 36 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=5`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Inline the match-layer predicate helpers `wc_preclass_match_layer_is_cidr_literal`/`wc_preclass_match_layer_is_ip_literal` directly into `wc_preclass_match_layer_output_label`, then remove the pure-forward `wc_preclass_match_layer_non_ip_label_literal` wrapper by routing its three call sites directly through `wc_preclass_non_ip_literal`.
2. [x] D2：Remove the decision-action predicate wrapper chain by routing the `wc_preclass_should_apply_decision_action` and `wc_preclass_decision_action_missing` call sites directly through `wc_preclass_has_text_value`, then delete the `has_decision_action`/`should_apply_decision_action`/`decision_action_missing` wrapper definitions.
3. [x] D3：Inline the route-change constant flag helpers `wc_preclass_route_change_enabled_flag`/`wc_preclass_route_change_disabled_flag` into literal 1/0 at their four call sites and the `wc_preclass_normalize_route_change_flag` ternary into its single call site, then delete the three flag helper definitions.
4. [x] D4：Remove the pure-forward `wc_preclass_match_layer_for_query_kind` wrapper by routing its single call site directly through `wc_preclass_match_layer_from_query_kind` and inline the `wc_preclass_normalize_query_for_kind` ternary into its single call site, then delete both wrapper definitions.
...
**执行回填（Checklist A，本期）**：
- 本期 A 阶段一次通过并满足 B 启动前提，最终 `A_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- A 运行目录：`out/artifacts/dev_verify_multiround/20260807-135744`，8/8 轮（D1-D4 + V1-V4）全部通过，`final_status.json` 于 2026-08-07 19:33:55 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-02-08 ~ 2027-02-15，串行第 38 份，Checklist B，已完成回填）**：

> 注：Checklist B 仅在 Checklist A（串行第 37 份）`result=pass` 且 A 成功快照固化后启动；保持 `state-only` 承接策略，重点放在 RIR/unknown-hint tuple 归一化、route-change 决策链扁平化与 match-layer 输入标签链内联。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270208_20270215.json`。
> 回填状态：2026-08-08 已按本期 A/B 会话结论与收尾验证结果完成回填。
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Normalize the RIR/unknown-hint tuple writing by routing the two `wc_preclass_apply_unknown_rir_tuple` call sites directly through `wc_preclass_apply_known_rir_tuple` with the rir-unknown literal and delete the `apply_unknown_rir_tuple` definition; route the `set_v6_unknown_hint_result` call site directly through the public `wc_preclass_apply_unknown_hint_if_unknown_class` and delete the `set_v6_unknown_hint_result` definition.
2. [x] D2：Inline the route-change predicate helpers `wc_preclass_action_route_change_not_allowed` and `wc_preclass_route_change_enabled_for_decision` directly into `wc_preclass_route_change_policy_applies`, then delete both helper definitions.
3. [x] D3：Flatten the route-change decision chain by inlining the full policy-applies/apply logic directly into the `wc_preclass_apply_route_change_block_or_skip` call site inside `wc_preclass_apply_route_change_finalize`, then delete the three adjacent helper definitions `wc_preclass_route_change_policy_applies`, `wc_preclass_route_change_policy_apply` and `wc_preclass_apply_route_change_block_or_skip`.
4. [x] D4：Flatten the match-layer input-label chain by inlining the full output/input label logic directly into the `wc_preclass_input_label_from_match_layer` call site inside `wc_preclass_resolve_decision_fields`, then delete the two adjacent helper definitions `wc_preclass_match_layer_output_label` and `wc_preclass_input_label_from_match_layer`.
...
**执行回填（Checklist B，本期）**：
- 本期 B 阶段一次通过并以 `B_FINAL_STATUS=PASS` 收口，最终会话 `SESSION_FINAL_STATUS=PASS`，终态由 `chat-session-final-status` 正常关闭（`SESSION_CLOSED_AT=2026-08-08 01:17:45`）。
- B 运行目录：`out/artifacts/dev_verify_multiround/20260807-193345`，8/8 轮全部通过，`B exit=0`，`main_round=D4`，`final_status.json` 于 2026-08-08 01:17:45 生成。
- 全程 0 起事故票（无 `incident-captured`），无自愈、无重启、无恢复动作。
...
- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20270201-20270215.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20270201_20270207.json`
  - B：`testdata/autopilot_code_step_tasks_20270208_20270215.json`
- 当前窗口：`WINDOW=2027-02-01 ~ 2027-02-15`
- 当前策略基线：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`、`AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`。
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`（A/B 阶段 launch-ready 均 PASS）。
- 当前终态：`A_FINAL_STATUS=PASS`、`B_FINAL_STATUS=PASS`、`SESSION_FINAL_STATUS=PASS`。
- 会话收口：`SESSION_CLOSED=true`、`SESSION_CLOSED_REASON=chat-session-final-status-pass`、`SESSION_CLOSED_AT=2026-08-08 01:17:45`。
- 关键锚点：`A run_dir=out/artifacts/dev_verify_multiround/20260807-135744`、`B run_dir=out/artifacts/dev_verify_multiround/20260807-193345`、`main_round=B/D4`、`guard_log=out/artifacts/ab_session_guard/20260807-135755/guard.log`、`live_status=out/artifacts/ab_session_guard/20260807-135755/live_status.json`。
- 阶段耗时：`A elapsed=0d 05:36:45（start 2026-08-07 13:57:11 / end 2026-08-07 19:33:55）`、`B elapsed=0d 05:44:30（start 2026-08-07 19:33:15 / end 2026-08-08 01:17:45）`、`A/B total=0d 11:20:34`。
- 任务定义静态体检（编制期）：A 全定义 / B 以 A 为前置链式 `errors=0 warnings=0`；运行期 0 起 task-static 事故，A/B 正式任务定义无需运行期修复。

**运行期票据（本期）**：
- [x] `a-pass-conclusion-b-started`（ticket `T20260807-193355943-7d1717e8`）：经唯一原子收尾闭环（handled 2026-08-07 19:34:44）。
- [x] `chat-session-final-status`（ticket `chat-final-20260808-011745`）：经唯一原子收尾闭环（handled 2026-08-08 01:18:17）。
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-02-16 ~ 2027-02-22，串行第 39 份，Checklist A，已完成回填）**：
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-02-23 ~ 2027-03-01，串行第 40 份，Checklist B，已完成回填）**：
...
**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-03-02 ~ 2027-03-08，串行第 41 份，Checklist A，已完成）**：

> 注：本清单在第 40 份 B 基础上继续推进 `preclass.c` 的 decision 字段层小包装移除：action 谓词、normalize/match-layer、apply-decision-action helper 与 guard 谓词全部内联到唯一调用点后删除定义。
> 对应任务定义：`testdata/autopilot_code_step_tasks_20270302_20270308.json`。
> 回填状态：2026-08-10 已完成运行回填，A 阶段一次通过，无人工干预（详见执行回填）。

**八轮通用约束（开跑前确认）**：
1. [x] 串行约束：仅在第 40 份已收口且会话稳定后启动，A 期间禁止并发跑 B。
2. [x] D1 reset 要求固定：运行范围包含 D1 时显式携带 `-ResetCodeStepState`（直跑入口可用 `-Reset` / `-ResetStateOnly`）。
3. [x] 提速模式固定：`-DevVerifyStride 2 -VerifyExecutionProfile d6-only -EnableGuardedFastMode $true -EnableGateOnlySourceDrivenSkip $true`。
4. [x] 质量闸固定：`-TaskDesignQualityPolicy enforce -UnknownNoOpBudget 1 -UnknownNoOpConsecutiveLimit 2 -DisableUnknownNoOpBudgetGate:$false`。
5. [x] 开发轮密度固定：`dRoundChangeDensity=very-high`，每个 D 轮 `minOperationsPerDRound=4`。
...
**开发四轮（D1~D4，超高密度）**：
1. [x] D1：Inline the three action predicates `wc_preclass_action_is_hint_applied` / `wc_preclass_action_is_preclass_short_circuit` / `wc_preclass_action_is_step47_short_circuit` into `wc_preclass_action_route_change_allowed` and delete the three predicate definitions.
2. [x] D2：Inline `wc_preclass_action_route_change_allowed`, `wc_preclass_route_change_fallback` and `wc_preclass_normalize_fallback_reason` into their single call sites and delete the three definitions.
3. [x] D3：Inline `wc_preclass_normalize_decision_action` and `wc_preclass_match_layer_from_query_kind`, inline `wc_preclass_apply_decision_action_fields` into `wc_preclass_resolve_decision_fields` and delete the three definitions.
4. [x] D4：Inline `wc_preclass_query_missing`, `wc_preclass_preclass_disabled_enabled` and `wc_preclass_decision_fields_missing` into `wc_preclass_resolve_decision_fields` and delete the three definitions.
...
**执行回填（Checklist A，本期，2026-08-10）**：
- **A 阶段终态**：`A_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）。
- **A 运行目录**：`out/artifacts/dev_verify_multiround/20260809-114503`（`final_status.json`、`summary.csv`、`a_success_snapshot/` 已固化）。
- **启动方式**：`open_unattended_ab_stage_window.ps1 -Stage A -StartMonitors`（外部 PowerShell 窗口），launch-ready PASS、网络预检 PASS、远端锁 absent。
- **事故与自愈**：无（`incident_dir` 为空），未触发代码/脚本自愈、未发生阶段重启。
- **A 阶段耗时**：`0d 08:30:42`（start=2026-08-09 11:44:25，end=2026-08-09 20:15:07，含 launcher/preflight）。

**编制期验收（已完成，2026-08-09）**：
- [x] `-SyntaxOnly` 装载检查：PASS。
- [x] 全定义静态检查（无 RoundTag）：`errors=0 warnings=0`，D1~D4 全部 op 唯一命中、marker 自有、replacement 收敛、replay 稳定、断言精确。
- [x] 有效源码 clang `-fsyntax-only -Wall -Wextra`：编译通过（仅既有 `strncasecmp` 隐式声明告警，与基线一致）。


**下次开工清单（无人值守超高密度档：开发四轮 + 复检四轮，提速模式，2027-03-09 ~ 2027-03-15，串行第 42 份，Checklist B，已完成）**：
...
> 回填状态：2026-08-10 已完成运行回填，B 阶段一次通过，无人工干预（详见执行回填）。
...
**开发四轮（D1~D4，超高密度跟进）**：
1. [x] D1：Inline `wc_preclass_apply_known_rir_tuple` into its four call sites (two in `wc_preclass_set_v6_global_unicast_result`, two in `wc_preclass_set_allocated_hint`) and delete its definition.
2. [x] D2：Inline `wc_preclass_apply_classified_tuple` into its three call sites (in `wc_preclass_apply_none_confidence_tuple`, `wc_preclass_apply_unknown_hint_default_tuple` and `wc_preclass_apply_unknown_hint_if_unknown_class`) and delete its definition.
3. [x] D3：Inline `wc_preclass_set_v6_global_unicast_result` into `wc_preclass_classify_ip` and inline `wc_preclass_v6_is_loopback` into its call site, then delete both definitions.
4. [x] D4：Inline `wc_preclass_assign_from_row` into `wc_preclass_lookup_row_and_assign_v4/v6` and inline `wc_preclass_read_be64` into `wc_preclass_lookup_row_and_assign_v6` hi/lo, then delete both definitions; merge-delete the adjacent `apply_unknown_hint_default_tuple` / `apply_unknown_hint_if_unknown_class` / `apply_allocated_hint_if_unknown_class` definitions in a single op and remove the now-redundant `set_allocated_hint` forward declaration (name-mapping helpers remain).
...
**执行回填（Checklist B，本期，2026-08-10）**：
- **B 阶段终态**：`B_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）。
- **B 运行目录**：`out/artifacts/dev_verify_multiround/20260809-201456`（`final_status.json`、`summary.csv` 已生成）。
- **启动方式**：以 A 成功快照为基线，`open_unattended_ab_stage_window.ps1 -Stage B -StartMonitors`；A→B 交接正常（`a-pass-conclusion-b-started` 事件评审闭环）。
- **事故与自愈**：无（`incident_dir` 为空），未触发代码/脚本自愈、未发生阶段重启。
- **B 阶段耗时**：`0d 06:37:48`（B start=2026-08-09 20:14:25，B end=2026-08-10 02:52:12）。
- **A/B 合计耗时**：`0d 15:07:48`（session start=2026-08-09 11:44:25，至 B 收尾 2026-08-10 02:52:12）。
...
**对应任务启动文件（2026-08-10，已完成回填）**：

- 启动文件路径：`testdata/unattended_start/active/unattended_ab_start_20270302-20270315.md`
- 绑定文件：
  - A：`testdata/autopilot_code_step_tasks_20270302_20270308.json`
  - B：`testdata/autopilot_code_step_tasks_20270309_20270315.json`
- 当前窗口：`WINDOW=2027-03-02 ~ 2027-03-15`
- 当前策略基线：`RUN_MODE=foreground-visible`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`、`AI_CHAT_POLICY_DELIVERY_PRIMARY=ipc`。
- 预检基线：`PRECHECK_STATUS=PASS`、`PRECHECK_START_GATE=READY`（A 阶段 launch-ready PASS；B 阶段按设计等待 A PASS snapshot 后由 stage window 启动）。
- 当前终态：**SESSION=PASS、A=PASS、B=PASS**（A/B 均一次通过，无人工干预）。
  - A 运行目录：`out/artifacts/dev_verify_multiround/20260809-114503`（A elapsed=0d 08:30:42，start=2026-08-09 11:44:25，end=2026-08-09 20:15:07）。
  - B 运行目录：`out/artifacts/dev_verify_multiround/20260809-201456`（B elapsed=0d 06:37:48，start=2026-08-09 20:14:25，end=2026-08-10 02:52:12；A/B 合计 0d 15:07:48）。
  - 会话结束：2026-08-10 02:52:12（`chat-session-final-status` 事件评审闭环）。
- 任务定义静态体检（编制期）：A 全定义 / B 以 A 为前置链式 `errors=0 warnings=0`；启动前由 launcher 执行 `-SyntaxOnly` 装载门禁，D 轮完整检查由运行期独立 checker 执行。
- **Strict 远程冒烟同步 + 黄金校验（2026-08-10）**：
...
**下次开工清单（Vx 多文件：统一 client-flow preclass decision，2027-03-16 ~ 2027-03-22，串行第 43 份，Checklist A，已完成）**：
...
1. [x] D1：以任务定义 operations 承载本地 clang portability 适配，并声明统一 preclass decision result 与 resolver；不得预先直接修改业务源码。
2. [x] D2：实现统一 resolver，固定 Step47 early-unknown、P1 controlled action 与 Step47 eligible 的优先级。
3. [x] D3：batch 路径调用统一 resolver，保持输出与短路行为。
4. [x] D4：single-query 路径调用统一 resolver，保留 guessed-RIR hint 路由语义。
5. [x] V1~V4：完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] A 使用 `restore-source` 串行启动，禁止自动提交与推送。

**编制期验收（已完成）**：TODO-free、`-SyntaxOnly`、Vx 专项安全回归通过；A 全定义静态检查 `errors=0 warnings=0 infos=13`，mandatory local clang syntax gate 通过；checker 以完整 effective-header overlay 编译内存 target map，未直接改动业务源码。

**执行回填（Checklist A，本期，2026-08-11）**：
- **A 阶段终态**：`A_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）。
- **A 运行目录**：`out/artifacts/dev_verify_multiround/20260810-141835`（`final_status.json`、`summary.csv` 已生成）。
- **启动方式**：`open_unattended_ab_stage_window.ps1 -Stage A -StartMonitors`；launch-ready PASS、网络预检 PASS、远端锁 absent。
- **事故与自愈**：A 首跑于 D1 编译/测试阶段失败（run=out/artifacts/dev_verify_multiround/20260810-125213，`compile-or-test-failure`，`code-or-unknown`），guard incident `20260810-130937` 捕获后重启；重启后 D1~V4 一次通过，未触发代码/脚本自愈。
- **A 阶段耗时**：`0d 07:51:00`（start=2026-08-10 12:51:41，V4 end=2026-08-10 20:42:41，含首跑 D1 失败与重启）。


**下次开工清单（Vx 多文件：提取公共 route-decision 策略，2027-03-23 ~ 2027-03-31，串行第 44 份，Checklist B，已完成）**：
...
1. [x] D1：在公共 preclass header 声明 route-decision value 与纯优先级 resolver。
2. [x] D2：在 `preclass.c` 实现 Config-independent 策略，保持 explicit-host precedence。
3. [x] D3：将 A/43 client result 转为公共类型别名，由 Config-bound adapter 委托公共策略。
4. [x] D4：batch 与 single-query call sites 使用公共类型，同时保留 client adapter 边界。
5. [x] V1~V4：完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] B 仅在 A PASS snapshot 完整后以 `state-only` 启动；A 失败必须阻断 B。

**编制期验收（已完成）**：TODO-free、`-SyntaxOnly`、Vx 专项安全回归通过；A→B 链式全定义静态检查 `errors=0 warnings=0 infos=24`，完整 target set 编译通过；prerequisite closure 与 D3 direct include 已补齐，磁盘业务源码保持基线。

**执行回填（Checklist B，本期，2026-08-11）**：
- **B 阶段终态**：`B_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）；B 从 A snapshot 承接，D1~D3 以 `D-CODESTEP-ONLY`/`already-applied` 快通过，D4 完整执行。
- **B 运行目录**：`out/artifacts/dev_verify_multiround/20260811-020525`（`final_status.json`、`summary.csv` 已生成）。
- **启动方式**：以 A 成功快照为基线，`open_unattended_ab_stage_window.ps1 -Stage B -StartMonitors`；A→B 交接正常（`a-pass-conclusion-b-started` 事件评审闭环）。
- **V2 快跳（原设计语义）**：V2 因 D1~D3 出现 1 个安全运行时 D-NOP 快跳（`D1D3NoOpCount=1`、`D1D3SafeNoOpCount=1`、`D1D3UnknownNoOpCount=0`，`SkipReason=fast-skip-v2-d-nop-count-1-of-3`），符合“D1-D3 出现 1~2 个安全 D-NOP 可跳 V2、unknown no-op 阻断”的既有契约。
- **事故与自愈**：B 首跑于 V1 验证阶段失败（run=out/artifacts/dev_verify_multiround/20260810-204443，`verify-failure`，D6 双轮一致性 strict gate 失败），guard incident `20260811-015808` 捕获后重启；重启后 D1~D3 承接、D4~V4 通过（V2 快跳），未触发代码/脚本自愈。
- **B 阶段耗时**：`0d 09:05:44`（B_TASK_FIRST_START_AT=2026-08-10 20:44:10，V4 end=2026-08-11 05:49:54，含首跑 V1 失败与重启）。


**对应任务启动文件（2026-08-11，已完成回填）**：
...
- 当前终态：**SESSION=PASS、A=PASS、B=PASS**（A/B 各 8/8 轮通过；B 承接 A snapshot，V2 按原设计快跳）。
  - A 运行目录：`out/artifacts/dev_verify_multiround/20260810-141835`（A 首跑 D1 失败重启后通过，V4 end=2026-08-10 20:42:41）。
  - B 运行目录：`out/artifacts/dev_verify_multiround/20260811-020525`（B 首跑 V1 失败重启后通过，V4 end=2026-08-11 05:49:54）。
  - 会话结束：2026-08-11（`chat-session-final-status` 事件评审闭环）。
- 事故与自愈：A-D1、B-V1 各 1 起失败经 guard incident 重启后收敛（`incident_20260810-130937` / `incident_20260811-015808`），未触发代码/脚本自愈。
- 任务定义静态体检（编制期）：A 全定义 / B 以 A 为前置链式 `errors=0 warnings=0`；启动前由 launcher 执行 `-SyntaxOnly` 装载门禁，D 轮完整检查由运行期独立 checker 执行。
- **Strict 远程冒烟同步 + 黄金校验（2026-08-11）**：
...
**下次开工清单（Vx 多文件：Step 4.7 受控放量切片 A——R0 候选治理，2027-04-01 ~ 2027-04-07，串行第 45 份，Checklist A，已完成）**：
...
1. [x] D1：在 `client_flow.c` 声明分类式 allocated/control 排除谓词 `wc_client_preclass_classification_allocated_control`。
2. [x] D2：实现该谓词（`allocated|legacy` 且 `rir` 非 `none|unknown` 即排除），置于 `wc_client_is_step47_trial_candidate` 之前。
3. [x] D3：Step47 early-unknown 候选路径接入分类治理：命中候选名单后先分类，allocated/control 一律返回 0（R2 禁入）；R0 默认列表与 CSV 语义不变。
4. [x] D4：P1 controlled-unknown 候选路径改为同一分类治理，替换原 guess-based 排除（`wc_client_guess_query_rir_host`），保持行为等价并统一入口。
5. [x] V1~V4：完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] A 使用 `restore-source` 串行启动，禁止自动提交与推送。

**编制期验收（已完成，2026-08-13）**：TODO-free、`-SyntaxOnly` 通过；A 全定义静态检查 `errors=0 warnings=0 infos=9`，`-RoundTag D1 -ChainRounds` 链式检查通过；各轮 mandatory local clang 语法门禁通过；Vx 专项安全回归 `result=pass`。

**执行回填（Checklist A，本期，2026-08-13）**：
- **A 阶段终态**：`A_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）。
- **A 运行目录**：`out/artifacts/dev_verify_multiround/20260813-044152`（`final_status.json`、`summary.csv`、`a_success_snapshot/` 已固化）。
- **启动方式**：`open_unattended_ab_stage_window.ps1 -Stage A -StartMonitors`；launch-ready PASS、PRECHECK 回填完成、event-only 调度，A 阶段一次通过（无事故/无自愈/无重启）。
- **A 阶段耗时**：`0d 05:44:34`（start=2026-08-13 04:41:17，A final_status 生成 2026-08-13 10:25:51）。


**下次开工清单（Vx 多文件：Step 4.7 受控放量切片 B——公共候选治理 API，2027-04-08 ~ 2027-04-14，串行第 46 份，Checklist B，已完成）**：

> 对应任务定义：`testdata/autopilot_code_step_tasks_20270408_20270414.json`（`schemaVersion=vx-draft`）。
> 目标闭包：`preclass_header`（`include/wc/wc_preclass.h`）、`preclass_source`（`src/core/preclass.c`）、`client_flow`；必须以第 45 份任务定义作为 prerequisite。

1. [x] D1：在公共 `wc_preclass.h` 声明 Config-independent 治理 API `wc_preclass_classification_is_allocated_control`。
2. [x] D2：在 `preclass.c` 实现该 API（字符串字面量判定，不依赖 Config），置于 route-decision 实现之后。
3. [x] D3：`client_flow.c` 的静态谓词改为薄适配器，委托公共 API。
4. [x] D4：两个候选调用点（early-unknown 与 P1）直接调用公共 API，并删除不再使用的 client 适配器定义与前向声明。
5. [x] V1~V4：完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] B 仅在 A PASS snapshot 完整后以 `state-only` 启动；A 失败必须阻断 B。

**编制期验收（已完成，2026-08-13）**：TODO-free、`-SyntaxOnly` 通过；A→B 链式全定义静态检查 `errors=0 warnings=0 infos=20`，`-RoundTag D1 -ChainRounds`（A 前置）链式检查通过；完整 target set 各轮编译门禁通过；Vx 专项安全回归 `result=pass`。

**执行回填（Checklist B，本期，2026-08-13）**：
- **B 阶段终态**：`B_FINAL_STATUS=PASS`，`Result=pass`、`ExitCode=0`、`CompletedRoundCount=8`（D1~D4 + V1~V4）、`FailedRoundTags=[]`（无失败轮次）。
- **B 运行目录**：`out/artifacts/dev_verify_multiround/20260813-113036`（`final_status.json`、`summary.csv` 已生成）。
- **启动方式**：以 A 成功快照为基线，`open_unattended_ab_stage_window.ps1 -Stage B -StartMonitors`；A→B 交接正常（`a-pass-conclusion-b-started` 事件评审闭环）。
- **事故与自愈**：B 首跑 D1 编译/验证阶段失败（run=out/artifacts/dev_verify_multiround/20260813-102725，`compile-or-test-failure`，`code-or-unknown`，failed_rounds=D1），guard incident `20260813-112537` 捕获后重启；下钻根因链为 Step47 preclass preflight fail（`pass=4 fail=1`）→ Step47 prerelease `step=ab` fail → `step47-ab mismatch: auth_changed=2/1`，源于 8.8.8.8 baseline 实况查询 `BaseAuth=error` 网络瞬态；判定非代码缺陷（D1 task-static PASS、全架构编译成功、D6 全 PASS），未修改任务定义/源码，走非代码恢复重启；重启后 D1~V4 一次通过，未触发代码/脚本自愈。
- **B 阶段耗时**：`0d 06:09:12`（B_TASK_FIRST_START_AT=2026-08-13 10:26:54，B end=2026-08-13 16:36:06，含首跑 D1 失败与重启）。


**对应任务启动文件（2026-08-13，已完成回填）**：

- 路径：`testdata/unattended_start/active/unattended_ab_start_20270401-20270414.md`。
- 绑定 A/45 与 B/46；`WINDOW=2027-04-01 ~ 2027-04-14`、A `restore-source`、B `state-only`、`AI_CHAT_POLICY_WORK_MODE=event-only`、`ENTRY_MODE=single-param-fastmode`、`A_FAILURE_BLOCKS_B=true`、`B_START_REQUIRES_A_PASS_WITH_SNAPSHOT=true`。
- 当前终态：**SESSION=PASS、A=PASS、B=PASS**（A/B 各 8/8 轮通过；B 首跑 D1 失败经 guard incident 重启后收敛）。
  - A 运行目录：`out/artifacts/dev_verify_multiround/20260813-044152`（A 一次通过，A final_status 生成 2026-08-13 10:25:51）。
  - B 运行目录：`out/artifacts/dev_verify_multiround/20260813-113036`（B 首跑 D1 失败重启后通过，B end=2026-08-13 16:36:06）。
  - 会话结束：2026-08-13（`chat-session-final-status` 事件评审闭环，`SESSION_CLOSED_AT=2026-08-13 16:38:11`）。
- 事故与自愈：B-D1 1 起失败经 guard incident `20260813-112537` 重启后收敛，根因 8.8.8.8 baseline 实况 `BaseAuth=error` 网络瞬态，判定非代码缺陷、未修改任务定义/源码、未触发代码/脚本自愈。
- 任务定义静态体检（编制期）：A 全定义 / B 以 A 为前置链式 `errors=0 warnings=0`；启动前由 launcher 执行 `-SyntaxOnly` 装载门禁，D 轮完整检查由运行期独立 checker 执行。
- **Strict 远程冒烟同步 + 黄金校验（2026-08-13）**：
  - 命令：`WHOIS_STRICT_VERSION=1 tools/remote/remote_build_and_test.sh -H 10.0.0.199 -u larson -k '/c/Users/妙妙呜/.ssh/id_rsa' -r 1 -q '8.8.8.8 1.1.1.1 10.0.0.8' -s '/d/LZProjects/lzispro/release/lzispro/whois;/d/LZProjects/whois/release/lzispro/whois' -P 1 -a '' -G 1 -E '' -O 'lto-auto' -K 0 -N 0`（Strict Version）。
  - 结果：无告警 + lto 无告警 + `Local hash verify: PASS` + `[golden] PASS` + referral check: PASS；用时 285s。
...
**Vx-3 覆盖边界与开关定调（2026-08-14 修订，用户确认）**：

- 本组 47/48 定位为 **Vx-3 预备能力切片**，不宣称完成 Phase B 默认首跳迁移。已覆盖：未显式 `-h` 的分类器优先首跳（batch/single）、unknown/low-confidence 保留旧策略回退、公共 Config-independent 首跳 API、`classifier-rir-hint` 观测动作。
...
**Phase C 默认翻转正式收尾（2026-08-17，PASS）**：

- opts/runner 默认开启；P0/P1、CIDR、Step47 readiness/rollback 黄金基线同步更新，`--disable-address-preclass` 保持全量回退。
- Phase C `20/20`、special-purpose `17/17`、P0 `12/12`、P1 `232/232`、CIDR `4/4 + 9/9`、Step47、12x6 全 PASS；12x6 证据 `out/artifacts/redirect_matrix_10x6/20260817-033253`。
- 最终 Strict `out/artifacts/20260817-034423` 完成 9 架构、Local hash、Golden、referral 全 PASS（302s）并同步发布制品。Phase C 与 Address-Space 前置分类器正式收尾。

**批量路径早收敛补全与自检黄金期望修复（2026-08-17，PASS）**：

- 背景：Phase C 默认翻转后，单条查询对高置信 `reserved|special + rir=none`（如 `10.0.0.8`）早收敛 `unknown @ unknown`，但批量流程在循环中先执行 `wc_handle_private_ip` 私网提示短路，导致同一查询在单条/批量模式下输出不一致（批量仍输出 `10.0.0.8 is a private IP address`），不符合“隐式查询收敛 unknown”契约。
- 发现路径：四轮 Golden + 12x6 复核中，raw 自检 golden 报告实际为 `[golden-selftest] FAIL`（`out/artifacts/batch_raw/20260817-051757/build_out/golden_selftest_report.txt` 缺少 `error pattern matched: private IP address`），而 suite 汇总显示 PASS——`selftest_golden_suite.ps1` 的 `cmd | tee` 管道无 `pipefail`，bash 返回 tee 退出码（0）掩盖了 `golden_check_selftest.sh` 的真实失败；`remote_batch_strategy_suite.ps1` 已带 pipefail，属遗漏。
- 修复：
  ...
  2. `tools/test/selftest_golden_suite.ps1` 与 `remote_batch_strategy_suite.ps1`：selftest golden 调用统一使用 `set -o pipefail` 和 `2>&1 | tee`，使失败可靠传播 rc=3，并把 stderr 诊断写入报告；缺失策略日志、非法 TagExpectations 与 checker 非零均 fail-close，trap 保留原始错误消息。`golden_check_selftest.sh` 新增可重复 `--require-line`，默认 `LinePatterns` 精确要求 `10\.0\.0\.8 is a private IP address`（CRLF/LF 均可），避免启动 marker 与预打错误行掩盖真实分支未执行。
...
- 2026-08-18 复核补强：提交 `22e0247f` 后的四轮 Golden + 12x6 全 PASS（Strict `20260817-090311`/`20260817-091109`，批量 `091732/092237/092812/093454`，自检 `094804/095342/095957/100557`，矩阵 `100855` authority 空表且无 errors）。审计确认 raw 自检走单条路径时 `--selftest-force-private 10.0.0.8` 仍被 early-converge 短路绕过（golden 仅靠启动 marker 通过），而批量循环已在 24.22 修复。最终实现将 force-private 检查前移到单条 preclass 决策之前，并按执行器相同优先级读取 `net_ctx->injection`、为空时回退 `wc_selftest_injection_view()`；命中即转入 `wc_client_run_single_query` 的真实私网分支，不再输出虚假的 early-converge 观察。golden checker 新增 `--require-line` 阻断 marker-only 假通过；最终真实 LTO 制品通过 Phase C review `26/26 PASS`（`out/artifacts/preclass_phasec_review/20260818-070138`）。普通单条隐式私网查询仍早收敛，显式 `-h` 保持兼容。
...
1. [x] D1：`wc_preclass.h` 声明校验函数；`preclass.c` 补 `stdio.h` 并追加冻结期望表与校验函数（全路径 + 表最长前缀双侧断言）。
2. [x] D2：`selftest.c` 新增 `selftest_preclass_consistency()`（fail-close），接入 `wc_selftest_run()` 头部。
3. [x] D3：`wc_preclass.h` 声明 metrics API；`preclass.c` 追加计数器（含 class_unallocated）与幂等 flush 打印（零计数也输出）。
4. [x] D4：`whois_query_exec.c` 观测路径记录一次分类；`runtime.c` 初始化启用并在 `wc_runtime_exit_flush()` 内 flush。
5. [x] V1~V4：A 完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] A 使用 `restore-source` 串行完成，运行期间禁止自动提交与推送。
7. [x] 专项验证（任务结束后单独执行）：`--retry-metrics` 冒烟 grep `[PRECLASS-METRICS]`；C 内置 selftest 新标签 PASS。
...
1. [x] D1：`preclass.c` 引入 `wc_preclass_classify_ip_with_row()`，公共 `wc_preclass_classify_ip()` 转为薄包装，`classify_query` 传入已找到的 row；并追加 16 槽轮转 query-keyed 缓存（FNV-1a 探测 + `wc_preclass_get_lookup_count()`），`wc_preclass.h` 声明计数 API。
2. [x] D2：`client_flow.c` 前向声明与定义同步新签名；`resolve_preclass_decision` 计算一次共享分类并传给 first-hop/Phase C 候选；批量候选与单查询派发两个 first-hop 调用点、批量循环 Phase C 门禁接入共享结果。
3. [x] D3：Step47 early-unknown 与 P1 controlled-unknown 候选改为接收共享结果（纯 IP 才读取分类），最终接线。
4. [x] D4：`selftest.c` 新增 `selftest_preclass_single_pass()` 并接入 `wc_selftest_run()`；内含缓存去重断言（第二次分类不再新增生成表查找）。
5. [x] V1~V4：B 完成基线、噪声、混合样本与最终 8/8 收口复检。
6. [x] B 在 A PASS snapshot 完整后以 `state-only` 承接启动；A 失败阻断 B。
7. [x] 行为零变化门禁：Phase C `26/26`、special `17/17`、P0 `12/12`、P1 `232/232`、CIDR `4/4 + 9/9`、Step47、12x6 authority 空表、Golden/referral 全 PASS；批量与单条输出契约一致。
...
**下次开工清单（交互式：24.23.7 代码清理，2026-08-20，切片 A/B/C，已完成回填 2026-08-21）**：
...
2. [x] 切片 B（低风险）：删除 `client_flow.c` 的 `wc_client_csv_is_default_marker`（公共 API 已确认，仅 3 处引用），step47 early-unknown 与 p1 tier 两处调用点改用公共版。验证：P1 232/232 + Phase C 26/26 + marker 专项（default/` default `/多 token/空值/自定义列表）+ 显式 `-h` 与 `--disable-address-preclass` 行为不变。
...
| 字段 | 值 |
|---|---|
| 清单状态 | 已回填（2026-08-23：A/B 全 PASS，最终 Strict/批量/自检黄金 + 12x6 矩阵全 PASS） |
| 运行窗口 | `2027-06-10 ~ 2027-06-23`（A：2027-06-10 ~ 2027-06-16；B：2027-06-17 ~ 2027-06-23） |
| 运行模式 | code-change |
| 质量策略 | enforce（`qualityPolicy.operationSafetyPolicy=enforce`） |
| Checklist A 任务定义 | `testdata/autopilot_code_step_tasks_20270610_20270616.json` |
| Checklist B 任务定义 | `testdata/autopilot_code_step_tasks_20270617_20270623.json` |
| active start-file | `testdata/unattended_start/active/unattended_ab_start_20270610-20270623.md`（已投入运行并完成，A/B 全 PASS） |
| A schema | `vx-draft` |
| B schema | `vx-draft` |
| A target set SHA-256 | `93485c168fb16f47dfcf09ca531686e30d4631a083e52e2194574f9a832cf166` |
| B target set SHA-256 | `93485c168fb16f47dfcf09ca531686e30d4631a083e52e2194574f9a832cf166` |
| 主归属文档 | `docs/RFC-address-space-preclassifier.md` |
| 协同文档 | `docs/RFC-whois-client-split.md` |
| 用户启动授权 | 已授权（2026-08-22，A/B 启动） |

**权威文档落点**

| 角色 | 文档路径 | 章节号或稳定标题 | 纳入原因 |
|---|---|---|---|
| 主归属 | `docs/RFC-address-space-preclassifier.md` | 24.29 | 直接拥有 literal 收敛（24.23.7 延迟项）第 3 批设计与执行 |
| 协同 | `docs/RFC-whois-client-split.md` | 快速索引 / 24.29 落点 | 客户端拆分架构与整体进度记录 |
...
#### Checklist A：literal 收敛第 3 批 15 helper（vx55，action/decision 域）

**目标与边界**

- 目标：内联 15 个单次使用 literal helper 并在其唯一调用点替换为字符串字面量，随后删除对应单行定义。
  - D1：`action_source_decision_value`/`action_hint_applied`/`action_classifier_rir_hint`/`action_early_converge`/`action_preclass_short_circuit`（`wc_preclass_resolve_route_decision()`/`wc_preclass_apply_route_change_finalize()` 5 处）。
...
```powershell
# SyntaxOnly 装载检查
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20270610_20270616.json -Policy enforce -SyntaxOnly

# A 编制期全定义严格检查
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20270610_20270616.json -Policy enforce -FailOnWarnings

# A 有效源码编译（D4 后）与内联结果核对
# clang -std=c11 -fsyntax-only -Wall -Wextra -I include <effective preclass.c>

# 经用户授权后使用 stage window 启动 A
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/open_unattended_ab_stage_window.ps1 -Stage A -StartFile "testdata/unattended_start/active/unattended_ab_start_20270610-20270623.md" -StartMonitors
```
...
| 门禁 | 状态 | 证据或摘要 |
|---|---|---|
| TODO-free / 编码 | PASS | TODO-free OK；JSON 解析通过；UTF-8 BOM+LF 检查 non_compliant=0 |
| SyntaxOnly | PASS | `status=PASS schema=vx-draft targets=1 target_set_sha256=93485c16...` |
| A 全定义静态检查 | PASS | `-Policy enforce -FailOnWarnings`：`errors=0 warnings=0 infos=32`；D1-D4 26 ops 全 `pattern_match=1`，各轮 clang syntax gate pass |
...
- 目标：内联 15 个单次使用 literal helper 并在其唯一调用点替换为字符串字面量，随后删除对应单行定义。
  - D1：`reason_non_ip_input`/`confidence_code_c0-c3`/`observe_reason_prefix`（`observe_reason_key`/`observe_confidence_code`/`observation_codes`/`classify_ip_with_row` 6 处）。
...
```powershell
# SyntaxOnly 装载检查
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20270617_20270623.json -Policy enforce -SyntaxOnly

# B 以 A 为 prerequisite 的编制期链式全定义检查
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20270617_20270623.json -PrerequisiteTaskDefinitionFiles testdata/autopilot_code_step_tasks_20270610_20270616.json -Policy enforce -FailOnWarnings

# A+B effective target set 的有效源码编译与内联结果核对
# clang -std=c11 -fsyntax-only -Wall -Wextra -I include <A+B effective preclass.c>

# 经用户授权且 A PASS 后启动 B
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/open_unattended_ab_stage_window.ps1 -Stage B -StartFile "testdata/unattended_start/active/unattended_ab_start_20270610-20270623.md" -StartMonitors
```
...
- [x] active start-file 已生成并通过字段同步、编码和 launch-ready 检查（DryRun，未写回 `PRECHECK_*`，保持初始 NOT_RUN）。
- [x] 权威文档集合内每份文档均已包含本组 Checklist A/B，身份字段和证据一致。
- [x] 用户已检查任务定义、权威文档集合中的清单和 start-file，并确认任务基线（提交推送 `e00080f4`）。
- [x] 用户已明确发出启动命令，A/B 已执行完成。

#### 执行回填（2026-08-22~23 完成）

**Checklist A 回填（vx55，第 3 批 15 helper，action/decision 域）**

| 字段 | 实际值 |
|---|---|
| final status | PASS |
| started_at / completed_at / elapsed | 2026-08-22 16:01:23 → 2026-08-22 23:57:32（A elapsed=0d 07:56:10） |
| run_dir | `out/artifacts/dev_verify_multiround/20260822-160200` |
| final result / summary | A PASS；15 个单次使用 literal helper 内联为字符串字面量并删除定义；成功快照交 B 承接 |

**Checklist B 回填（vx56，第 3 批 15 helper，早段 6 + class/rir/conf/reason 9）**

| 字段 | 实际值 |
|---|---|
| final status | PASS（exit=0） |
| started_at / completed_at / elapsed | 2026-08-22 23:56:47 → 2026-08-23 07:15:20（B elapsed=0d 07:18:34） |
| run_dir | `out/artifacts/dev_verify_multiround/20260822-235721` |
| final result / summary | B PASS；以 A 成功快照为基线承接，15 个单次使用 literal helper 内联并删除定义；`main_round=D4` 收敛 |

**会话收尾与最终验证（2026-08-23）**
...
| 轮次 | 参数 | 用时 | 结果/产物 |
|---|---|---|---|
| 1. 远程编译冒烟同步 + 黄金校验（Strict） | `lto-auto` 默认 | 268s | 无告警 + lto 无告警 + Local hash verify: PASS + Golden PASS + referral check: PASS；`out/artifacts/20260823-090004` |
| 2. 远程编译冒烟同步 + 黄金校验（Strict） | `lto-auto` + `--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first` | 259s | 无告警 + lto 无告警 + Local hash verify: PASS + Golden PASS + referral check: PASS；`out/artifacts/20260823-090701` |
| 3. 批量策略黄金（raw/health-first/plan-a/plan-b） | `lto-auto` | 1,198.287s | 四策略全 `[golden] PASS`（`batch_raw/20260823-091349`、`batch_health/20260823-091840`、`batch_plan/20260823-092356`、`batch_planb/20260823-092948`） |
| 4. 自检黄金（raw/health-first/plan-a/plan-b） | `lto-auto` + `--selftest-force-suspicious 8.8.8.8` | 1,222.364s | 四策略全 `[golden-selftest] PASS`（`batch_raw/20260823-093542`、`batch_health/20260823-094029`、`batch_plan/20260823-094550`、`batch_planb/20260823-095134`） |
| 5. 重定向矩阵（IP x RIR：12 x 6） | — | — | Authority mismatches 空、`(no errors found)`；`out/artifacts/redirect_matrix_10x6/20260823-100221` |

- 审计复核：抽样核验 `golden_report.txt`/`golden_report_*` 均 `[golden] PASS`、`golden_selftest_report.txt` 均 `[golden-selftest] PASS`、冒烟日志 warning 计数=0、矩阵 `errors_20260823-100221.txt` 仅 `(no errors found)`。
- 分类语义/输出契约/生成表零变化；release 二进制与 `SHA256SUMS-static.txt` 已同步（工作区含运行产物，待用户授权提交推送）。
