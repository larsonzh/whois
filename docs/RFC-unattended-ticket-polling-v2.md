...
默认频率：
- 常态：按定时状态票节奏（默认值由 start-file 配置决定）。
- 事件态（`FAIL`/`BLOCKED` 或高严重级）：每 5 分钟。

单轮执行预算：
- 状态票仍按“最新一张优先、旧票覆盖”的幂等规则处理。
- 事件驱动票按队列顺序排空，允许在同一次轮询消费中连续处理多张，只要它们属于本期未处理事件票且事件仍存在。
...
4. 若 drain pass 中出现多个未处理事件驱动票，按事件顺序排空处理，事件已解除则先标记已处理，再继续查找下一张，避免跨轮次漏票。
...
- 允许被认领并输出对应报告/决策动作。
- `manual-wait-paused`、`budget-exhausted-stop`、`known-infra-transient-stop` 的 `business_command`、`continue_watch_command` 与 `recovery_transaction_command` 均留空，`next_command_order` 固定为 `route_guard_command -> atomic_closeout_command`。
