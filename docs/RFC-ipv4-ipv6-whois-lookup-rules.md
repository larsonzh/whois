...
- 2026-08-17 Phase C 默认开启正式通过：高置信 IANA reserved/special 隐式查询默认早收敛 `unknown @ unknown`；显式 `-h` 与 `--disable-address-preclass` 边界保持不变。Phase C `20/20`、special-purpose `17/17`、P0 `12/12`、P1 `232/232`、CIDR `4/4 + 9/9`、Step47 PASS、12x6 全绿（`out/artifacts/redirect_matrix_10x6/20260817-033253`）；最终 Strict 9 架构 hash/Golden/referral PASS（`out/artifacts/20260817-034423`，302s）。
...
- 对 LACNIC 内部重定向到 ARIN 的场景，若查询项为**非 IP 字面量**（例如 ASN/域名/句柄等），必须立即按非权威继续跳转，且不得提前将 ARIN 记为 visited；只有在后续真实 ARIN hop 形成稳定响应后，才允许按常规 visited 规则记账。
