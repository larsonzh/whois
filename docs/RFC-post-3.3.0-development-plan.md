...
| WP-12 | implemented/pending-release | Phase 5 | GPL-3.0 → MIT 许可证迁移 | 变更已提交（`0202efc6`）并推送 `origin/master`；自 v3.3.3 正式发布版本起生效，v3.3.2 仍为 GPL；发布前须完成权利来源确认（2026-08-29 登记，2026-09-03 v3.3.3 发布准备） |
| WP-13 | done | Phase 5 | 通过代理访问 RIR 站点 | WP-13A、WP-13B-1/2/3、WP-13C、WP-13D 均已交付；官方远程静态构建与 release 制品默认启用 `WHOIS_TLS=1`，本地保留无 OpenSSL 兼容构建；A/B 与完整生产回归全 PASS（2026-09-03） |
...
### 11.8 v3.3.2 发布候选验证（2026-08-28）

- 触发：v3.3.1 后新增 WP-09 生产修复、WP-10 审计结论与 ProofRail 产品 RFC；按封板目标编制 v3.3.2。
...
- WP-13D CA 契约（2026-09-01）：默认使用构建时固定并嵌入静态二进制的 Mozilla CA bundle，保持九架构单文件发布；非空 `SSL_CERT_FILE` 可覆盖以支持企业私有 CA，覆盖无效必须 fail-close 且不得回退。禁止依赖交叉编译 `OPENSSLDIR`、平台默认 CA 路径或 Windows 证书库。bundle 来源、日期、SHA-256、许可证与月度更新审计必须进入发布证据。
- WP-13D transport 生命周期（2026-09-01）：`wc_net_info` 继续作为单个 WHOIS hop 的连接所有者，并新增可选的堆分配自定义 transport；空指针表示按 `fd` 即时构造现有 bare-socket adapter，HTTPS proxy 成功后则把 TLS adapter/context 从 CONNECT 阶段移动到该连接对象，供 query send、body recv 和统一 close 全程复用。禁止把含有指向自身 `fd` 的 `wc_transport_t` 按值嵌入或复制，因为现有候选/empty-response 路径使用 `ni = ni4` / `*ni = ni2` 的移动式赋值。所有成功转移必须清空源所有权，所有失败、signal、send/recv 完成和 fallback 替换必须走同一连接关闭 helper，保证 TLS shutdown/context/free 与 socket close 恰好一次。
- WP-13D 构建接入（2026-09-01）：使用显式 `WHOIS_TLS=1` 编译特性开关；普通开发构建继续无 OpenSSL 依赖，识别到 `https://` 但后端未编译时须在 lookup 前稳定报 unsupported。九架构发布构建从 `/home/larson/.local/openssl-static/3.5.8/<target>` 的隔离 `pkgconfig` 目录调用 `pkg-config --static --cflags/--libs openssl`，复用 TLS dependency probe 已验证的完整参数及链接顺序；启用特性后缺少目标 prefix、静态库或 pkg-config 元数据必须硬失败，禁止回退动态库或宿主库。
- WP-13D 实施切片（2026-09-01）：先以传统交互式开发完成固定 CA bundle 获取/摘要/许可证、确定性 C 数据生成器和 Makefile/remote-build 注入，因为 Vx checker 只对 `c-source`/`c-header` 执行 effective syntax gate；再以 Vx A 任务完成 `wc_net_info` 自定义 transport 所有权、统一 init/move/close 及 direct-path 回归；最后以 Vx B 任务完成 HTTPS scheme/config、OpenSSL TLS adapter、deadline 内 handshake + CONNECT、证书/SNI/主机名验证、错误分类和 fake/loopback selftest。B 必须把 A 作为 prerequisite，并覆盖 connect、empty-response fallback、send、recv、signal 和 referral hop 的完整连接闭包。
- WP-13D CA/构建准备切片完成（2026-09-01）：固定 Mozilla CA bundle `2026-08-13`（121 张证书，SHA-256 `f66dff1bdf8f96060b8177976f8b7d9254bc89bc4db933d769f7384d28480bc9`，MPL-2.0），生成器通过成功、重复输出一致和篡改摘要拒绝验证；`WHOIS_TLS=1` 从每目标 OpenSSL 3.5.8 pkg-config 隔离目录注入静态参数，默认路径仍无 OpenSSL 依赖。TLS/LTO 九架构构建、9/9 本地哈希、Windows full-static 模式与依赖审计 PASS（`out/artifacts/20260901-170601`）；Windows 仅导入系统 DLL，loongarch64 仅有既有 `libc.so.6` NEEDED。
- WP-13D 已完成实施与生产验收（2026-09-03）：A=`testdata/autopilot_code_step_tasks_20270805_20270811.json` 负责 `wc_net_info` 自定义 transport 所有权，B=`testdata/autopilot_code_step_tasks_20270812_20270818.json` 负责 HTTPS scheme、OpenSSL transport、CA/peer/hostname/IP 验证、共享 deadline 与 CONNECT 接入。A/B 各 8/8 PASS（run=`20260902-090840`/`20260902-160535`，合计 `0d 14:37:31`）；最终两轮 Strict 九架构/hash/Golden/referral、Batch 4/4、Selftest 5/5、12×6 redirect 72/72、CIDR 4/4+9/9 全 PASS。HTTPS proxy 仅在 `WHOIS_TLS=1` 构建启用，普通 `WHOIS_TLS=0` 构建继续无 OpenSSL 并 fail-fast unsupported；权威回填见 `docs/RFC-proxy-access.md` 第 14 节。
- 暂缓项联动：本工作包不改变网络读取早停、并发批量、RIR DNS 候选排序、健康/回退决策或权威判定（见 5.6 / 11.6）。

#### 12.2.8 WP-13A、WP-13B-1 至 WP-13B-3 与 WP-13C 实施记录（2026-08-29 至 2026-09-01）
...
- 新增 `tools/test/proxy_tls_dependency_spike.sh`，用于每个远程编译器验证 OpenSSL 客户端、peer verification、SNI、hostname verification API 与全静态链接。2026-09-01 使用 `tools/remote/bootstrap_openssl_static.sh` 部署并审核 OpenSSL 3.5.8 LTS：官方源码 SHA-256 与固定主指纹签名验证通过，七个 POSIX 目标和 win32/win64 的统一静态矩阵 9/9 PASS；许可证、CVE 与更新策略及逐目标 manifest/哈希/SPDX/依赖审计见 `out/artifacts/proxy_tls_dependency_matrix/20260901-072331`。该检查点解除 WP-13D TLS 依赖门禁；当时 HTTPS proxy 尚未实现，后续已按本节回填完成。
...
- WP-13B-2 第二对 Vx A/B 已完成（2026-08-30 至 2026-08-31）：A 为 `testdata/autopilot_code_step_tasks_20270708_20270714.json`（target set `150b6f49ab217b65e2c048f407023d8d74dc69feaf670ccfeb5db71ad52a099b`），B 为 `testdata/autopilot_code_step_tasks_20270715_20270721.json`（target set `3808d4e86be49df0170add303e6acdf7e9fe7178cd526d0a445d73ac94c7d8e3`）。A 8/8 一轮通过（run=`out/artifacts/dev_verify_multiround/20260830-133709`）；B 首跑 D1 后 `runtime-fail`（`.Count` 属性异常）触发 3 处编排脚本根因修复（`start_dev_verify_8round_multiround.ps1` 单行输出数组化、`task_definition_vx_checker.ps1`/`task_definition_vx_prerequisite_union.ps1` 的 Vx artifact 目录移动有限重试、`unattended_ab_session_guard.ps1` A 快照锚点保护/重载）并恢复 start-file `A_SUCCESS_SNAPSHOT_*` 锚点，经标准 stop/reset/launch-ready 后 stage-window 重启（B pid 13396）从 D1 续跑 8/8 收敛（run=`out/artifacts/dev_verify_multiround/20260831-032025`）；SESSION=PASS，A/B 合计 `0d 18:51:00`（session start 2026-08-30 13:36:33）。事件票 `T20260831-024551167-8db236de`、`T20260831-024556280-6af52afe`、`chat-final-20260831-082732`（handled_at 2026-08-31 08:28:44）闭环。运行后九架构 Strict `lto-auto` 远程构建冒烟同步 + 黄金校验无告警 PASS（`out/artifacts/20260831-091653`，254s；Local hash verify/Golden/referral 全 PASS）。默认直连、stdout、RIR referral、DNS-health、batch strategy 与 retry metrics 契约不变；权威双语回填见 `docs/RFC-proxy-access.md` 第 11 节。WP-13C 已完成；WP-13D 的 TLS 依赖门禁现已通过，仍须独立 ready 评审及生产验收。
- WP-13B-2 收尾编辑器 IntelliSense 兼容修正（2026-08-31）：`src/core/proxy.c` `wc_proxy_selftest` 的 `UINT64_MAX` 在 VS Code C/C++ IntelliSense（windows-msvc-x64 + clang-cl）下报“编号的预期结尾后有多余文本”（code 19）误报，真实 GCC/Clang C11 与远程九架构构建不受影响。改为自测局部 `const uint64_t no_deadline = ~(uint64_t)0;` 并复用（数值与 `UINT64_MAX` 完全等价、不依赖实现特定整数后缀）；仅自测路径改动，生产 CONNECT/NO_PROXY 逻辑与输出契约零变化。修正后 Strict Version `lto-auto` 远程构建冒烟同步 + 黄金校验重跑无告警 + lto 无告警 + Local hash verify/Golden/referral 全 PASS（`out/artifacts/20260831-095325`，270s；本地重算 9/9 SHA-256 与 `SHA256SUMS-static.txt` 完全一致；IANA → ARIN → AFRINIC referral 链路正常收敛）。
- WP-13B-3 SOCKS5/5h 数据面完成（2026-08-31）：实现 method negotiation、RFC 1929、IPv4/IPv6/domain ATYP、完整 REP 分类与 terminal policy；`socks5://` 保留本地解析，未 bypass 的 `socks5h://` 使用逻辑 hostname 并阻断本地目标 DNS、DNS-health/backoff 与 empty-response IP fallback，远程解析后的目标 IP 保持 unknown。fake transport 与 loopback 探针覆盖协议和 DNS policy，21/21 PASS（`out/artifacts/proxy_protocol_spike/20260831-030507`）。严格 ISO C11 clang 零诊断；远程预检发现并修复 loongarch64 `strdup` 隐式声明。最终 Strict `lto-auto` 九架构、9/9 hash、POSIX/QEMU/Windows smoke、Golden 与三起点 referral 全 PASS（`out/artifacts/20260831-112633`，436s）。带 release 同步的最终复核：无告警 + lto 无告警 + Local hash verify/Golden/referral 全 PASS，284s，`lzispro`/`whois` release 双目录与 `SHA256SUMS-static.txt` 逐字一致（`out/artifacts/20260831-114158`）。WP-13C/13D 边界不变。
- WP-13C SOCKS4/4a 完成（2026-08-31 至 2026-09-01）：`socks4://` 仅向代理发送本地解析的 IPv4 候选（不宣称 IPv6）；`socks4a://` 对未被 per-hop `NO_PROXY` 绕过的域名使用逻辑 hostname，与 `socks5h://` 共用目标 family/fallback/RIR override 冲突门禁与目标健康隔离；USERID 为解析后的 username，password 不进入帧。loopback 探针扩展至 27/27 PASS（`out/artifacts/proxy_protocol_spike/20260831-043917`）。Vx A/B 均 8/8 一轮通过、无 incident/自愈/重启：A 为 `testdata/autopilot_code_step_tasks_20270722_20270728.json`（target set `d60f26e6efbd732de60766306ec9d65a1a19cf2e32b9422c6514ace84ddd8eb7`，run=`out/artifacts/dev_verify_multiround/20260831-150933`，`2026-08-31 15:08:58 → 20:51:29`，A elapsed `0d 05:42:32`），B 为 `testdata/autopilot_code_step_tasks_20270729_20270804.json`（target set `adc9aea696e8a74705a52b5cd37362b37b4913b9075a2ac11f4ffbac9f554361`，run=`out/artifacts/dev_verify_multiround/20260831-205121`，`2026-08-31 20:50:40 → 2026-09-01 03:10:37`，B elapsed `0d 06:19:58`）；A/B 合计 `0d 12:01:40`（session start 2026-08-31 15:08:58）。事件票 `T20260831-205129509-bb7b885e`（handled_at 2026-08-31 20:54:38）、`chat-final-20260901-031037`（handled_at 2026-09-01 03:12:26）闭环。运行后九架构 Strict `lto-auto` 远程构建冒烟同步 + 黄金校验无告警 PASS（`out/artifacts/20260901-121843`，755s；Local hash verify/Golden/referral 全 PASS）。默认直连、stdout、RIR referral、DNS-health、batch strategy 与 retry metrics 契约不变；权威双语回填见 `docs/RFC-proxy-access.md` 第 13 节。WP-13D 的 TLS 依赖门禁现已通过，仍须独立 ready 评审及生产验收，不得提前开放。

### 12.3 发布顺序提醒

- v3.3.2：已于 2026-08-28 发布（仅含 v3.3.2 既有变更，**不含许可证迁移**），产物版本精确为 v3.3.2。
- v3.3.3：WP-12 许可证迁移与 WP-13 代理访问（WP-13A–13D）均已实施并通过全部生产门禁，源码版本号已置为 3.3.3，`docs/release_bodies/v3.3.3.md`、README、RELEASE_NOTES 与进度文档已同步（2026-09-03）；正式 release 制品由 One-Click Release 以 `WHOIS_FORCE_VERSION=v3.3.3` 强制重建。发布前按第 7 节完成完整回归并满足第 12.1 节权利来源门禁。
