...
- 会话内主动拉取建议使用 `tools/test/poll_agent_tickets.ps1`（建议每 5~10 分钟执行一次）。脚本会返回待处理工单，并为每张工单生成两段执行指令：`business_command`（业务恢复动作）与 `continue_watch_command`（继续监控并保持会话在线节奏）；若返回 `mark_processed_command`，应在前两段执行成功后立即执行以回写完成标记，避免跨轮次重复拉取。当前脚本也会显式返回 `receipt_type=handled_at` 与 `handled_receipt_command`，表示每张短票在完成当轮动作后都必须回传 handled_at 回执。
- 在无人值守运行期间，以上工单流属于预授权既定工作，AI 不应为这些标准步骤反复询问用户是否执行。
- 对 `running-status-report`（定时状态票），`poll_agent_tickets.ps1` 默认会填充状态检查型 `business_command`。在 `normal/anti-missent` 下，默认仍包含主进程健康检查与轻量巡检；在 `low-disturb` 下，会收敛为最小健康检查，只保留当前运行状态与主进程/监控链真实存活检查（避免把残留 `-NoExit` 空壳 shell 当作健康主进程），以降低常态打扰。若检查结果正常且未触发自愈/故障处理，聊天回报仅保留“运行正常”与 `handled_at`；若检查结果异常，或触发了自愈/故障处理，则按 normal 口径回复完整状态。
