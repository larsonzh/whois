...
极简一行版（聊天框速贴）：
从现在起，会话内代理保持在线并静默等待现有 guard/trigger/dispatch 链投送的事件驱动票或状态票；收到工单后按其中的指令与 next_command_order 执行无需用户确认的预授权操作，完成回执后继续静默等待；等待期间不得主动定时调用 heartbeat/poll，不得创建或运行定时巡检脚本、轮询循环、后台 job、watcher 或常驻内存命令；仅独立事故票可在停机门禁后进入修复闭环，状态票只读汇报并回传 handled_at；不得手工创建心跳/回执文件或非 tmp 新脚本；仅在 A/B 都到终态或我明确下达“停止监控”时结束。

短版（默认推荐）：
从现在起，会话内代理保持在线并静默等待 guard/trigger/dispatch 投送工单，不主动拉票或发送定时 heartbeat；收到事件票后按 next_command_order 执行无需用户确认的预授权操作并回执，收到状态票时只读汇报；处理完继续静默等待。禁止创建或运行定时巡检脚本、轮询循环、后台 job、watcher 或常驻内存命令。任务定义修复必须先做故障 op 目标检查，再做全部受影响 D 轮整轮检查，检查通过后只通过标准 stage window 重启票据对应阶段；不得手工创建心跳/回执文件、非 tmp 新脚本或执行未授权 commit/push；仅在 A/B 都到终态或我明确下达“停止监控”时结束。

强约束版（高风险轮次推荐）：
从现在起，会话内代理保持在线并静默等待现有投送链发送事件票或状态票；等待本身不执行任何命令，不主动定时 heartbeat/poll，也不创建或运行巡检脚本、轮询循环、后台 job、watcher 或常驻内存命令。收到工单后严格按 next_command_order 执行无需用户确认的预授权操作并回执；任务定义故障只改允许边界内 operations，先运行故障 op 的 -OperationIndex 目标检查，再运行所有受影响 D 轮的整轮严格检查；设计时空轮才可用最小 noop，吸收/幂等态必须保持 regex-patch；两层检查与修复证据通过后仅用标准 stage window 重启票据对应阶段；重启前必须采证；运行期不得手工创建心跳/回执文件、非 tmp 新脚本或执行未授权 commit/push；仅在 A/B 终态或我明确停止时结束。

## 强绑定句（建议原样保留）
进入事件驱动被动接收模式：静默等待现有 guard/trigger/dispatch 链投送工单，后续动作只由工单指令触发；不得自建或运行任何定时巡检脚本、轮询循环、后台 job、watcher 或常驻内存命令。
...
4. guard/trigger/dispatch 链负责运行期监控、产票与消息投送；Copilot 会话保持在线并静默等待工单，收到后响应。Copilot 不得自行执行按节奏轮询，不得创建或运行定时巡检脚本或常驻内存命令。
5. 运行期处置采用票据驱动策略：
