...
13. 事件驱动票与定时状态票中的既定工作内容视为预授权动作：AI 在无人值守运行期间直接执行 `business_command -> continue_watch_command -> mark_processed_command -> handled_receipt_command`，不再逐项向用户确认；`handled_at` 为强制回执字段。
