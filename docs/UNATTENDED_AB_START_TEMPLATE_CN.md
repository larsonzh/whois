...
- 建议保留 `running-status-report` 在 `LOCAL_GUARD_POLL_STATUS_REPORT_EVENTS` 与 `LOCAL_GUARD_POLL_DRAIN_SAFE_EVENTS` 中；若需要扩展 drain 白名单，可将 `manual-wait-paused`、`budget-exhausted-stop`、`known-infra-transient-stop` 按策略加入。
