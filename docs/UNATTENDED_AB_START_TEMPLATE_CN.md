...
- `TASK_STATIC_CROSS_ROUND_REPAIR_ENABLED` 只控制 task-static 代码自愈工单的范围，默认 `false`；字段缺失、空值或非法值均按关闭。关闭时使用默认单轮事务，只修当前故障 D 轮；开启时使用同一 candidate 修复故障轮到 D4，并以 `Validate -ValidateThroughRound D4 -ChainRounds` 一次链式验证完整序列，全部通过后只 Promote 一次。它不作用于 code-step，也不扩大编译/验证代码故障的追加式修复边界。
