...
- `LOCAL_GUARD_STATUS_TICKET_ENABLED` 与 `LOCAL_GUARD_STATUS_TICKET_INTERVAL_MINUTES` 用于定时上报运行状态工单（event=`running-status-report`）；模板默认开启状态票交互分发（`AI_CHAT_DISPATCH_DELIVERY_PROFILE=interactive-smoke`、`AI_CHAT_DISPATCH_STATUS_REPORT_INTERACTIVE=true`、allowlist 含 `running-status-report`），模板默认间隔为 10 分钟；如需降噪可按场景调大。
