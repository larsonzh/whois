...
Stress plan note (2025-12-31): manual long-line/dense-continuation/CRLF scenarios (fold-unique + grep block) now pass with no truncation/crash; latest remote smoke + golden logs: `out/artifacts/20251231-145229` (default, PASS) and `out/artifacts/20251231-145548` (debug+metrics+interleave-v4-first, PASS). For quick regressions, add `--selftest-workbuf` to trigger built-in long-line/CRLF/high-continuation coverage; stderr emits `[WORKBUF]`/`[WORKBUF-STATS]` markers for golden checks.
