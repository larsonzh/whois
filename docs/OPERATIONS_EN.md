...
CIDR body-contract smoke task (2026-02-20): VS Code now includes `Test: CIDR Body Contract` (parameterized) and `Test: CIDR Body Contract (prefilled)` (one-click preset), both mapped to `tools/test/cidr_body_contract_smoke.ps1`; latest local run is `pass=2 fail=0` (log `out/artifacts/cidr_body_contract/20260220-124043`).
