...
Post-3.3.0 WP-01 real one-click release rehearsal review (2026-08-25): the v3.3.1 release passed the full one-click flow (build+verify → statics sync/commit → annotated tag v3.3.1 → GitHub/Gitee Release update) with no incidents; the tag points at the final statics commit `4357c6a0`, the artifact `out/artifacts/20260825-101612` shows golden PASS and `18/3/3` smoke with zero alerts, the Gitee Release body update exited 0, and origin/master is synchronized. WP-01 is closed (done).

Post-3.3.0 WP-07 final rebuild review (2026-08-24): after the fold token capacity reservation optimization, the default Strict `lto-auto` round has no compile/LTO warnings; all nine local hashes, Golden, and all three referral starts pass (`out/artifacts/20260824-205103`, 311s). Linux/QEMU, win32, and win64 smoke logs complete `18/3/3` queries with matching query-header and authoritative-tail counts and zero alerts; IANA, ARIN, and AFRINIC starts all converge to AFRINIC. Independent SHA-256 verification is `9/9` for the artifact, repository release directory, and external lzispro release directory; both win32 and win64 builds are full-static. No additional code fix is required.

Post-3.3.0 WP-02 final artifact review (2026-08-24): after incorporating the `src/cond/fold.c` fix for reading stale pointers following workbuf growth, the default Strict `lto-auto` round has no compile/LTO warnings; all nine local hashes, Golden, and all three referral starts pass (`out/artifacts/20260824-094617`, 299s). Linux/QEMU, win32, and win64 smoke logs complete `18/3/3` queries with matching query-header and authoritative-tail counts and zero alerts. Independent artifact verification is `9/9`, and both the repository and external lzispro sync directories also match `9/9`. No additional code fix is required.

Post-3.3.0 WP-01 rebuild review (2026-08-24): after correcting the `injection-view-fallback` standalone selftest assertion and adding it to the core golden gate, the default Strict `lto-auto` round has no compile/LTO warnings; all nine local hashes, Golden, and all three referral starts pass (`out/artifacts/20260824-081341`, 281s). Linux/win32/win64 smoke logs complete `18/3/3` queries with matching query-header and authoritative-tail counts and no hard errors. The Strict win64 standalone `--selftest` exits 0 with fallback PASS=1/FAIL=0. All nine hashes in both the repository and external lzispro sync directories match the artifact manifest. This is a post-v3.3.0 fix verification and does not rewrite the historical 2026-08-23 release-candidate state below.

Release-candidate verification record (2026-08-23): after the response-classification fix, default/debug Strict `lto-auto` rounds have no compile/LTO warnings, both nine-architecture SHA-256 lists recalculate with `9/9` matches, and Golden/referral checks pass (`out/artifacts/20260823-151731`, 292s; `20260823-152347`, 281s). All four Batch strategies pass (`152928/153443/154054/154744`); all four Selftest strategies plus the standalone core golden gate pass (`155355/155909/160500/161048`), matching every new classification-priority PASS line and forbidding each FAIL form. The 12x6 authority-mismatch table is empty with no errors (`redirect_matrix_10x6/20260823-161855`); the CIDR bundle passes body `4/4` plus draft `9/9` and exits 0 (`cidr_bundle_summary_20260823-163528.txt`). The complete standalone core raw log still contains pre-existing network WARN/SKIP results and `injection-view-fallback: FAIL`; it must not be described as an all-green standalone run, but it is not a regression from this change. No code fix is required. Next steps are limited to version, release body, tag, and GitHub Release closeout; rerun affected gates after any source, test, or artifact change.

Release-candidate verification record (2026-08-16): the default and debug Strict rounds were labelled `lto-auto` at the time but are now confirmed to have used O3; they have no compile warnings and pass the nine-architecture SHA-256, Local hash, Golden, and referral checks (`out/artifacts/20260816-203059`, 276s; `20260816-203903`, 286s). All four batch goldens pass (`204504/205026/205622/210225`, 1315.436s), as do all four selftest goldens (`210950/211526/212129/212741`, 1369.958s). The final 12x6 run has an empty authority-mismatch table and no errors (`out/artifacts/redirect_matrix_10x6/20260816-213231`). The original empty-APNIC transient remains under `20260816-161935`; no repair was required for this run.
...
LTO alias audit correction (2026-08-18): `remote_build_and_test.sh` previously normalized `RB_OPT_PROFILE` before `getopts`, so command-line `-O lto-auto|lto-serial` never became the Makefile-supported `OPT_PROFILE=lto`. Historical runs labelled `lto-auto` remain valid cross-architecture compile, hash, Golden/referral, and behavioral evidence, but they are not actual LTO-warning evidence. Alias normalization now runs after argument parsing, and `remote_build.sh` explicitly exports `LTO_MODE/LTO_SERIAL/LTO_PARALLEL` for POSIX and Windows targets. In the final true-LTO run `out/artifacts/20260818-065540`, every architecture compiles and links with `-flto=auto`; all nine architectures pass Local hash, Golden, and the three referral checks, and release artifacts are synchronized (455s).

Post-commit `7e1f2fa7` verification (2026-08-18): Strict `lto-auto` default/debug rounds show no compile or LTO warnings and pass Local hash, Golden, and referral checks (`out/artifacts/20260818-071217`, 224s; `20260818-072952`, 255s). The default round's POSIX hashes match the true-LTO baseline `065540` exactly (Windows PE binaries differ only by embedded timestamps), confirming LTO is really enabled with deterministic builds. All four Batch goldens (`073512/073943/074445/075120`, 1185.080s) and all four Selftest goldens (`080108/080605/081113/081614`, 1201.862s) pass; selftest reports include `output line matched: 10\.0\.0\.8 is a private IP address`, proving the `--require-line` real-output assertion is active. The 12x6 matrix has an empty authority table; the only `afrinic_45.113.52.0_22` error is an external LACNIC rate-limit that recovers to APNIC on the targeted rerun (`redirect_matrix_10x6/20260818-083320`), so it is not a regression. Release artifacts are synchronized to the latest debug round.
...
## A/B Session Guard (Low-Noise + V1 Auto-Fix Loop)

- Low-noise logging (2026-04-25): `tools/test/unattended_ab_session_guard.ps1` now prints restart pasted blocks as split begin/end + separator lines, making grep/manual review easier without changing gate semantics.
- New start-file keys (see `docs/UNATTENDED_AB_START_TEMPLATE_CN.md`):
  - `LOCAL_GUARD_AUTO_FIX_BUDGET_PER_D`: max auto-fix attempts per D round (recommended `3`).
  - `LOCAL_GUARD_AUTO_FIX_COOLDOWN_SECONDS`: cooldown after each auto-fix (recommended `20`~`60`).
  - `LOCAL_GUARD_MANUAL_WAIT_ON_EXHAUST`: enter manual wait when budget is exhausted (recommended `true`).
...
49/50 gap-validation record (2026-08-16): the Windows `getopt_long` shim now performs GNU-style argument permutation, fixing trailing options such as `query -h arin`; `opts-permuted-parser` guards the behavior. CIDR body contract is `4/4 PASS` and draft TSV is `9/9 PASS` (`out/artifacts/cidr_bundle/cidr_bundle_summary_20260816-042354.txt`). Initial IPv4/parameterized Redirect Matrix runs were both `66/66 PASS`; stable 10x6 is `authMismatchFiles=0/errorFiles=0`; `remote_batch_strategy_suite.ps1` reports raw/health-first/plan-a/plan-b all `[golden] PASS` in `1775.592s`. Direct `--selftest` execution passes the new Phase C parser/policy assertions, while the complete legacy selftest still reports `injection-view-fallback: FAIL` plus network WARN/SKIP results; do not report the standalone run as all green.
Golden-script boundary: `golden_check_batch_presets.sh`/`golden_check_batch_suite.ps1` assert headers, tails, redirects, and batch tags; `golden_check_selftest.sh` asserts `[SELFTEST]` expectations in selftest-only logs. Running `whois-win64.exe --selftest` is a separate core selftest and is not equivalent to Batch Golden; record any standalone legacy failure independently.
...
1. Run the remote smoke with plan-a enabled:
   ...
   ```
    ...
    1. Full example (remote fetch + `[SELFTEST] action=*` assertions):
      ...
      - `-SelftestExpectations`, `-ErrorPatterns`, `-LinePatterns`, and `-TagExpectations` accept semicolon-separated lists that become `--expect`, `--require-error`, `--require-line`, and `--require-tag component regex` arguments for `golden_check_selftest.sh`. Leave them blank or type `NONE` to skip a category. The default `LinePatterns` requires the complete body line `10\.0\.0\.8 is a private IP address`, proving force-private reached the real private branch rather than only emitting startup markers; CRLF line endings are normalized before matching.
...
- Fastmode A/B wrappers now execute a hard remote-lock precheck (via `tools/dev/check_remote_lock.ps1`) before launch; if the remote lock is present, lock state is unexpected, or SSH lock check fails, startup is blocked immediately.
...
- Task-definition self-heal restart gate (2026-06-13): if a recovery action modifies task-definition content, `tools/test/check_task_definition_static.ps1` must pass before any `stage_restart` or `business_resume`; do not restart/resume while static precheck is failing.
- Stage-target correctness guard (2026-06-13): restart only the main process of the ticketed stage (`A` issue -> restart `A`, `B` issue -> restart `B`), and always use the stage-window entry (`tools/test/open_unattended_ab_stage_window.ps1 -Stage A|B -StartMonitors`); do not guess/swap stage targets or bypass stage-window routing.
...
- Final-summary dispatch liveness guard (2026-05-19): `tools/test/unattended_ab_takeover_trigger.ps1` now applies "post-start liveness check + fast retry" on the `chat-session-final-status` dispatch path. Tune with `AI_CHAT_FINAL_TRIGGER_VERIFY_MS` (default 1200) and `AI_CHAT_FINAL_TRIGGER_MAX_ATTEMPTS` (default 2); when dispatch is not confirmed, trigger writes `auto_stop_deferred` and stays resident instead of auto-stopping early.
