...
- `schemaVersion=2`：允许多文件定义，同时保留 V1 字段语义。
- 若 V2 文件仅使用 V1 字段（`targetFile` + operation 不含 `target`/`file`），运行行为必须与 V1 一致。

### 3.2 新增/扩展字段

- 根级字段：
  - `targetFile`（可选，历史默认目标）
  - `targetFiles`（可选，V2）：命名目标数组
  - `defaultTarget`（可选，V2）：当 operation 未显式指定目标时使用的目标 id
...
- V2 增加按目标存储 baseline 的目录，例如：
  - `_code_step_state/target_baseline/<target-id>.baseline`
- `-Reset`（restore-source）在 V2 下应恢复所有被跟踪目标。
...
- 范围：
  - 解析 V2 字段（`targetFiles`、`defaultTarget`、operation 的 `target`/`file`）
  - 实现按目标文件的文本映射执行引擎
...
- 高风险：V2 下 restore-source 语义在“仅部分目标 baseline 与 HEAD 不一致”时可能出现恢复语义不一致。
...
- 增加一份真实 V2 任务定义用于 dry-run 验证。

Phase 3：
- 完成上线文档与迁移说明。
- 在新的 A/B 清单中推广 V2 模板。

## 11. 双轨实施建议（V1 保留 + V2 复制）

为降低改造风险，建议采用双轨并行：

- V1 轨道：冻结现有行为，不做语义改动，仅接收必要 bugfix。
- V2 轨道：从 V1 复制生成独立脚本与模板，在副本上实现多文件能力。
...
- `tools/test/autopilot_code_step_rounds_v2.ps1`
- `tools/test/start_dev_verify_8round_multiround_v2.ps1`
- `tools/test/start_autopilot_8round_code_change_v2.ps1`
- `testdata/autopilot_code_step_tasks_template_v2.json`

### 11.2 入口分流建议

- V1 入口维持默认，继续服务既有任务定义（`schemaVersion=1`）。
- V2 入口默认读取 V2 模板（`schemaVersion=2`），仅用于多文件任务。
- 在 VS Code 任务层增加明确命名（例如 `... (V1)` / `... (V2)`），避免误跑。

### 11.3 回退策略

- 若 V2 任一阶段出现稳定性问题，直接切回 V1 入口继续执行。
- V2 问题修复后重新 dry-run，不影响 V1 交付节奏。

## 12. 落地执行清单（按双轨方案）

### 12.1 第 0 步：复制与冻结

1. 复制 V1 脚本/模板生成 V2 文件。
2. 在 RFC 中记录“V1 冻结基线提交哈希”。
3. 约定：V1 仅允许兼容性 bugfix，不引入 V2 功能。

### 12.2 第 1 步：V2 执行器能力

1. 在 `autopilot_code_step_rounds_v2.ps1` 实现 `targetFiles/defaultTarget/operation.target|file`。
2. 增加 V2 多目标 baseline 与 reset 恢复。
3. 保持 V1 日志兼容，同时新增多文件汇总行。

### 12.3 第 2 步：V2 包装器质量闸

1. 在 `start_dev_verify_8round_multiround_v2.ps1` 改造多目标 no-op/source-delta 判定。
2. 增加 target 解析失败、未知 target id 的 fail-fast。
3. 与现有 D1 reset guard 保持一致口径。

### 12.4 第 3 步：验证与发布

1. 先跑 V2 dry-run（单轮 + 多轮）。
2. 再跑完整无人值守 golden 流程。
3. 验证通过后，将 V2 模板纳入后续 A/B 清单；V1 继续保留。
