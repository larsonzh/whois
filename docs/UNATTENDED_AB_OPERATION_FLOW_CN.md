...
- 温窗期结束且仍确认主进程故障退出后，guard 写入 FAIL 与故障证据，并按既定策略生成完整事件票；随后才进入宽限期（默认 20 分钟，由 `MONITOR_CHAIN_GRACE_MINUTES` 或 `LOCAL_GUARD_MAIN_EXIT_MONITOR_GRACE_MINUTES` 控制），保持监控链在线以等待恢复、重锚定和票据闭环。
