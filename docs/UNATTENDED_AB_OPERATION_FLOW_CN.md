...
- 代码自愈修复不允许直接手改源码，也不允许直接编辑正式任务定义；必须通过 2.10.6 的候选事务修改当前阶段允许轮次的代码改动内容，验证后原子提升正式文件。
- 运行期候选修改后，必须先通过 `-SyntaxOnly`，再通过故障目标 op 快检（可定位时）及当前故障 D 轮递进严格检查，并完成 Promote 的哈希/receipt 门禁，才允许重启本阶段主进程；后续轮在实际 code-step 到达时检查。
...
### 2.11 任务切片设计与合并原则（通用规则）

**设计原则**：当前代码已趋于成熟稳定，A/B 任务设计中任何代码改动都必须全盘深入考虑与评估后再做出，最大限度减少缺陷的产生，不给代码整体执行带来负面影响；严禁为“凑轮次”引入不必要或未经充分论证的改动。

**合并原则**：A/B 无人值守脚本本身已成熟稳定、执行流畅，但编译/验证环节较多、任务整体耗时较长。当单个任务切片代码开发量较小（单独占用一次 A/B 会造成轮次任务不饱和、时间成本浪费过大，发挥不出无人值守脚本的任务执行优势）时：

1. 优先评估将多个独立、无冲突的小切片合并编排进同一次 A/B 无人值守任务，而不是逐个单独占用完整任务窗口；若单个切片确实过小，也可评估直接采用传统交互式开发而非 A/B 无人值守。
2. 合并前提：各合并切片的目标文件/函数域相互独立、无依赖冲突，合并后的 D1-D4 轮次编排必须通过完整静态检查与真实编译验证。
3. 验证兜底：若现有 A/B 任务的验证测试无法覆盖某切片特有需求，该任务结束后单独执行专项验证测试（如 Phase C review、special 矩阵、CIDR 契约、12x6、Strict 多架构等），并记录证据。
4. 不得破坏验收语义：有独立评审要求的改动（如默认翻转、开关放量）仍须独立任务或独立评审记录，不得借合并混批绕过。
5. 失败面控制：合并后任一切片失败即按既有 first-fail-stop 规则阻断整轮，故每片应保持小而独立，合并数量适度，避免一轮承载过多改动导致失败面过大。

### 2.12 event-only 重设计、轻运行的成本结论（2026-08-18，已验证）

> 经验来源：event-only 模式 A/B 无人值守任务的实际运行观察。

- **现象**：前期链式验证（A 全定义静态检查 → B 以 A 为前置的链式检查 → 编译/黄金/Step47）的任务定义设计阶段 token 消耗较大；但任务设计成功率非常高——只要不是环境等外在原因导致任务故障，任务执行基本一次性通过，运行中故障消息与代理参与度极低。
- **结论（降本增效）**：无人值守体系“重设计、轻运行”的设计哲学已得到实战验证。设计期多投入的验证成本是一次性的，换来的是每次执行可复利的一次通过收益；运行期零故障意味着 guard 产票、代理分析、自愈/重启、复验等隐形成本全部消失。
- **指导意义**：
  1. 编制任务定义时，应优先保证链式验证的完整性（宁可设计期多投入时间/token），而不是压缩设计期、把缺陷留给无人值守运行期暴露；
  2. event-only 模式下代理静默等待事件驱动票、参与度极低，适合高成功率任务；若预期任务设计成功率低，才需要评估运行期介入的成本；
  3. 该结论支持 2.11 的合并策略：小切片合并进同一次高质量 A/B 任务，用一次设计期验证覆盖多个改动，进一步摊薄单位成本。
...
- 本轮是否需要外部 NoExit 窗口
- 本轮任务切片的粒度与合并策略是否符合 2.11（是否过小、是否需要合并多个独立小切片）
...
- 初始编制期可设计和调整 D1-D4 全部内容，但每次修改仍须满足本节安全契约。start-file 一旦生成并进入运行期，后续修改必须服从事故阶段、故障轮次和 operation 只读边界。
...
#### 4.2.1 初始编制与运行期修复的编辑边界

| 场景 | 允许代理编辑的位置 | 是否使用候选事务 | 验证完成事实 |
|---|---|---|---|
| start-file 尚未生成，新建任务定义 | 新建的正式 `testdata/*.json`；D1-D4 均可按设计调整 | 否 | 正式文件通过 `-SyntaxOnly`、完整静态检查及适用的编译/测试 |
| start-file 已生成但尚未启动，需要修订计划 | 先确认没有 A/B 业务进程；修改正式文件会改变启动计划，应重新执行本节完整验收并重新生成或重新预检 start-file | 通常否；不得伪造事故票 | 新正式文件完整验收通过，start-file 重新绑定并通过 launch-ready |
| 运行期 task-static 或经确认的代码故障 | 只编辑事务目录中的 `candidate.json`，并服从故障轮/op 前向边界 | 是，`Prepare -> Inspect -> Validate -> Promote` | 同票据 manifest/receipt/hash 门禁通过且正式文件已提升 |
| code-step、noncode 或自愈关闭的脚本故障 | 不得编辑任务定义 | 否 | 按对应 noncode 或 diagnose-only 流程处理 |
...
目的：在生成 start-file 前完成任务定义的设计验收，避免把可预先发现的 operation 错误带入无人值守运行。

这里必须区分三种检查，不能再把它们统称为“启动前静态检查”：

1. **初始编制验收（本阶段）**：读取目标源码，模拟 operations，检查唯一匹配、replacement、marker、收敛、replay、`postApplyAssertions` 和 effective source。A/B 正式任务定义必须在 start-file 生成前完成全定义检查。
2. **启动入口装载门禁**：start-file 已生成后，launcher/launch-ready 对当前阶段任务定义只执行 `-SyntaxOnly`，只验证文件、JSON 与基础结构；不读取目标源码、不检查 D1-op1、不执行 operation 正则，也不生成 code-step 绑定产物。
3. **运行时 D 轮门禁**：每个 D 轮到达 code-step 前，独立 checker 以当时的 A 基线或 B snapshot 基线执行当前轮完整检查，生成哈希绑定的 effective source 与 manifest；code-step 只校验并原子应用该产物。运行期故障修复按 2.10.6 使用候选事务，不直接编辑正式文件。

编制期完整检查不能替代运行时轮次门禁，因为源码基线可能在 A/B 串行执行过程中变化；启动时的 `-SyntaxOnly` 也不能替代编制期完整验收，因为它故意不执行任何 operation。
...
```powershell
# 第一步：机械建档或每次结构性修改后先确认 JSON 可装载
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20261031_20261107.json -Policy enforce -SyntaxOnly

# 编制 A 时的局部反馈；不能作为最终验收
...
powershell -NoProfile -ExecutionPolicy Bypass -File tools/test/check_task_definition_static.ps1 -TaskDefinitionFile testdata/autopilot_code_step_tasks_20261031_20261107.json -Policy enforce -FailOnWarnings -RoundTag D1
...
start-file 尚未生成时没有“故障 op 之前只读”或“只能前向修复”的运行期限制。代理应：

1. 从 checker 的首个结构化错误定位当前正式任务定义中的 round/op。
2. 回到目标源码和相关 RFC 验证设计假设，不根据外层 wrapper 摘要盲改 regex。
3. 仅使用 VS Code `apply_patch` 修改正式新任务定义；不要修改业务源码来迁就 pattern，也不要使用终端字符串替换 JSON。
4. 先重跑 `-SyntaxOnly`；若正在迭代某个 op，可运行目标 op 和目标轮快检。
5. 局部检查通过后，重新运行 A 全定义检查；B 还要使用已通过的 A 定义执行 `-PrerequisiteTaskDefinitionFiles` 链式全检查。
6. 若修改影响生成源码结构，运行适用的编译、模块自测、黄金样例或 Step47 门禁。
7. 全部通过后才生成 start-file，并由 launch-ready 再执行 `-SyntaxOnly`、字段同步、编码、进程、SSH/锁等启动门禁。

若 start-file 已经生成或 A/B 曾经启动，不得把本节的“初始编制自由修改”当作绕过运行期编辑边界的依据；应先按会话状态判断是重新规划、重新生成 start-file，还是进入带 ticket 的候选事务修复。
...
3. 对任务定义候选执行 Inspect（推荐）、Validate 和 Promote；确认同票据 manifest/receipt/hash 门禁通过，并对提升后的正式文件完成当前故障轮严格检查。
...
- 自愈修复通过“修改事务 candidate + Validate/Promote + launch-ready + 重启本阶段”完成，不通过直接修改正式任务定义或源码完成。
...
自愈修复的源码修改应通过 2.10.6 候选事务变更当前阶段允许轮次的任务定义，验证后原子提升正式文件，而非直接编辑正式任务定义或源文件。
...
自愈修复不得把失败的 `regex-patch` 轮改成 `type=noop` 来消除 pattern、marker、replay 或断言错误。若执行时发现目标已由前置轮吸收，保留 `regex-patch` 并补齐逐 op 幂等证据；只有在该轮执行前已明确确认“从设计上无代码变更目标”、整轮均处于允许编辑范围、且不存在已执行或只读 operation/契约时，才可按 4.2.2 的空轮结构定义为 `noop`。不能满足这些条件时必须继续修复任务定义或报告阻塞，不得以 `noop` 绕过静态门禁。
...
#### 10.8.8 candidate.json 损坏或不可恢复时的回退方案
