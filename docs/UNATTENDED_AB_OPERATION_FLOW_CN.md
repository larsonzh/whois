...
- 温窗期结束且仍确认主进程故障退出后，guard 写入 FAIL 与故障证据，并按既定策略生成完整事件票；随后才进入宽限期（默认 40 分钟，由 `MONITOR_CHAIN_GRACE_MINUTES` 或 `LOCAL_GUARD_MAIN_EXIT_MONITOR_GRACE_MINUTES` 控制），保持监控链在线以等待恢复、重锚定和票据闭环。
...
2. 先判断路由属于哪一类，再决定修改面：
    ...
    - **[D1-D4 code-step 阶段失败]（code-edit-failure / task-definition-mismatch）**：源码尚未被当前轮次修改；当前故障 op 之前的 op 只读，仅可从故障 op 位置起修改、删除、插入或追加 op。
...
- **编译/验证阶段故障（D1-D4 轮次内）：**
  ...
  - 修复方式：只可前向变更任务定义：D1 故障可改 D1-D4，D2 可改 D2-D4，D3 可改 D3-D4，D4 仅可改 D4；不得改前置 D 轮。由于该轮次原有的 operation 已经执行完毕（源码已改变），该轮既有 op 只读，只能在该轮末尾连续追加一个或多个 op。
...
A/B 启动时基于当前源码执行 D1-op1 的入口静态检查（pattern_match=1 且无错误），以确保任务定义可以进入主流程。进入每个 D 轮次前，round runtime static gate 会对该轮全部 operations 做顺序 dry-run：op2 使用 op1 replacement 后的内存文本，依次类推，因此允许同轮后续 op 依赖前序 op 的增量结果。

code-step 使用同样的顺序语义，在每个 op 应用前检查 pattern 是否唯一匹配；当前 op 通过后先更新轮次内存文本，再检查下一个 op。任一 op 失败立即终止该轮，目标源码仅在整轮 operations 全部通过后统一写入，避免留下半轮源码状态。
...
A/B 启动
  ...
  → D1（整轮顺序 dry-run → code-step 逐 op fail-fast → strict build gate）
  → D2（整轮顺序 dry-run → code-step 逐 op fail-fast → strict build gate）
  → D3（整轮顺序 dry-run → code-step 逐 op fail-fast → strict build gate）
  → D4（整轮顺序 dry-run → code-step 逐 op fail-fast → strict build gate）
  → V1（verify round）
  → V2（verify round）
  → V3（verify round）
  → V4（verify round）
...
1. **目标 op 快检**：仅用于 code-step 故障修复后的快速定位。使用 `-RoundTag <Dn> -OperationIndex <n>` 顺序模拟只读前置 op，并检查目标 op 的匹配、replacement 可落地性、marker 所有权、pattern 收敛和孤儿函数体风险。
2. **整轮严格检查**：任何任务定义变更在重启前都必须再运行不带 `-OperationIndex` 的 `-RoundTag <Dn>` 检查；若修改跨越多个允许的后续 D 轮，则逐轮检查所有受影响轮。整轮检查负责验证完整 replay 稳定性与 `postApplyAssertions`，不能由目标 op 快检替代。
...
故障发生时该轮次的源码改动尚未生效（code-step 阶段中断），当前源码状态为该轮次开始执行之前的基线。AI 修改任务定义后，先对触发故障的当前 op 执行一次目标静态检查，再按本节开头的两层规则执行受影响轮的整轮严格检查。使用 `-RoundTag <Dn> -OperationIndex <n>` 时，checker 会按顺序模拟同轮 op1 至 op(n-1) 来构造目标输入，但只把 op(n) 作为目标检查；前置 op 仍为只读，不因模拟而扩大编辑范围。
...
- 静态门禁按上述允许范围检查任务定义变更，不得仅检查或仅允许当前 D 轮。

- 若该轮次有**修改**或**追加**的 op，则先静态检查编辑后的当前故障 op；若在其前方删除或插入了允许编辑的 op，`OperationIndex` 使用编辑后的实际索引
- 若该轮次仅有**删除**的 op：
  - 先目标检查删除位置之后的第一个 op（按编辑后排序）
  - 若删除的是最后一项，则没有目标 op 快检，但仍必须执行该轮整轮严格检查
- 若同一轮次同时存在修改/追加和删除的 op：
  - 目标检查编辑后最靠前的修改/追加 op；若其前方删除导致索引变化，使用新索引
  - 目标检查通过后仍须执行整轮严格检查，不能以“最多一次目标检查”为由跳过 replay 和断言验证

#### 10.5.2 D1-D4 轮次内编译/验证阶段故障

故障发生时该轮次的源码改动已经生效（code-step 已成功执行），当前源码状态为该轮次改动完成后的状态。由于该轮次原有 op 已经执行完毕，目标 op 快检按该轮次所**追加 op** 的排序，以**最靠前优先原则**进行一次；随后仍须对该轮运行整轮严格检查：

- 先目标检查该轮次排序最靠前的新追加 op
- 不检查该轮次原有 op（原有 op 的 pattern 在当前已被该轮次修改过的源码上可能不再匹配，属于预期行为）
- 重启前基于正确阶段基线执行该轮整轮严格检查，验证原有 op 与追加 op 的完整顺序 replay 和断言

#### 10.5.3 V1-V4 轮次故障

V1-V4 是纯验证轮次，当前源码状态为 D1-D4 全部执行完成后的最终源码。若需要修改业务逻辑，只能在 D4 轮次**既有 operations 的末尾**连续追加一个或多个 op；不得改 D1-D3，也不得修改或删除 D4 原有 op。先目标检查 D4 最靠前的新追加 op；随后仍须基于正确阶段基线对 D4 运行整轮严格检查：

- 先目标检查 D4 轮次排序最靠前的新追加 op
- 不检查 D4 轮次原有 op（原有 op 的 pattern 在 D1-D4 已全部执行完成的最终源码上可能不再匹配，属于预期行为）
- 重启前整轮检查 D4 的原有 op 与追加 op，验证完整顺序 replay 和断言
...
- 重启前先跑目标 op 快检，再跑所有受影响轮的整轮严格检查；若检查失败，继续在当前故障 op 及其后续修复，禁止回头改前置 op。
