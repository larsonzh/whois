...
49/50 缺口验证记录（2026-08-16）：CIDR body contract `4/4 PASS`；CIDR draft TSV `8/9`（`203.0.113.0/24 @ arin` 期望与当前 IANA 实际响应不一致）；Redirect IPv4 与参数化矩阵均 `66/66 PASS`；10x6 稳态 `authMismatchFiles=0/errorFiles=0`；`remote_batch_strategy_suite.ps1` 的 raw/health-first/plan-a/plan-b 全 `[golden] PASS`，总用时 `1775.592s`。直接 `--selftest` 的 Phase C 新增五项断言全 PASS，但完整旧 selftest 仍有 `injection-view-fallback: FAIL` 与网络 WARN/SKIP，不能把 standalone 总结果写成全绿。
