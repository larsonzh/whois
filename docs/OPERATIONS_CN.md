...
- 自检 raw/health-first/plan-a/plan-b（`--selftest-force-suspicious 8.8.8.8`，含 `--selftest-workbuf` + `-DWC_WORKBUF_ENABLE_STATS`）：`out/artifacts/batch_raw/20251231-160549/build_out/smoke_test.log`，`out/artifacts/batch_health/20251231-160835/build_out/smoke_test.log`，`out/artifacts/batch_plan/20251231-161104/build_out/smoke_test.log`，`out/artifacts/batch_planb/20251231-161318/build_out/smoke_test.log`（stderr 已含 `[WORKBUF]`/`[WORKBUF-STATS]`）。
注入视图验证示例：
```bash
