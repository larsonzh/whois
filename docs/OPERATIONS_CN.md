...
post-3.3.0 WP-01 真实一键发布演练复核（2026-08-25）：v3.3.1 发布经 one-click build+verify → statics 同步与提交 → annotated tag v3.3.1 → GitHub/Gitee Release 更新全流程 PASS，过程中无任何故障与不适；tag 指向最终静态产物提交 `4357c6a0`，构建产物 `out/artifacts/20260825-101612`（golden PASS、smoke `18/3/3` alerts=0），Gitee Release body 更新 exit=0，origin/master 已同步。WP-01 关闭（done）。
...
## A/B 会话 guard（低噪声 + V1 自动修复闭环）

- 日志降噪（2026-04-25）：`tools/test/unattended_ab_session_guard.ps1` 的 restart pasted block 采用 begin/end 与分隔线分行输出，便于 grep 与人工复盘，不改变既有判定语义。
- 启动文件新增键（见 `docs/UNATTENDED_AB_START_TEMPLATE_CN.md`）：
  - `LOCAL_GUARD_AUTO_FIX_BUDGET_PER_D`：每个 D 轮可触发的自动修复上限（建议 `3`）。
  - `LOCAL_GUARD_AUTO_FIX_COOLDOWN_SECONDS`：自动修复后的冷却秒数（建议 `20`~`60`）。
  - `LOCAL_GUARD_MANUAL_WAIT_ON_EXHAUST`：预算耗尽后是否进入人工等待（建议 `true`）。
...
- PowerShell 5.1 兼容守则（2026-06-12）：无人值守脚本中禁止使用内联 `$(if(...){...} else {...})` 作为字符串格式化/参数子表达式；统一采用“先计算变量，再格式化/传参”。历史故障表现为 dispatch 链路提前退出并出现 `dispatch_main_failed` / `if cannot be recognized`。
...
LTO 别名审计更正（2026-08-18）：`remote_build_and_test.sh` 过去在 `getopts` 前归一化 `RB_OPT_PROFILE`，因此命令行 `-O lto-auto|lto-serial` 未转换为 Makefile 可识别的 `OPT_PROFILE=lto`；历史标记为 `lto-auto` 的轮次仍可作为跨架构编译、hash、Golden/referral 与业务行为证据，但不能作为实际 LTO 告警证据。现将别名归一化移到参数解析后，并在 `remote_build.sh` 中显式 export `LTO_MODE/LTO_SERIAL/LTO_PARALLEL`，覆盖 POSIX 与 Windows 构建。最终真实 LTO 轮 `out/artifacts/20260818-065540` 的所有架构编译/链接均含 `-flto=auto`，9 架构 Local hash、Golden、三条 referral 全 PASS，release 已同步（455s）。
2026-08-18 提交 `7e1f2fa7` 后复核：Strict lto-auto 默认/debug 两轮无编译与 LTO 告警，Local hash、Golden、referral 全 PASS（`out/artifacts/20260818-071217`，224s；`20260818-072952`，255s）；默认轮 9 架构 POSIX 哈希与真实 LTO 基准轮 `065540` 完全一致（Windows PE 因时间戳不同除外），确认 LTO 真启用且构建确定性同源。Batch 四策略（`073512/073943/074445/075120`，1185.080s）与 Selftest 四策略（`080108/080605/081113/081614`，1201.862s）黄金全 PASS，自检报告均含 `output line matched: 10\.0\.0\.8 is a private IP address`（`--require-line` 真实输出断言生效）。12x6 authority 表空；唯一 `afrinic_45.113.52.0_22` LACNIC rate-limit 复测收敛 APNIC 正确（`redirect_matrix_10x6/20260818-083320`），属外部限流非回归。release 已同步最新 debug 轮产物。
发布候选复核记录（2026-08-16）：当时标记为 Strict `lto-auto`、审计后确认实际为 O3 的默认/debug 两轮无编译告警，9 架构 SHA-256 实算、Local hash、Golden、referral 全 PASS（`out/artifacts/20260816-203059`，276s；`20260816-203903`，286s）；批量四策略 Golden 全 PASS（`204504/205026/205622/210225`，1315.436s），自检四策略 Golden 全 PASS（`210950/211526/212129/212741`，1369.958s）。最终 12x6 authority mismatch 空表、errors=`(no errors found)`（`out/artifacts/redirect_matrix_10x6/20260816-213231`）。原始 APNIC 空响应瞬态仍保留于 `20260816-161935`，本轮无需修复。
...
CIDR 正文契约冒烟任务（2026-02-20）：VS Code 已新增 `Test: CIDR Body Contract`（可参数化）与 `Test: CIDR Body Contract (prefilled)`（一键预填）任务，对应脚本 `tools/test/cidr_body_contract_smoke.ps1`；当前覆盖 first-marker 抑制、post-marker 一致性路径、no-marker 直达与 iana 链路尾行在内的 4 个 case，最近一次本地运行 `pass=4 fail=0`（日志 `out/artifacts/cidr_body_contract/20260220-130633`）。
CIDR 草案矩阵扩展（2026-02-20）：`testdata/cidr_matrix_cases_draft.tsv` 新增 no-marker 跨 RIR、全空间 unknown、invalid-input 三类可执行用例；定向执行 `redirect_matrix_test.ps1 -CasesFile testdata/cidr_matrix_cases_draft.tsv` 结果 `pass=8 fail=0`，日志 `out/artifacts/redirect_matrix/20260220-125430`。VS Code 已新增一键任务 `Test: Redirect Matrix (CIDR Draft TSV)` 与 `Test: Redirect Matrix (CIDR Draft TSV, prefilled)`；组合任务提供参数化与预填两种入口：`Test: CIDR Contract Bundle` / `Test: CIDR Contract Bundle (prefilled)`（脚本入口：`tools/test/run_cidr_contract_bundle.ps1`），其中参数化入口已验证通过，任务等价命令复跑 `pass=4/8, fail=0/0`（日志 `out/artifacts/cidr_body_contract/20260220-135818` 与 `out/artifacts/redirect_matrix/20260220-135910`），并生成汇总文件 `out/artifacts/cidr_bundle/cidr_bundle_summary_20260220-135818.txt`。
...
重定向矩阵 10x6（2026-02-17，追加复跑）：同参数再次复跑，`authMismatchFiles=0、errorFiles=0`，日志 `out/artifacts/redirect_matrix_10x6/20260217-105213`。
重定向矩阵 10x6（2026-02-17，最新复跑）：同参数再次复跑，`authMismatchFiles=0、errorFiles=0`，日志 `out/artifacts/redirect_matrix_10x6/20260217-171711`。
重定向回访修复（2026-02-17）：统一“已访问 RIR 不回访”规则（不限于 ARIN→APNIC），当 referral 指向已访问过的 RIR 时改走未访问 RIR 轮询；`-h apnic 45.113.52.0` 复测不再出现二次 APNIC 回访且尾行权威为 APNIC；`-h lacnic 1.1.1.1` 复测正常。
当前执行建议（2026-02-17）：将 `out/artifacts/20260217-170956`（Strict）与 `out/artifacts/redirect_matrix_10x6/20260217-171711`（matrix）作为本轮基线；后续保持“关键命令单点复测（`-h apnic 45.113.52.0`、`-h lacnic 1.1.1.1`）+ 10x6 全量矩阵”双层验证。
APP-RETRY 干净探测（2026-02-17）：为避免 PowerShell `NativeCommandError` 包装噪音，建议使用 `cmd /c` 包装原生程序并将 stderr 直落日志：`$bin="d:\LZProjects\whois\release\lzispro\whois\whois-win64.exe"; $outDir=".\out\artifacts\app_retry_probe_clean"; $log=Join-Path $outDir "stderr.log"; New-Item -ItemType Directory -Force $outDir | Out-Null; Remove-Item $log -ErrorAction SilentlyContinue; 1..80 | % { cmd /c "\`\"$bin\`\" --debug --retry-metrics --rate-limit-retries 2 --rate-limit-retry-interval-ms 1500 -h arin 45.121.52.0/22 1>nul 2>>\`\"$log\`\"" }; Select-String -Path $log -Pattern "\[APP-RETRY\]" | Select-Object -First 20`。
...
WORKBUF 标记字段说明：`action` 为用例名（如 long-crlf/dense-lf），`result` 为 PASS/FAIL；`len/max_view` 为单次可视长度；`max_request` 为本用例请求的最大读取长度；`reserves` 为初始预留；`grow` 为扩容次数；`max_cap` 为达到的最大缓冲容量；`cases=a/b` 汇总已跑用例数。
压力计划补充（2025-12-31 → 2026-01-09）：fold unique body aliasing 修复已上线（token 视图改用 workbuf scratch），长行/高密度续行/CRLF 手工场景（fold unique + grep 块模式）已跑通，无截断/崩溃；最新远程冒烟 + 黄金日志：默认参数 `out/artifacts/20260109-120735`，`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first` `out/artifacts/20260109-124459`；主控拆分后复跑：`out/artifacts/20260109-145129`（默认）与 `out/artifacts/20260109-145610`（同上 debug/metrics/family），均无告警 `[golden] PASS`。批量 raw/health-first/plan-a/plan-b 黄金：`out/artifacts/batch_raw/20260109-124921/build_out/smoke_test.log`，`out/artifacts/batch_health/20260109-125305/build_out/smoke_test.log`，`out/artifacts/batch_plan/20260109-125524/build_out/smoke_test.log`，`out/artifacts/batch_planb/20260109-125751/build_out/smoke_test.log`；拆分后复跑同样 PASS：`out/artifacts/batch_raw/20260109-150005/...`、`batch_health/20260109-150359/...`、`batch_plan/20260109-150620/...`、`batch_planb/20260109-150841/...`（报告同目录）。自检黄金（`--selftest-force-suspicious 8.8.8.8`）：`out/artifacts/batch_raw/20260109-130135/build_out/smoke_test.log`，`out/artifacts/batch_health/20260109-130353/build_out/smoke_test.log`，`out/artifacts/batch_plan/20260109-130611/build_out/smoke_test.log`，`out/artifacts/batch_planb/20260109-130839/build_out/smoke_test.log`；拆分后复跑同样 PASS：`out/artifacts/batch_raw/20260109-151205/...`、`batch_health/20260109-151420/...`、`batch_plan/20260109-151631/...`、`batch_planb/20260109-151848/...`。自检黄金默认编译已包含 `-DWC_WORKBUF_ENABLE_STATS`，配合 `--debug` 可在 stderr 看到 `[WORKBUF-STATS]`。如需本地/远端快速回归，可追加 `--selftest-workbuf` 触发内建长行/CRLF/高续行覆盖，stderr 会输出 `[WORKBUF]`/`[WORKBUF-STATS]` 供黄金校验。手工 workbuf 压力重跑（fold+unique、grep-block）PASS：`out/artifacts/workbuf_stress/20260109-133951`（退出码 1 原因为自测内置 force-suspicious 触发）。批量渲染 crash 修复后的四轮黄金回归：远程冒烟+黄金 `out/artifacts/20260109-214157`（默认）与 `out/artifacts/20260109-214706`（debug/metrics/family）；批量 raw/health-first/plan-a/plan-b `out/artifacts/batch_raw/20260109-215119/...`、`batch_health/20260109-215529/...`、`batch_plan/20260109-215904/...`、`batch_planb/20260109-220154/...`（报告同目录）全 PASS；自检黄金 `out/artifacts/batch_raw/20260109-220449/...`、`batch_health/20260109-220718/...`、`batch_plan/20260109-220945/...`、`batch_planb/20260109-221213/...` 全 PASS。渲染入口下沉后复跑：远程冒烟+黄金 `out/artifacts/20260109-225321`（默认）与 `out/artifacts/20260109-225816`（debug/metrics/family）PASS；批量 raw/health-first/plan-a/plan-b `out/artifacts/batch_raw/20260109-230239/...`、`batch_health/20260109-230638/...`、`batch_plan/20260109-230940/...`、`batch_planb/20260109-231213/...`（报告同目录）PASS。自检 plan-b referral 校验失败：`out/artifacts/batch_planb/20260109-232630/...` 缺少 whois.afrinic.net redirect/尾行（143.128.0.0），需对比黄金 referral 期望并复查日志。自检复跑后四向全 PASS：`out/artifacts/batch_raw/20260109-234646/...`、`batch_health/20260109-234931/...`、`batch_plan/20260109-235158/...`、`batch_planb/20260109-235420/...`。
日志出口归一化复跑：远程冒烟+黄金 `out/artifacts/20260110-013330`（默认）与 `out/artifacts/20260110-013821`（`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）均 PASS。
批量错误路径瘦身复跑：远程冒烟+黄金 `out/artifacts/20260110-020138`（默认）与 `out/artifacts/20260110-020851`（`--debug --retry-metrics --dns-cache-stats --dns-family-mode interleave-v4-first`）均 PASS；批量 raw/health-first/plan-a/plan-b 全 PASS（`out/artifacts/batch_raw/20260110-021336/...`、`batch_health/20260110-021724/...`、`batch_plan/20260110-021950/...`、`batch_planb/20260110-022212/...`）；自检 raw/health-first/plan-a/plan-b 全 PASS（`out/artifacts/batch_raw/20260110-022514/...`、`batch_health/20260110-022750/...`、`batch_plan/20260110-023009/...`、`batch_planb/20260110-023227/...`）。
批量行规范化/渲染拆分修复后，远程冒烟 + 黄金（默认）无告警 `[golden] PASS`，日志 `out/artifacts/20260109-154512`。

最新批量/自检黄金（2026-01-09）：
- 批量 raw/health-first/plan-a/plan-b：`out/artifacts/batch_raw/20260109-124921/build_out/smoke_test.log`，`out/artifacts/batch_health/20260109-125305/build_out/smoke_test.log`，`out/artifacts/batch_plan/20260109-125524/build_out/smoke_test.log`，`out/artifacts/batch_planb/20260109-125751/build_out/smoke_test.log`（各自 golden_report 同目录）。
- 自检 raw/health-first/plan-a/plan-b（`--selftest-force-suspicious 8.8.8.8`，含 `--selftest-workbuf` + `-DWC_WORKBUF_ENABLE_STATS`）：`out/artifacts/batch_raw/20260109-130135/build_out/smoke_test.log`，`out/artifacts/batch_health/20260109-130353/build_out/smoke_test.log`，`out/artifacts/batch_plan/20260109-130611/build_out/smoke_test.log`，`out/artifacts/batch_planb/20260109-130839/build_out/smoke_test.log`（stderr 已含 `[WORKBUF]`/`[WORKBUF-STATS]`）。
注入视图验证示例：
```bash
...
- 无 IPv6 环境下的 v6-only 异常观测：
  ...
  # 预期 stderr: [DNS-CAND] 仅 v6，可能 ENETUNREACH/EHOSTUNREACH/ETIMEDOUT；[RETRY-METRICS*] 记录失败，stdout 仍保持契约（失败尾行 error）
  ```
...
1. 完整示例（远端抓取 + `[SELFTEST] action=*` 断言）：
  ...
  - `-SelftestExpectations` / `-ErrorPatterns` / `-LinePatterns` / `-TagExpectations` 为分号分隔列表，分别转换成 `--expect`、`--require-error`、`--require-line`、`--require-tag 组件 正则`；留空或输入 `NONE` 即视为跳过。默认 `LinePatterns` 要求完整正文行 `10\.0\.0\.8 is a private IP address`，防止仅靠启动 marker 误判 force-private 已进入真实私网分支；匹配前会归一化 CRLF 行尾。
...
- 可选开关：`-SkipRaw/-SkipHealthFirst/-SkipPlanA/-SkipPlanB`、`-RemoteGolden`（同时启用远端 `-G 1`）、`-NoGolden`（仅抓日志不跑本地黄金）、`-SkipRemote`（复用各目录最新日志，仅重放本地黄金）、`-DryRun`（只打印命令），`-SelftestActions 'force-suspicious,*;force-private,10.0.0.8'`（批量透传到黄金脚本），以及 `-RemoteExtraArgs "-M nonzero"` / `-GoldenExtraArgs ''` 等。若需在四轮远程冒烟时统一追加额外客户端参数（例如 `--selftest-force-suspicious '*' --selftest-force-private 10.0.0.8`），可使用 `-SmokeExtraArgs "..."`，无需手动修改基础 `-a '--debug --retry-metrics ...'` 字符串。
...
- 终态总结票分发存活保障（2026-05-19）：`tools/test/unattended_ab_takeover_trigger.ps1` 在 `chat-session-final-status` 分发路径新增“启动后存活校验 + 快速重试”。可通过 `AI_CHAT_FINAL_TRIGGER_VERIFY_MS`（默认 1200）与 `AI_CHAT_FINAL_TRIGGER_MAX_ATTEMPTS`（默认 2）调节；若分发未确认成功，trigger 会记录 `auto_stop_deferred` 并继续驻留，不会提前 auto-stop。
