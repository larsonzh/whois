...
CIDR 正文契约冒烟任务（2026-02-20）：VS Code 已新增 `Test: CIDR Body Contract`（可参数化）与 `Test: CIDR Body Contract (prefilled)`（一键预填）任务，对应脚本 `tools/test/cidr_body_contract_smoke.ps1`，最近一次本地运行 `pass=2 fail=0`（日志 `out/artifacts/cidr_body_contract/20260220-124043`）。
