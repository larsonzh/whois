...
- 必跑门禁（顺序固定）：
  ...
  4. `Test: Step47 PreRelease Check (reserved, list file)`（启用 preclass gate）
     ...
     - 默认附带 `write-core-guard` 专项：校验 start-file 写盘核心保持共享化（无本地 wrapper、无旧调用）。
...
- D. 最小命令块（可复制执行）
   ...
   # 如需在 dry-run 门禁中同时验证“事件队列幂等基线”回归，可加：-IncludeEventQueueIdempotentRegression true
   ...
   # 发布前若要纳入事件票幂等回归门禁：追加 -IncludeEventQueueIdempotentRegression true
