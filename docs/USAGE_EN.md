...
- Smart redirects: non-blocking connect, timeouts, light retries, and referral following with loop guard (`-R`, disable with `-Q`).
  ...
  - CIDR terminal lookback preconditions (2026-02-22): run pre-APNIC candidate baseline lookback only when all conditions hold together: CIDR query, APNIC ERX/IANA marker observed, pre-APNIC candidate RIR exists (excluding IANA/APNIC), and the RIR cycle is exhausted. Any hit converges to `unknown`; all misses fall back to APNIC only when no unresolved failure debt remains.
  - Failure-debt settlement (2026-03-03): for the same RIR, if the original query item fails (including empty-response retry exhaustion) without a determinable body, record unresolved failure debt; debt is settled only when the same RIR later returns a determinable body for the same original query item. If the RIR cycle is exhausted with unresolved debt, terminal authority converges to `error`.
  ...
  - LACNIC→ARIN hard-rule refinement (2026-02-22): when LACNIC internally points to ARIN and the query is a non-IP literal, force immediate non-authoritative continuation and do not pre-mark ARIN as visited; this prevents ARIN-side rule handling (including query-prefix injection paths) from being short-circuited by premature visited state.
...
- Authoritative tail tightening: if unresolved failure debt remains (including rate-limit/denied/connect failure or empty-response retry exhaustion) and convergence is not achieved, the authoritative tail prints `error` to distinguish failure from a true unknown.
...
WP-13D HTTPS proxy TLS preparation (2026-09-02): CA/static-OpenSSL build preparation and Vx A/B task-definition authoring are complete. A consolidates custom-transport ownership in `wc_net_info`; B adds `https://`, mandatory certificate/hostname verification, the pinned CA or a fail-closed `SSL_CERT_FILE` override, and TLS handshake + CONNECT under one absolute deadline. The A→B full-definition check has zero errors and warnings, and default/TLS x86_64 and win64 quick builds pass. The feature is not enabled on master; nine-architecture production gates, start-file generation, and execution remain pending. See section 14 of `docs/RFC-proxy-access.md`.

WP-13C SOCKS4/4a complete (2026-09-01): `socks4://` sends only locally resolved IPv4 candidates to the proxy (SOCKS4 does not support IPv6); `socks4a://` delegates an unbypassed domain to proxy-side DNS and shares the `socks5h://` target family/fallback/RIR-override conflict gates and target-health isolation. USERID is the resolved credential username; the password never enters the frame. The loopback protocol/configuration spike passes 27/27 cases; both Vx A/B stages pass 8/8 rounds on the first attempt with no incidents/self-heals/restarts (A run=`out/artifacts/dev_verify_multiround/20260831-150933`, B run=`out/artifacts/dev_verify_multiround/20260831-205121`, combined A/B `0d 12:01:40`, session start 2026-08-31 15:08:58). The final Strict Version `lto-auto` remote build + smoke sync + Golden check passes with no warnings (`out/artifacts/20260901-121843`, 755s; Local hash verify, Golden, and referral all PASS). Default direct connect, stdout, RIR referral, DNS health, batch strategy, and retry-metrics contracts remain unchanged; HTTPS proxy (WP-13D) stays gated by section 9 of the proxy RFC.

WP-13B-3 SOCKS5/5h data plane complete (2026-08-31): `socks5://` keeps local target resolution and sends IPv4/IPv6 ATYP, while `socks5h://` delegates an unbypassed domain to proxy-side DNS without invoking local target DNS, DNS health/backoff, or empty-response IP fallback. RFC 1929 username/password and complete REP classification are supported, and the target IP remains unknown after remote resolution. The loopback protocol/configuration spike passes 21/21 cases; the final Strict `lto-auto` nine-architecture build, all 9 hashes, POSIX/QEMU and Windows smoke, Golden, and all three referral origins pass (`out/artifacts/20260831-112633`, 436s). The final run with release synchronization (2026-08-31) has no warnings, no LTO warnings, and Local hash verify/Golden/referral all PASS in 284s with both release directories byte-for-byte matching `SHA256SUMS-static.txt` (`out/artifacts/20260831-114158`). SOCKS4/4a and HTTPS proxy are not enabled.

WP-13B-2 HTTP CONNECT proxy and per-hop bypass complete (2026-08-31): both Vx A/B stages passed 8/8 rounds (A run=`out/artifacts/dev_verify_multiround/20260830-133709`, B run=`out/artifacts/dev_verify_multiround/20260831-032025`, combined A/B `0d 18:51:00`). B's first run hit `runtime-fail` at D1; after three orchestration-script fixes and A-snapshot anchor recovery it was restarted and completed 8/8 from D1. The final Strict Version `lto-auto` remote build + smoke sync + Golden check passes with no warnings (`out/artifacts/20260831-091653`, 254s; Local hash verify, Golden, and referral checks all PASS). Default direct connect, stdout, RIR referral, DNS health, batch strategy, and retry-metrics contracts remain unchanged; WP-13C/13D remain gated by section 9 of the proxy RFC.

WP-13B-1 transport baseline and final-artifact review (2026-08-30): the endpoint-dialer policy preserves `wc_dial_43` default-direct and target DNS-health semantics. Lookup I/O now runs through an injectable `read/write/wait/close` transport adapter with a shared absolute monotonic deadline. The deterministic transport contract and endpoint-family/health-isolation selftests pass across Linux and Windows (`out/artifacts/20260830-010926`, `20260830-012006`). After the code adjustments, the final Strict `lto-auto` synchronized nine-architecture round has no compiler/LTO warnings; independent SHA-256 recomputation for all nine binaries, Golden, all three referral origins, and default smoke pass (`out/artifacts/20260830-014939`, 241s). That default smoke round did not enable `--selftest` and does not replace the focused evidence above. Public CLI and default output contracts are unchanged.
...
- 2026-08-17 follow-up: default/debug Strict runs pass Local hash, Golden, and referral (`20260817-000833`, `20260817-001953`); all four Batch and Selftest Golden strategies pass (`002627/003207/003826/004414`, `010022/010604/011235/011843`). The 12x6 authority-mismatch table is empty; the only LACNIC `45.113.52.0/22` rate-limit error recovered to APNIC in the same-parameter targeted rerun and is retained as transient evidence at `out/artifacts/redirect_matrix_10x6/20260817-014619`.
- Terminal failed-node retries: after the RIR cycle is exhausted with unresolved authority, each RIR that produced denial, rate limiting, or persistent empty responses receives one bounded single-hop retry. Only an authoritative retry settles failure debt; non-authoritative or failed retries preserve the original terminal semantics. `[TERMINAL-RETRY]` is emitted to stderr only. Validation: P0 `12/12`, special `17/17`, CIDR `4/4 + 9/9`, 12x6 `authMismatchFiles=0 errorFiles=0` (`out/artifacts/redirect_matrix_10x6/20260816-194455`), and full-architecture Strict PASS (`out/artifacts/20260816-201756`, 325s).
- Latest release-candidate verification: the default and debug Strict `lto-auto` rounds have no compile/LTO warnings and pass the nine-architecture SHA-256, Local hash, Golden, and referral checks (`out/artifacts/20260816-203059`, 276s; `20260816-203903`, 286s). All four batch goldens pass (`204504/205026/205622/210225`, 1315.436s), and all four selftest goldens pass (`210950/211526/212129/212741`, 1369.958s). The final 12x6 run has an empty authority-mismatch table and no errors (`out/artifacts/redirect_matrix_10x6/20260816-213231`). The earlier empty-APNIC transient remains preserved under `20260816-161935`; the affected case converged normally in this run.
...
- Selftest goldens (2026-02-23, current round, `--selftest-force-suspicious 8.8.8.8`): raw/health-first/plan-a/plan-b all `[golden-selftest] PASS`, logs `out/artifacts/batch_raw/20260223-070056`, `batch_health/20260223-070613`, `batch_plan/20260223-071033`, `batch_planb/20260223-071536` (total 1155.471s).
- Redirect matrix (12x6, 2026-02-23, current round): authority mismatch table is empty and `errors=(no errors found)`, log `out/artifacts/redirect_matrix_10x6/20260223-072410`.
- Golden-script compatibility recheck (2026-02-23, current round): all `golden_report*.txt` and `golden_selftest_report.txt` are PASS, confirming the golden assertion chain remains stable after output-format updates.
...
- Redirect matrix (12x6, including `47.96.0.0/10`): `authMismatchFiles=0 errorFiles=0`, logs `out/artifacts/redirect_matrix_10x6/20260219-051415`.
...
- Bundle script: `tools/test/run_cidr_contract_bundle.ps1` (runs CIDR body-contract smoke first, then CIDR draft matrix)
- Tasks: Test: Redirect Matrix (IPv4), Test: Redirect Matrix (IPv4, Params), Test: Redirect Matrix (CIDR Draft TSV), Test: Redirect Matrix (CIDR Draft TSV, prefilled), Test: CIDR Contract Bundle, Test: CIDR Contract Bundle (prefilled)
...
- `--selftest-force-private <query|*>` follows the same pattern for private-IP handling and takes precedence over Phase C early convergence. A forced hit renders the standard private-IP body/tail and exits for the affected query; stderr receives `[SELFTEST] action=force-private ...` and `Error: Private query denied`, so smoke tests exercise the real private branch. Normal implicit private queries (including batch) that do not match this hook still emit Address Status and early-converge to `unknown @ unknown`; explicit `-h` keeps the legacy private output.
...
- `[DNS-CAND-UNIQ]` – emitted only with `--debug` or `--retry-metrics`; reports per-hop candidate uniqueness (`total/unique/duplicate`) to observe duplicate density, without changing candidate order or query semantics.
...
Runtime / query options:
  ...
  -J, --retry-jitter-ms MS    Extra random jitter in milliseconds (0..MS, default 300)
      --rate-limit-retries N   App-layer retries for temporary denied/rate-limit responses (default 2, range 0..10)
      --rate-limit-retry-interval-ms M  Sleep between app-layer retries in ms (default 2500)
...
- `--preclass-action-list <csv>`: override the P1 action candidate list (exact CSV match, case-insensitive; unset or `default` keeps tier defaults; `default` accepts surrounding whitespace).
...
- `--enable-preclass-early-converge`: Phase C reserved/special early convergence, on by default. High-confidence `reserved|special` with `rir=none` may short-circuit implicit queries. IANA special-purpose matches add an `=== Address Status: ... ===` line and normalize the authoritative tail to `unknown @ unknown`. Explicit `-h` still queries and renders the selected server body, while successful responses receive the same authority normalization. Low-confidence, allocated/legacy, and non-`none` RIR cases remain excluded. Use `--disable-address-preclass` for a full rollback to the legacy path.

Notes:
- Explicit `-h` remains compatibility-first and bypasses Step 4.7 short-circuit behavior.
- Address Status appears only in normal output; `--plain`/`--fold` add no structural line, and folded output keeps `unknown` as its RIR token.
- `tools/test/preclass_special_registry_matrix.ps1` covers 10 offline special-purpose cases; `-RunExplicitHosts` adds seven TEST-NET-3 start-host consistency cases.
