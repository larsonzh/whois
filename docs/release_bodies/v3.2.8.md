...
---
## Multi-arch retry metrics excerpt
...
- Numeric errno difference does not alter classification logic; switch is symbolic.
...
> 链接在正式发布后由 CI 自动生成/上传；若网络访问受限，可在后续补充镜像地址。
> Links are uploaded by CI after tagging; mirror URLs may be added later if regional access is slow.
