...
- DNS 第一阶段（服务器解析）奠基：引入以 `AI_ADDRCONFIG` 为基础的解析策略与 IPv4/IPv6 家族控制（仅/优先），对解析得出的多候选做去重与上限，为后续 3.2.8 的三跳稳定化与 `@ <ip|unknown>` 观测统一打基础（本版用户可见行为保持稳定）。
...
- DNS phase‑1 groundwork: introduce `AI_ADDRCONFIG`‑aware resolution, IPv4/IPv6 family controls (only/prefer), and de‑dup/cap of address candidates; this paves the way for 3.2.8’s stabilized three‑hop and unified `@ <ip|unknown>` observability (no user‑visible changes in this tag).
