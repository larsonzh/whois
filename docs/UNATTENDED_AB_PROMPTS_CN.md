...
- 运行期工单由现有 guard/trigger/dispatch 链生成并投送到会话。AI 只需静默等待已投送的事件驱动票或状态票；收到后严格按 `next_command_order` 执行，不遗漏操作，回执必须真实准确并完成校验与票据闭环。不得自行启动 heartbeat/poll 定时巡检，不得创建监控脚本、循环、后台 job、watcher、常驻 PowerShell 命令或长时间跨轮次巡检命令；等待本身不需要执行任何命令。通过标准 stage window 重启主进程后，必须在 3 分钟内完成当前自愈/故障处理的收尾、回执校验与 `mark_processed`，然后静默等待；3 分钟不是巡检窗口。
...
11. 运行期静默等待 guard/trigger/dispatch 投送到会话的事件驱动票或状态票；收到后严格按票据 `next_command_order` 执行其中所有无需用户确认的预授权操作，不得遗漏。回执必须真实准确并有证据支撑，完成 handled_at 校验与 mark_processed 后继续静默等待。不得自行定时调用 heartbeat 或 `poll_agent_tickets.ps1`，不得创建任何定时巡检监控脚本、轮询循环、后台 job、watcher、常驻内存命令或长时间跨轮次巡检命令。通过标准 stage window 重启主进程后，必须在 3 分钟内完成当前处置的收尾、回执校验和 mark_processed；无法完成则立即如实报告阻塞，不得转为主动巡检。
...
运行期只需静默等待 guard/trigger/dispatch 投送到会话的事件驱动票或状态票；收到后严格按 `next_command_order` 执行全部预授权操作，不遗漏任务，以真实准确的 handled_at 完成校验与票据闭环，再继续静默等待。不得主动定时执行 heartbeat 或 poll，不得创建巡检脚本、轮询循环、后台 job、watcher、常驻内存命令或长时间跨轮次巡检命令。重启主进程后 3 分钟内必须完成当前处置的收尾、回执校验与 mark_processed；该期限不是巡检窗口。对 low-disturb 的 running-status-report，正常时只回“运行正常”+ handled_at，异常时只回异常摘要+handled_at，等待独立事故票。
...
运行期不要结束会话；静默等待现有 guard/trigger/dispatch 链投送事件票或状态票，收到后严格按 next_command_order 执行全部预授权操作，不遗漏任务，以真实准确的 handled_at 完成校验与票据闭环，再继续等待。不得自建或运行任何定时巡检脚本、轮询循环、后台 job、watcher、常驻内存命令或长时间跨轮次巡检命令。重启主进程后 3 分钟内完成当前处置收尾、回执校验与 mark_processed，不把该期限用于巡检。状态票只读汇报 SESSION/A/B、main_round、进程/监控存活、heartbeat 与待处理事故票，并回传 handled_at。
